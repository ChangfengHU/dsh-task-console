...
- [ ] [TASK-SESSION-CAPABILITY-033] 0.30.28 已双零窗口部署：能力目录/Capabilities、受限浏览器身份过滤与执行拒绝、重复结果与24步预算。修复真实启动数据库顺序及完整角色提示覆盖问题；真实Qwen原问题29.337秒正常结束，只调用两个只读目录，明确列出受限浏览器操作；公网1440/390通过。Task/定时/Action哈希不变，原异常历史保留。剩余：模型部分措辞仍将注册称为可用；自动委派、CLI私有观测、独立搜索认证未实现/修复。测试与回执见 `dev-log/2026-09-15-session-capability-truth.md`。
