...
- [ ] [TASK-WORKFLOW-REEXECUTE-009] 0.30.3：用户确认仅隐藏装机Task旧执行并在同Task对63重跑，保留双浏览器20分钟验收。已新增Batch级归档及调度隔离，206回归/构建通过并部署。2026-09-10 09:43:40 UTC仅归档 b-chat-b4c6fcb369f0c20a9739；前后Task定义、核心图/事件、Cards、Runs哈希一致，另31个Task及其他Batch未改。私有在线备份 batch-archive-backup-VPU3WJ/task.db integrity_check=ok。公网浏览器验证旧记录默认隐藏、展开可选、原链接可读且不能取消，无pageerror。旧阻塞保留，不伪造自动恢复；新完整业务验收尚未启动。此次用户确认取代下方0.30.2的待答状态。
