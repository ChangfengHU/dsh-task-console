...
- [ ] [TASK-WORKFLOW-REEXECUTE-009] 0.30.4：仅归档旧Batch b-chat-b4c6fcb369f0c20a9739，32Task及全部历史保留。新Batch b-chat-53c7651dd3a78824d506 装机十阶段/Runner八项签名检查通过；浏览器复用登录，操作32e5614ad7e91082499a2894da1b084b于10:16:13 UTC完成双实例各21样本、完整20分钟stable=true。但Qwen收到通用催交后仍引用旧running上下文且未调用task_complete，Batch于10:16:31真实failed，不能宣称整个工作流成功。已核对工具确实暴露、非token截断；新增带当前同会话终态回执的异步续接提醒，不自动代交、不换模型、不放宽验收。取消态归档及旧观察器参数提示一并补齐，208测试/构建通过，零运行后部署；待新执行验证模型实际收尾。第一失败Batch/旧归档/另一个巡查Task不删不改，定时仍关闭。详见既有工作流dev-log最新段。
