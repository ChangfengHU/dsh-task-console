...
- [ ] [MANUAL-FLEET-WITHDRAW-20260923] Fleet c3ff0b7 pushed/deployed: manual recycle preserves ALL credentials/access/remote data, disables Runner and refuses active jobs;31 Fleet tests pass. Existing fleet-ops stale MCP connection regenerated with host references; separate manual Action installed, no global reload or auto-retirer changes. Browser-created session agent-fleet-ops-mue2yyrn handles only authorized236 legacy noVNC preparation. Actual installation/withdrawal acceptance remains pending; user approval is no longer pending.

- [ ] [ONBOARD-PROBE-DIAGNOSTICS-20260923] Scoped bb3a01f inspection deployed without global reload;48 DSH and89 host tests pass. Browser-created installer session agent-fleet-installer-mudto47m found236 port6080 held by unmanaged novnc.service; public Chrome31.72s, no page errors. Deletion-preflight session agent-fleet-node-retirer-mudtrsm2 correctly finds236 unregistered and makes zero mutations; public Chrome39.97s. Actual installation, adoption/replacement scope and real deletion target/scope remain pending; do not call preflight final acceptance. Existing global integrated55ea995 remains until dual-zero full release; preserve Studio Run1110 and unrelated lib edits. Prior Runs1105/1106 hot-reload interruption retained/documented. See HANDOFF and existing model-startup-fallback log.
