...
- Production smoke found that `allow: []` was evaluated at the final Agent scope and hid the preset's own runtime and Skill registrations. The generator now emits an exact allow-list for the selected native/runtime/MCP/Skill schemas; the same 81-test suite and build pass with regression coverage.
