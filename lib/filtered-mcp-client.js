...
function isTaskNotification(exec) {
...
function resolveSourceConfig(ctx, sourceEntryId) {
  ...
  if (!entry) {
    const taskConsole = ctx.get?.("taskConsole");
...
function assertBrowserSession(rawName, candidate, exec) {
  ...
  if (!actual || candidate.sessionId !== actual) throw new Error("MCP browser sessionId must match this live Agent session");
}
function bindBrowserSessionDefinition(rawName, definition) {
...
\u5F53\u524D sessionId \u7531 DSH \u5BBF\u4E3B\u81EA\u52A8\u7ED1\u5B9A\uFF0C\u65E0\u9700\u586B\u5199\uFF1B\u5176\u4ED6\u5DE5\u5177\u6743\u9650\u548C Task \u8303\u56F4\u4FDD\u6301\u4E0D\u53D8\u3002`,
    parameters: { ...schema, properties, required: schema.required?.filter((key) => key !== "sessionId") },
    execute(args, exec) {
      const session = exec?.agent?.session;
      const actual = session?.id ?? session?.header?.id;
        ...
        return (definition) => {
          ...
          return target.register(bindBrowserSessionDefinition(rawName, {
            ...
            execute(args, exec) {
              ...
              if (rawName.replace(/-/g, "_") === "vyibc_wecom_send_message" && String(exec?.agent?.session?.id ?? "").startsWith("task-") && !isTaskNotification(exec)) throw new Error("Task notifications must use task_notify with reviewed recipients and durable deduplication");
              if (/^fleet-proxy(?:-|$)/.test(stableServerName) && rawName.startsWith("proxy_") || /^browser_login_(copy|provision|resume)$/.test(rawName)) {
                const console = ctx.get?.("taskConsole");
                if (console?.scopedMcp) return console.scopedMcp(rawName, args, exec, (next) => execute(next, exec));
                if (String(exec?.agent?.session?.id ?? "").startsWith("task-")) throw new Error("Task MCP scope guard unavailable");
              }
