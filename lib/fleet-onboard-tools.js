  ...
  async assessed(operation, ip, profile, exec) {
    ...
    try {
      return await withCredentialFile(lease, this.config, async (env) => {
        ...
        run = driven.run;
        const observedReason = inventory.components?.["clash-control-plane"]?.reason_code;
        const dashboardDiagnostic = run.currentStage === 5 && run.status !== "complete" && (/* @__PURE__ */ new Set([
