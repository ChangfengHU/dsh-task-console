  ...
  async drive(run, inventory, assessment, history, env, signal) {
    ...
    while (run.currentStage < 10) {
      ...
      if (observed.disposition !== "repairable" || !executionConfigured || probeGate && !unattendedProbeRepair) {
        const reason = probeGate && !unattendedProbeRepair && observed.disposition === "repairable" ? "probe-gate-not-satisfied" : !executionConfigured && observed.disposition === "repairable" ? isCloudStage(stage.stage) ? "cloud-transport-not-configured" : "executor-not-configured" : safeReason(observed.reason, "component-blocked");
        const classified = probeGate && !unattendedProbeRepair && observed.disposition === "repairable" ? "needs-user" : !executionConfigured && observed.disposition === "repairable" ? "repairable" : failureClass(observed.disposition);
