...
import { access, chmod, lstat, mkdtemp, open, readFile as readFile2, realpath, rm, stat as stat2, writeFile } from "node:fs/promises";
...
async function taskCredential(ip, sessionId, root = join(process.env.DSH_HOME ?? join(homedir(), ".dsh"), "task-console")) {
  ...
  try {
    ...
    const binding = db.prepare(`SELECT c.batch_id FROM dsh_run_bindings b
      ...
      AND r.status = 'running' AND t.status = 'running'`).get(sessionId);
    ...
    if (!info.isFile() || info.mode & 63 || info.uid !== process.getuid?.()) return absent;
    const value = JSON.parse(await readFile(path, "utf8"));
    const credential = value.expiresAt > Date.now() && value.credentials?.find((c) => c.ip === ip);
...
var ScopedFleetMcpClient = class {
  ...
  async call(tool, args, signal) {
    ...
    if (!envelope || typeof envelope !== "object" || envelope.error || !envelope.result || !Array.isArray(envelope.result.content)) {
    ...
    const knownError = LEDGER_DIAGNOSTICS.get(payload.error);
    if (knownError && (payload.ok === false || envelope.result.isError === true)) {
      throw new LedgerDiagnosticError(knownError);
    }
    if (envelope.result.isError === true) throw new Error("fleet-onboard-ledger-call-failed");
    return payload;
  }
};
var LEDGER_DIAGNOSTICS = /* @__PURE__ */ new Map([
  ["active run belongs to a different mode or session; use an explicit resume/handoff", "ledger-explicit-handoff-required"],
  ["an active run cannot be handed to another session", "ledger-session-lease-active"],
  ["active run is leased by another worker", "ledger-worker-lease-active"],
  ["run changed or write challenge does not match", "ledger-cas-conflict"],
  ["run changed or is leased by another worker", "ledger-cas-conflict"],
  ["run lease expired; resume before writing", "ledger-lease-expired"]
]);
var LedgerDiagnosticError = class extends Error {
};
...
var VaultFirstCredentialProvider = class _VaultFirstCredentialProvider {
  ...
  async resolve(ip, exec, preferManaged = false) {
    ...
    const root = await mkdtemp(join2(this.config.tempRoot ?? tmpdir(), "dsh-fleet-vault-"));
    await chmod(root, 448);
    const file = join2(root, "credential.json");
    try {
...
async function prepareCommand(command, label) {
  ...
  const metadata = await stat2(executable);
...
async function dshOnboardGroupId() {
  try {
    const rows = (await readFile2("/etc/group", "utf8")).split(/\r?\n/);
...
async function prepareConfig(config) {
  ...
  const contractMetadata = await stat2(contractPath);
  ...
  const contract = JSON.parse(await readFile2(contractPath, "utf8"));
...
var AdapterProcessError = class extends Error {
  constructor(diagnostic) {
    super("fleet-onboard-adapter-process-failed");
    this.diagnostic = diagnostic;
  }
};
function adapterFailureCode(stderr) {
  try {
    ...
    if ((/* @__PURE__ */ new Set([
      ...
      "probe-executor-unavailable",
      "probe-result-identity-mismatch",
      "probe-target-fingerprint-required",
      "probe-result-shape-invalid",
      "host-adapter-config-required",
      "host-adapter-config-unavailable",
      "host-adapter-config-not-private-to-service",
      "host-adapter-config-invalid",
      "runtime-unavailable"
    ])).has(value.error)) return value.error;
  } catch {
  }
...
`);
  ...
  if (code !== 0) {
    stdout.fill(0);
    throw new AdapterProcessError(failureCode);
  }
  const lines = stdout.toString("utf8").split(/\r?\n/).filter((line) => line.trim());
  stdout.fill(0);
...
async function withCredentialFile(lease, config, task) {
  ...
  try {
    ...
    if (lease.file) {
      ...
      root = await mkdtemp(join2(config.tempRoot ?? tmpdir(), "dsh-fleet-intake-"));
      await chmod(root, 448);
      const credentialFile = join2(root, "credential.json");
...
function startMode(inventory, assessment, prior, config) {
  if (prior.ok && prior.run && ["running", "blocked"].includes(prior.run.status) && prior.run.executorVersion === config.executorVersion && prior.run.contractVersion === config.contractSha256) return prior.run.mode;
...
function isHostStage(stage) {
  return stage === 2 || stage === 4 || stage === 9;
}
function stageOperationId(run, stage, attempt) {
...
function continuation(phase, executionAvailable, failureClass2) {
  return executionAvailable && (phase === "running" || phase === "blocked" && failureClass2 === "repairable") ? {
    can_resume: true,
    next_tool: "fleet_onboard_resume",
  ...
  } : phase === "blocked" && executionAvailable && failureClass2 === void 0 ? {} : { can_resume: false };
}
function projectLedger(operation, ip, value, executionAvailable) {
  ...
  const latest = [...stages].filter((row) => row.stage === run.currentStage + 1).sort((a, b) => b.attempt - a.attempt)[0];
  const result = {
    schema: 1,
    ok: true,
    operation,
    ...
    phase: run.status,
    execution_available: executionAvailable,
    needs_input: run.status === "blocked" && latest?.failure_class === "needs-user",
    run_created: false,
    probe_executed: false,
    run_id: run.id,
    revision: run.revision,
...
var SubprocessFleetOnboardAdapter = class _SubprocessFleetOnboardAdapter {
    ...
    if (prepared.executor) {
      try {
        ...
        const configured = expectedStages.every((expected) => {
          ...
          return stage?.component === expected.id && stage.executor_id === expected.executor_id && stage.execution_mode === expectedMode && stage.execution === "configured";
        });
        if (capabilities.schema !== 1 || capabilities.ok !== true || capabilities.probe !== "configured" || !configured) prepared.executor = void 0;
      } catch {
  ...
  async assess(inventory, env, signal) {
    const state = await mkdtemp(join2(this.config.tempRoot ?? tmpdir(), "dsh-fleet-assess-"));
  ...
  async drive(run, inventory, assessment, history, env, signal) {
    ...
    while (run.currentStage < 10) {
      ...
      const executionConfigured = isHostStage(stage.stage) ? Boolean(this.config.executor) : isCloudStage(stage.stage) ? Boolean(this.config.cloud) : false;
      const unattendedProbeRepair = stage.stage === 9 && probeGate && executionConfigured && observed.disposition === "needs-user" && observed.reason === "probe-gate-not-satisfied";
      const effectiveDisposition = unattendedProbeRepair ? "repairable" : observed.disposition;
      if (effectiveDisposition !== "repairable" || !executionConfigured || probeGate && !unattendedProbeRepair) {
        const reason = probeGate && !unattendedProbeRepair && effectiveDisposition === "repairable" ? "probe-gate-not-satisfied" : !executionConfigured && effectiveDisposition === "repairable" ? isCloudStage(stage.stage) ? "cloud-transport-not-configured" : "executor-not-configured" : safeReason(observed.reason, "component-blocked");
        const classified = probeGate && !unattendedProbeRepair && effectiveDisposition === "repairable" ? "needs-user" : !executionConfigured && effectiveDisposition === "repairable" ? "repairable" : failureClass(effectiveDisposition);
  ...
  async assessed(operation, ip, profile, exec) {
    ...
    let lease;
    try {
      lease = await this.config.credentials.resolve(ip, exec, operation === "resume");
    } catch {
    ...
    try {
      return await withCredentialFile(lease, this.config, async (env) => {
        ...
        if (operation === "start") {
          ...
          if (prior.run && ["running", "blocked"].includes(prior.run.status) && prior.run.sessionId !== sessionId && prior.run.contractVersion === this.config.contractSha256 && prior.run.executorVersion === this.config.executorVersion) {
            run = checkedRun(prior.run, this.config, ip, prior.run.sessionId);
            assertRunBinding(run, inventory, this.config);
            const expires = Date.parse(run.leaseExpiresAt ?? "");
            const waiting = run.status === "running" && Number.isFinite(expires) && expires > Date.now();
            return {
              schema: 1,
              ok: false,
              operation,
              ...
              revision: run.revision,
              current_stage: run.currentStage,
              reason: waiting ? "ledger-session-lease-active" : "ledger-explicit-handoff-required",
              can_resume: !waiting && this.executionAvailable,
              next_tool: waiting ? "fleet_onboard_status" : "fleet_onboard_resume",
              next_action: waiting ? "Wait for the existing session lease to expire, then explicitly resume this transaction. Do not create another run." : "Call fleet_onboard_resume to claim this existing transaction with fresh inventory and CAS. Do not create another run.",
              ...waiting ? { retry_after: new Date(expires).toISOString() } : {}
            };
          }
        ...
        const observedReason = inventory.components?.[diagnosticComponent]?.reason_code;
        const allowedDiagnostics = diagnosticStage === 7 ? [
          "browser-desktop-packages-missing",
          "browser-binary-unavailable-arm64",
          "browser-binary-unavailable-other",
          "browser-startup-unconfirmed"
        ] : [
          "dashboard-port-in-use",
          "dashboard-permission-denied",
          "dashboard-dependency-missing",
          "dashboard-database-unavailable",
          "dashboard-auth-config-invalid",
          "dashboard-startup-unconfirmed"
        ];
        const showDiagnostic = [6, 7].includes(diagnosticStage) && run.status !== "complete" && new Set(allowedDiagnostics).has(observedReason);
        const result = {
          schema: 1,
      ...
      return {
        ...
        execution_available: this.executionAvailable,
        diagnostic: { boundary, code: error instanceof AdapterProcessError ? error.diagnostic : error instanceof LedgerDiagnosticError ? error.message : "adapter-result-validation-failed" },
        run_created: operation === "start" && created,
...
async function registerFleetOnboardTools(ctx, adapter = new UnavailableFleetAdapter(), readOnly = false) {
  const defineTool = process.env.NODE_ENV === "test" ? ((spec) => spec) : (await import("@deepseek-ai/dsh-tools")).defineTool;
  const register = (tool) => readOnly && !["fleet_onboard_status", "fleet_onboard_report"].includes(tool.name) ? () => void 0 : ctx.tools.register(tool);
  const disposers = [
...
async function apply(ctx, pluginConfig = {}) {
  ...
  try {
    ...
    const scripts = join2(skillRoot, "scripts");
    const credentialCommand = { executable: join2(scripts, "vault-credential-provider.py") };
    ...
    const hostAdapter = { executable: join2(scripts, "host-adapter.py") };
    ...
    adapter = await SubprocessFleetOnboardAdapter.create({
      ...
      ledger,
      probe: hostAdapter,
      runtime: { executable: join2(scripts, "onboard-runtime.py") },
      ...executionEnabled ? { executor: hostAdapter } : {},
      ...cloud ? { cloud } : {},
      contractFile: join2(skillRoot, "component-contract.json"),
      hmacKeyFile,
      ...
      environment: { ...vaultEnvironment, FLEET_ONBOARD_HOST_CONFIG_FILE: hostConfig },
      workerId,
      executorVersion
    });
  } catch {
