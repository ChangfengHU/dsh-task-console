...
${persona}`);
  ...
  for (const [serverName, selected] of Object.entries(spec.mcpTools)) {
    ...
    const config = { ...host.config, serverName: name2, allowedTools, ...Object.keys(toolRules).length ? { toolRules } : {} };
...
- id: tool-skill
  ...
  const deniedInherited = [...new Set(inherited)].filter((name2) => !selectedLocalNames.has(name2));
