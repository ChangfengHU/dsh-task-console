...
// src/service.ts
import { randomUUID as randomUUID7, createHash as createHash9 } from "node:crypto";
...
async function decideTaskSignalWithAgent(ctx, signal, context, options = {}) {
  ...
  const digest2 = createHash5("sha256").update(signal.id).digest("hex").slice(0, 12);
...
import { createHash as createHash6, createHmac, timingSafeEqual } from "node:crypto";
...
function validateDesign(value) {
  ...
  if (d.evidenceContract !== void 0 && d.evidenceContract !== "browser-patrol-v1") throw new Error("\u672A\u77E5 evidenceContract");
...
// src/task-create.ts
var digest = (value) => createHash7("sha256").update(JSON.stringify(value)).digest("hex");
...
var TaskCreator = class {
  ...
  queue = Promise.resolve();
  catalog() {
    return [...this.runner.store.tasks.values()].filter((t) => t.enabled && t.origin?.source === "task-chat").map(({ id, title, brief, participants, graphMode, workflowRecipe, design }) => ({
      id,
      title,
      brief,
      participants,
  ...
  async dispatch(raw, input, exec, cwd, directWorkflow = false, stageOnly = false) {
    ...
    try {
      ...
      if (stageOnly) {
        ...
        if (!oldPlan) {
          const accepted = db2.prepare("SELECT id FROM dsh_task_plans WHERE request_id=? AND state IN ('approved','dispatched')").get(input.requestId);
...
function browserPatrolEvidence(input, events) {
  ...
  for (const e of events) {
    if (e.type === "tool/call") {
      const name2 = /^mcp__fleet-browser(?:-browser-manager)?__(browser_fleet_inventory|browser_inspect|browser_status)$/.exec(e.data.name)?.[1];
      if (!name2) continue;
      try {
    ...
    for (const part of e.data?.message?.content ?? []) {
      ...
      if (call.name === "browser_status" && value.id === call.args.operationId && value.args?.ip === call.args.ip && value.args.sessionId === input.sessionId && ["login-verify", "login-provision", "login-copy"].includes(value.action) && value.phase === "complete") {
        ...
        verifications.set(`${value.args.ip}:${value.args.instance}`, {
...
async function validateWorkflowCompletion(input, deps = {}) {
  ...
  for (const id of ids) {
    ...
    const expected = createHash8("sha256").update(JSON.stringify([sessionId, job?.args?.requestId])).digest("hex").slice(0, 32);
  ...
  async intakeAgents() {
    ...
    for (const preset of await presets.list()) {
      ...
      rows.push({
        ...
        profileHash: createHash9("sha256").update(JSON.stringify(spec)).digest("hex"),
