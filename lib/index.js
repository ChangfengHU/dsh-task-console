...
var TaskCreator = class {
  ...
  queue = Promise.resolve();
  catalog() {
    return [...this.runner.store.tasks.values()].filter((t) => t.enabled && t.origin?.source === "task-chat").map(({ id, title, brief, participants, graphMode, workflowRecipe }) => ({ id, title, brief, participants, graphMode, workflowRecipe }));
  }
  async context() {
    return {
  ...
  async dispatch(raw, input, exec, cwd, directWorkflow = false) {
