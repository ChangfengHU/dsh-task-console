...
async function fleetActionOptions(config, p, values, load) {
...
function publicToolName(serverName, rawName) {
  ...
  const hash2 = createHash3("sha256").update(`${serverName}\0${rawName}`).digest("hex").slice(0, 12);
...
// src/task-create.ts
var digest = (value) => createHash10("sha256").update(JSON.stringify(value)).digest("hex");
...
function collectBrowserEvidence(input, events) {
  ...
  for (const server of ["fleet-browser", `fleet-browser-${input.profileId}`]) for (const raw of ["browser_fleet_inventory", "browser_inspect", "browser_status"]) {
    ...
    names.set(full.length <= 64 ? full : `${full.slice(0, 51)}_${createHash11("sha256").update(`${server}\0${raw}`).digest("hex").slice(0, 12)}`, raw);
  }
...
function proxyRequestId(sessionId, requestId) {
  ...
  return createHash12("sha256").update(sessionId + "\0" + requestId).digest("hex");
}
...
var TaskNotifications = class {
  ...
  async send(input, stage, report, deliver) {
    ...
    for (const chatId of config.chatIds) {
      const id = createHash13("sha256").update(JSON.stringify([input.batch.id, input.card.round, stage, chatId])).digest("hex").slice(0, 24);
      const markdown = [
...
\u4F01\u5FAE\uFF1A${notifications.length ? notifications.map((n) => n.state).join("\u3001") : "\u672C\u4EFB\u52A1\u672A\u914D\u7F6E\u901A\u77E5"}`, metadata: { ...report2.metadata, notifications } };
  ...
  async intakeAgents() {
    ...
    for (const preset of await presets.list()) {
      ...
      rows.push({
        ...
        profileHash: createHash14("sha256").update(JSON.stringify(spec)).digest("hex"),
  ...
  async agentActionOptions(payload) {
    ...
    const preset = (await this.ctx.get("agentPresets").list()).find((p) => p.id === catalog.agentId);
    const spec = preset && await readSpec(dirname5(String(preset.path)));
    ...
    try {
      ...
      const latest = JSON.parse(await this.agentActions(payload));
