...
// src/service.ts
import { randomUUID as randomUUID9, createHash as createHash16 } from "node:crypto";
  ...
  for (const { root, label } of skillRoots(home)) {
    ...
    for (const name2 of names.sort()) {
      ...
      try {
        ...
        const text2 = await readFile2(join3(dir, "SKILL.md"), "utf8");
        const fm = frontmatter(text2);
...
function indent(text2, n) {
  const pad = " ".repeat(n);
  return text2.split("\n").map((l) => l.length ? pad + l : l).join("\n");
}
function mask(text2) {
  return text2.replace(/(Authorization:\s*)(["']?)Bearer\s+\S+/gi, "$1$2Bearer \u2022\u2022\u2022\u2022").replace(/((?:token|secret|password|api[-_]?key)\s*:\s*)(["']?)[^\s"'#]+/gi, "$1$2\u2022\u2022\u2022\u2022").replace(/\/\/[^@\s/]+@/g, "//\u2022\u2022\u2022\u2022@");
}
...
\u6267\u884C\u5339\u914D\u4EFB\u52A1\u524D\u4F7F\u7528 skill \u5DE5\u5177\u52A0\u8F7D\u5B8C\u6574\u8BF4\u660E\uFF1B\u76EE\u5F55\u4E0D\u662F\u6388\u6743\u7ED5\u8FC7\u5176\u4ED6\u5DE5\u5177\u9650\u5236\u3002` }] }];
  ...
  async describe(agent, exposed) {
    ...
    for (const event of session.events ?? []) {
      ...
      const text2 = toolText(d.message);
      const failed = /^error|"ok"\s*:\s*false|Authentication Fails/i.test(text2);
      if (failed) errors.set(call.name, /Authentication Fails|unauthorized|api key.*invalid/i.test(text2) ? "authentication-failed" : /session-required|not.granted|not.authorized|denied/i.test(text2) ? "permission-or-identity-denied" : "last-call-failed");
      else {
...
function validateDesign(value) {
  ...
  const branches = d.branches.map((b) => {
    ...
    return { id: b.id, when: text2(b.when, "when"), action: text2(b.action, "action"), evidence: text2(b.evidence, "evidence") };
  });
  ...
  return {
    ...
    ...proxy ? { proxy } : {},
    scope: text2(d.scope, "scope"),
    branches,
    coordination: text2(d.coordination, "coordination"),
...
var EventStore = class {
  ...
  async importLegacyJsonl() {
    let text2 = "";
    try {
      text2 = await readFile5(this.legacyFile, "utf8");
    } catch {
      return;
    }
...
${JSON.stringify(plan.items)}`;
      ...
      flight.handle.agent.followup({ id: messageId, role: "user", content: [{ type: "text", text: text2 }], source: { kind: "user" } });
...
function messageIpv4s(text2) {
  const values = [];
  for (const match of text2.matchAll(/(?:^|[^\d.])((?:\d{1,3}\.){3}\d{1,3})(?=$|[^\d.])/g)) {
    if (validIpv4(match[1])) values.push(match[1]);
  }
  return [...new Set(values)];
}
function labeledCredential(text2) {
  return {
    username: /(?:^|\s)(?:username|user|账号|用户)(?:\s*(?:[:=：]|is|是)\s*|\s+)([a-z_][a-z0-9_-]{0,31})/i.exec(text2)?.[1],
    password: /(?:^|\s)(?:password|passwd|密码)(?:\s*(?:[:=：]|is|是)\s*|\s+)(\S{1,256})/i.exec(text2)?.[1]
  };
}
function conventionalCredential(text2, ip) {
...
function credentialFromSession(ip, exec) {
  ...
  if (anchorIps.some((value) => value !== ip) || rows.slice(anchor + 1).some((text2) => messageIpv4s(text2).some((value) => value !== ip))) {
    return { available: false, missing: ["ssh_username", "ssh_credential"] };
  }
  let username;
  let password;
  for (let index = anchor; index < rows.length; index += 1) {
    const text2 = rows[index];
    if (!text2) continue;
    const labeled = labeledCredential(text2);
    if (labeled.username) {
    ...
    if (index === anchor) {
      const conventional = conventionalCredential(text2, ip);
...
var TaskActions = class {
  ...
  resolve(input) {
    ...
    const redact = (text2) => password ? text2.split(password).join("[credential supplied privately]") : text2;
...
function userInput(exec) {
  ...
  const text2 = typeof last?.content === "string" ? last.content : (last?.content ?? []).filter((p) => p.type === "text").map((p) => p.text).join("\n");
  const sessionId = String(exec.agent?.session?.id ?? exec.agent?.session?.header?.id ?? "");
  if (!sessionId || !text2?.trim()) throw new Error("\u9700\u8981\u771F\u5B9E\u7528\u6237\u6D88\u606F\uFF0C\u4E0D\u80FD\u7528\u6A21\u578B\u7F16\u9020\u7684\u8F93\u5165\u521B\u5EFA\u4EFB\u52A1");
  return { text: text2, sessionId, requestId: digest2([sessionId, last.id ?? users.length, text2]) };
}
...
${p.input.text}`,
  ...
  async launch(taskId, text2, requestId, cwd) {
    if (typeof text2 !== "string" || text2.trim().length < 2 || text2.length > 32e3) throw new Error("\u8BF7\u8F93\u5165\u672C\u6B21\u4EFB\u52A1\u53C2\u6570\uFF08\u6700\u591A 32000 \u5B57\u7B26\uFF09");
    if (!/^[a-zA-Z0-9-]{16,80}$/.test(requestId)) throw new Error("\u9700\u8981\u7A33\u5B9A\u7684\u63D0\u4EA4 ID");
    const exec = { agent: { session: { id: `workflow-${requestId}`, deriveMessages: () => [{ id: requestId, role: "user", content: [{ type: "text", text: text2 }] }] } } };
    ...
    try {
      ...
      if (proposal.decision === "reuse" || proposal.decision === "revise") {
        ...
        if (proposal.decision === "revise") {
          ...
          previous = found;
          const reusable = (text2) => ips.reduce((s, ip) => s.split(ip).join("{{target}}"), text2);
          task = {
            ...found,
            ...validateTask({
              ...found,
...
\u4F01\u5FAE\uFF1A${notifications.length ? notifications.map((n) => n.state).join("\u3001") : "\u672C\u4EFB\u52A1\u672A\u914D\u7F6E\u901A\u77E5"}`, metadata: { ...report2.metadata, notifications } };
  ...
  async intakeAgents() {
    ...
    for (const preset of await presets.list()) {
      ...
      rows.push({
        ...
        profileHash: createHash16("sha256").update(JSON.stringify(spec)).digest("hex"),
  ...
  async agents() {
    ...
    for (const p of await presets.list()) {
      ...
      if (!p.name || !p.description) {
        try {
          const text2 = await (await import("node:fs/promises")).readFile(`${dir}/preset.yml`, "utf8");
          const n = /^name:\s*(.*)$/m.exec(text2)?.[1];
          const d = /^description:\s*(.*)$/m.exec(text2)?.[1];
  ...
  async startAgentSession(payload) {
    const { agentId, text: text2, cwd } = JSON.parse(payload);
    const presets = this.ctx.get("agentPresets");
    if (!presets) throw new Error("\u8FD9\u4E2A\u90E8\u7F72\u6CA1\u6709 preset \u670D\u52A1");
    ...
    if (text2 && text2.trim()) handle.agent.followup({ id: randomUUID9(), role: "user", content: [{ type: "text", text: text2.trim() }], source: { kind: "user" } });
  ...
  async launchWorkflow(payload) {
    await this.ready;
    const { taskId, text: text2, requestId, cwd } = JSON.parse(payload);
    return JSON.stringify(await this.creator.launch(taskId, text2, requestId, cwd));
  }
