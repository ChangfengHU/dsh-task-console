...
function composeRecipe(recipe) {
  ...
  return {
    ...
    brief: "\u5BF9\u672C\u6B21\u8F93\u5165\u6307\u5B9A\u7684 Fleet \u57FA\u7840\u8282\u70B9\u68C0\u67E5\u3001\u590D\u7528\u5065\u5EB7\u7EC4\u4EF6\u5E76\u5728\u6388\u6743\u8303\u56F4\u4FEE\u590D\u6F02\u79FB\uFF1B\u4FDD\u7559\u65E2\u6709\u73AF\u5883\u3001\u6D4F\u89C8\u5668\u8D44\u6599\u548C\u6709\u6548\u767B\u5F55\uFF0C\u4E0D\u5378\u8F7D\u91CD\u88C5\u6216\u8F6E\u6362\u51ED\u636E\u3002" + (provision ? "\u8D26\u53F7\u914D\u7F6E\u5C5E\u4E8E\u672C\u6B21\u72EC\u7ACB\u4E1A\u52A1\u9A8C\u6536\uFF1A\u4E3A\u6307\u5B9A\u6D4F\u89C8\u5668\u9009\u62E9\u5DF2\u6388\u6743 Gemini \u6765\u6E90\u5E76\u8865\u9F50\u767B\u5F55\uFF0C\u4EE5\u5B9E\u9645\u9A8C\u8BC1\u4E3A\u51C6\u3002" : "\u4FDD\u6301\u8D26\u53F7\u767B\u5F55\u73B0\u72B6\uFF1B\u7F3A\u5C11\u7AD9\u70B9\u767B\u5F55\u4E0D\u5C5E\u4E8E\u57FA\u7840\u8282\u70B9\u6545\u969C\u3002") + "\u4EC5\u7531\u57FA\u7840\u88C5\u673A\u8005\u3001\u6D4F\u89C8\u5668\u7BA1\u7406\u5458\u3001Runner \u8FD0\u7EF4\u8005\u6309\u987A\u5E8F\u534F\u4F5C\uFF0C\u672B\u4F4D\u89D2\u8272\u6839\u636E\u539F\u59CB\u4EA4\u63A5\u548C\u672C\u8F6E\u7B7E\u540D\u8BC1\u636E\u6C47\u603B\u5B8C\u6574\u62A5\u544A\u3002",
...
var TaskCreator = class {
  ...
  queue = Promise.resolve();
  catalog() {
    return [...this.runner.store.tasks.values()].filter((t) => t.enabled && t.origin?.source === "task-chat").map(({ id, title, brief, participants, graphMode }) => ({ id, title, brief, participants, graphMode }));
  }
  async context() {
    return {
...
var TaskConsoleService = class extends TypertRemoteService {
  ...
  async tryRun(payload) {
    ...
    try {
      handle = await this.ctx.agents.create({
        sessionId,
        ...selection ? { agentOptions: selection } : {},
        meta: { cwd: homedir6(), agentPreset: preset.id },
        setup: async (agentCtx) => {
          await presets.mount(agentCtx, preset.id);
        }
      });
  ...
  async startAgentSession(payload) {
    ...
    const dir = cwd && cwd.trim() ? cwd.trim() : workspaces[0]?.path ?? homedir6();
