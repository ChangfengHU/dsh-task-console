...
function publicToolName(serverName, rawName) {
  ...
  const hash = createHash2("sha256").update(`${serverName}\0${rawName}`).digest("hex").slice(0, 12);
...
var TaskConsoleService = class extends TypertRemoteService {
  ...
  async startAgentSession(payload) {
    ...
    if (text && text.trim()) handle.agent.followup({ id: randomUUID6(), role: "user", content: [{ type: "text", text: text.trim() }], source: { kind: "user" } });
