...
import { basename as basename2, extname, isAbsolute as isAbsolute2, join as join2, relative as relative2, resolve as resolve3, sep as sep2 } from "node:path";
...
async function readPublishableHtml(filePath) {
  ...
  return { path: file, name: basename(file), data: await readFile2(file) };
}
...
async function captureArtifacts(ctx, requested) {
  ...
  const dir = join2(ctx.root, "artifacts", safePart(ctx.task.id), safePart(ctx.batchId), safePart(ctx.runId));
  await mkdir(dir, { recursive: true, mode: 448 });
  const out = [];
  for (const file of checked) {
    const id = `a-${randomUUID2()}`;
    const storagePath = join2(dir, `${id.slice(2, 10)}-${file.name}`);
...
async function readArtifact(root, task, artifact) {
  ...
  const allowed = artifact.legacy ? await realpath2(task.cwd) : await realpath2(join2(root, "artifacts"));
...
import { basename as basename3, join as join3, resolve as resolve4 } from "node:path";
  ...
  for (const { root, label } of skillRoots(home)) {
    ...
    for (const name2 of names.sort()) {
      ...
      try {
        ...
        const text2 = await readFile4(join3(dir, "SKILL.md"), "utf8");
...
async function verifyPresetSkills(spec, library, dir) {
  const lock = await readFile4(join3(dir, SKILL_LOCK_FILE), "utf8").then(parseSkillLock).catch(() => null);
  ...
  for (const name2 of spec.skills) {
    ...
    const copied = join3(dir, "skills", source ? basename3(source.dir) : name2);
...
async function syncPresetSkills(spec, library, dir) {
  ...
  const staged = join3(dir, `.skills-next-${randomUUID4()}`);
  const stagedLock = join3(dir, `.skills-lock-next-${randomUUID4()}`);
  const backup = join3(dir, `.skills-backup-${randomUUID4()}`);
  const backupLock = join3(dir, `.skills-lock-backup-${randomUUID4()}`);
  ...
  try {
    for (const { name: name2, entry } of sources) {
      const target = join3(staged, basename3(entry.dir));
...
`, { mode: 384 });
    ...
    if (existed) {
      try {
        await writeFile2(join3(staged, ACTION_FILE), await readFile4(join3(dir, ACTION_FILE)), { mode: 384 });
      } catch (error) {
...
async function readSpec(dir) {
  try {
    return validateSpec(JSON.parse(await readFile4(join3(dir, SPEC_FILE), "utf8")));
  } catch {
    return null;
  }
}
async function readAgentCreatedAt(dir) {
  try {
    const { createdAt } = JSON.parse(await readFile4(join3(dir, "agent-meta.json"), "utf8"));
...
function storeDir(home = homedir3()) {
  return join4(process.env.DSH_HOME ?? join4(home, ".dsh"), "task-console");
}
var EventStore = class {
  ...
  get file() {
    return join4(this.dir, "task.db");
  }
  get legacyFile() {
    return join4(this.dir, "events.jsonl");
  }
  get root() {
    return this.dir;
  }
...
var TaskIntakeCoordinator = class {
  ...
  workspace() {
    return this.options.workspace ?? process.env.DSH_TASK_INTAKE_WORKSPACE ?? join5(homedir4(), ".dsh", "task-console", "intake-workspace");
  }
  async start() {
...
${p.input.text}`,
  ...
  async dispatch(raw, input, exec, cwd, directWorkflow = false, stageOnly = false) {
    ...
    try {
      ...
      if (leases.length) {
        const root = join8(store.root, "private-inputs");
        await mkdir5(root, { recursive: true, mode: 448 });
        await writeFile4(join8(root, `${batchId}.json`), JSON.stringify({
...
${JSON.stringify(current)}`, metadata: { proxy: status } };
  ...
  pending(input) {
    ...
    let local;
    try {
      local = new Database3(join9(process.env.DSH_PROXY_STATE_DIR ?? join9(homedir6(), ".local/state/dsh-proxy"), "proxy.db"), { readonly: true, fileMustExist: true });
      for (const row of rows) {
...
async function browserJobs(deps) {
  return (deps.jobs || (async () => {
    const root = process.env.FLEET_BROWSER_STATE_DIR || join10(homedir7(), ".local/state/fleet-browser-manager");
    let names;
    try {
...
async function readBrowserAcceptance(id) {
  ...
  let file;
  try {
    file = await open2(join10(root, id + ".json"), constants.O_RDONLY | constants.O_NOFOLLOW);
...
\u4F01\u5FAE\uFF1A${notifications.length ? notifications.map((n) => n.state).join("\u3001") : "\u672C\u4EFB\u52A1\u672A\u914D\u7F6E\u901A\u77E5"}`, metadata: { ...report2.metadata, notifications } };
  ...
  async agentActions(payload) {
    ...
    const dir = dirname4(String(preset.path));
