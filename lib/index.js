...
var BrowserPatrolWorkflow = class {
  ...
  plan(input, candidate) {
    ...
    if (input.card.round > 1 && items.every((row) => row.action === "verify") && input.task.design.browserPatrol.actions.includes("provision")) {
      const repairable = this.status(input).items.filter((row) => !row.accepted && row.readAuthorized && inventory.nodes.some((n) => n.ip === row.ip && n.reachable) && row.state === "signed_out" && row.loginAuthorized && row.attempts < input.task.design.failurePolicy.maxAttempts);
  ...
  status(input, now = Date.now()) {
    ...
    const canHandoffForRework = (input.card.round ?? 0) < input.task.design.failurePolicy.maxAttempts && items.some((i) => !i.accepted && i.readAuthorized && inventory.nodes.some((n) => n.ip === i.ip && n.reachable) && i.loginAuthorized && i.attempts < input.task.design.failurePolicy.maxAttempts && (i.checkedAt && i.state !== "verified" || i.verificationFailure));
