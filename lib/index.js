...
var TaskConsoleService = class extends TypertRemoteService {
  ...
  constructor(ctx) {
    ...
    void this.runner.start().then(async () => {
  ...
  async markExistingTaskSessionsInternal() {
    const sessionIds = [...new Set([...this.runner.store.s.runs.values()].map((run) => run.sessionId).filter((id) => Boolean(id)))];
  ...
  async archiveBatchSessions(batchId) {
    const registry = this.ctx.get("workspaceRegistry");
    if (!registry?.archiveSession) return;
    const sessionIds = [...new Set([...this.runner.store.s.runs.values()].filter((run) => run.batchId === batchId && run.sessionId).map((run) => run.sessionId))];
    for (const sessionId of sessionIds) {
      try {
        await registry.archiveSession(sessionId);
      } catch (error) {
        console.warn(`[task-console] could not archive task session ${sessionId}:`, error);
      }
    }
  }
  /** Reconcile batches completed by older plugin versions on startup. */
  async archiveSettledTaskSessions() {
  ...
  async taskSessions() {
    ...
    const archived = new Set((this.ctx.get("workspaceRegistry")?.archivedSessionIds ?? []).map(String));
    const internal = new Set((this.ctx.get("workspaceRegistry")?.internalSessionIds ?? []).map(String));
    const rows = [...state.runs.values()].filter((run) => run.sessionId).map((run) => {
