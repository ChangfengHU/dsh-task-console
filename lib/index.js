...
// src/service.ts
import { randomUUID as randomUUID10, createHash as createHash17 } from "node:crypto";
...
async function uploadConfig(envelope, config) {
  if (!config.token) throw Error("\u5BBF\u4E3B\u672A\u914D\u7F6E\u914D\u7F6E\u5BFC\u51FA\u6240\u9700\u7684 R2 \u51ED\u636E");
  const data = encodeEnvelope(envelope), name2 = `dsh-config-${Date.now()}-${randomUUID9().slice(0, 8)}.json`;
  const form = new FormData();
  ...
  const check = await fetch(publicUrl, { redirect: "error", signal: AbortSignal.timeout(3e4) });
...
async function createBootstrapCommand(value, config) {
  ...
  if (!response.ok || !body2?.ok || typeof body2.command !== "string") throw Error(body2?.error || `\u65B0\u673A\u5668\u5F15\u5BFC\u547D\u4EE4\u751F\u6210\u5931\u8D25\uFF08HTTP ${response.status}\uFF09`);
  ...
  return { command: body2.command, expiresInSeconds: Number(body2.expiresInSeconds) || 0 };
}
  ...
  async intakeAgents() {
    ...
    for (const preset of await presets.list()) {
      ...
      rows.push({
        ...
        profileHash: createHash17("sha256").update(JSON.stringify(spec)).digest("hex"),
...
async function apply(ctx, config = {}) {
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        const body2 = await readFile8(new URL("./client-heavy.js", import.meta.url));
