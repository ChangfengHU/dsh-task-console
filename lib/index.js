...
import { basename as basename5, dirname as dirname5, resolve as resolve7 } from "node:path";
...
async function registerWorkerTools(agentCtx, hooks, options = {}) {
  ...
  if (hooks.notify) disposers.push(agentCtx.tools.register(defineTool({
    ...
    parameters: { stage: { type: "string", required: true, enum: ["started", "findings", "rework", "restored", "unresolved", "blocked"] } },
...
async function writePreset(spec, hostMcp, library, root = userPresetRoot(), inheritedTools = []) {
  return withPresetLock(resolve2(root, spec.id), () => writePresetLocked(spec, hostMcp, library, root, inheritedTools));
}
async function writePresetLocked(spec, hostMcp, library, root, inheritedTools) {
...
\u6267\u884C\u5339\u914D\u4EFB\u52A1\u524D\u4F7F\u7528 skill \u5DE5\u5177\u52A0\u8F7D\u5B8C\u6574\u8BF4\u660E\uFF1B\u76EE\u5F55\u4E0D\u662F\u6388\u6743\u7ED5\u8FC7\u5176\u4ED6\u5DE5\u5177\u9650\u5236\u3002` }] }];
  ...
  async describe(agent, exposed) {
    ...
    const tools = schemas.map((s) => ({ name: s.name, kind: s.name.startsWith("mcp__") ? "mcp" : "native", server: this.mcpSources().find((m) => m.tools.some((name2) => publicToolName(m.serverName, name2) === s.name))?.serverName ?? null, source: CAPABILITY_TOOLS.includes(s.name) ? "platform" : declared.has(s.name) ? "agent-definition" : "environment-inherited", state: availableNames.has(s.name) ? "registered" : "restricted", ...this.restricted(s, agent) ? { reason: "explicit-standard-exclusion" } : {}, ...errors.has(s.name) ? { lastFailure: errors.get(s.name) } : {} }));
    const out = {
      ...
      current: { tools, skills, skillDiscovery, permissionPreset: this.ctx.get("permissionPresets")?.current(session.events ?? []) ?? "not-observed", configuredButNotRegistered: [...declared].filter((n) => !schemas.some((s) => s.name === n)) },
...
var labels = { started: "\u5F00\u59CB\u5DE1\u67E5", findings: "\u5DE1\u67E5\u53D1\u73B0", rework: "\u7EE7\u7EED\u8FD4\u5DE5", restored: "\u72EC\u7ACB\u9A8C\u6536\u901A\u8FC7", unresolved: "\u4ECD\u6709\u672A\u89E3\u51B3\u9879", blocked: "\u5DE1\u67E5\u5904\u7406\u53D7\u963B" };
...
function createEnvelope(payload, version, now = /* @__PURE__ */ new Date(), runtime = FLEET_RUNTIME) {
  const normalized = {
...
\u4F01\u5FAE\uFF1A${notifications.length ? notifications.map((n) => n.state).join("\u3001") : "\u672C\u4EFB\u52A1\u672A\u914D\u7F6E\u901A\u77E5"}`, metadata: { ...report2.metadata, notifications } };
  ...
  async exportConfig() {
    ...
    envelope.runtime = { ...envelope.runtime, bootstrap: { issuer: "https://fleet.vyibc.com/api/hub/dsh-config-bootstrap", token: bootstrap.bootstrapToken, expiresAt: new Date(Date.now() + bootstrap.expiresInSeconds * 1e3).toISOString() } };
    const result = await uploadConfig(envelope, this.configR2(), basename5(new URL(initial.publicUrl).pathname));
  ...
  configImportView(envelope) {
    ...
    return Promise.all([presets ? presets.list() : [], scanSkills()]).then(([rows, skillRows]) => {
      ...
      const available = /* @__PURE__ */ new Set([...existingAgents, ...agents.filter((row) => !row.conflict && row.ready).map((row) => row.id)]);
  ...
  async applyConfigImport(payload) {
    ...
    for (const row of pending2.envelope.payload.agents) {
      ...
      if (state.conflict) {
        skippedAgents.push({ id: row.spec.id, reason: "\u540C ID Agent \u5DF2\u5B58\u5728" });
        continue;
      }
      if (!state.ready) {
        skippedAgents.push({ id: row.spec.id, reason: `\u7F3A\u5C11 ${[...state.missingSkills, ...state.missingMcp].join("\u3001")}` });
        continue;
      }
    ...
    const availableAgents = /* @__PURE__ */ new Set([...view.agents.filter((row) => row.conflict).map((row) => row.id), ...importedAgents]);
