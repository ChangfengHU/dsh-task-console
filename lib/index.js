...
var ProxyWorkflow = class {
  ...
  status(input) {
    ...
    const ips = db.prepare("SELECT DISTINCT ip FROM dsh_proxy_round_items WHERE batch_id=?").all(input.batch.id).map((r) => r.ip);
...
${JSON.stringify(status.items)}`, metadata: { proxy: status } };
  ...
  async invoke(input, raw, args, invoke) {
    ...
    if (raw === "proxy_inspect") {
      ...
      if (!inventory2 || !JSON.parse(inventory2.inventory_json).nodes.some((n) => n.ip === args.ip && n.readAuthorized)) throw Error("proxy-target-not-in-inventory");
      return invoke(args);
    }
    if (raw === "proxy_status") {
      const row2 = db.prepare("SELECT * FROM dsh_proxy_calls WHERE operation_id=? AND session_id=?").get(args.operationId, input.sessionId);
    ...
    if (!inventory || !JSON.parse(inventory.inventory_json).nodes.some((n) => n.ip === args.ip && n.readAuthorized)) throw Error("proxy-target-not-in-inventory");
