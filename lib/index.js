...
// src/service.ts
import { randomUUID as randomUUID8, createHash as createHash12 } from "node:crypto";
...
async function registerWorkerTools(agentCtx, hooks, options = {}) {
  ...
  if (hooks.notify) disposers.push(agentCtx.tools.register(defineTool({
    name: "task_notify",
    description: "\u89C4\u5212\u8005\u901A\u8FC7\u73B0\u6709\u4F01\u5FAE MCP \u6C47\u62A5\u771F\u5B9E\u5DE1\u67E5\u8FDB\u5EA6\uFF1B\u6536\u4EF6\u7FA4\u3001\u5185\u5BB9\u548C\u53BB\u91CD\u7F16\u53F7\u7531\u5DF2\u5BA1\u67E5\u5951\u7EA6\u4E0E\u6570\u636E\u5E93\u8BC1\u636E\u751F\u6210\u3002\u91CD\u590D\u8C03\u7528\u540C\u9636\u6BB5\u53EA\u91CD\u8BD5\u786E\u5B9A\u672A\u53D1\u9001\u7684\u901A\u77E5\uFF0C\u7EDD\u4E0D\u91CD\u590D\u6D4F\u89C8\u5668\u4FEE\u590D\uFF1Bunknown \u4E0D\u76F2\u76EE\u91CD\u53D1\u3002",
  ...
  if (hooks.patrolStatus) disposers.push(agentCtx.tools.register(defineTool({
    ...
    description: "\u8BFB\u53D6\u5BBF\u4E3B\u4ECE\u771F\u5B9E\u5DE5\u5177\u56DE\u6267\u63D0\u53D6\u7684\u9010\u6D4F\u89C8\u5668\u8BC1\u636E\u3001\u5F53\u524D\u8F6E\u6B21\u7CBE\u786E\u6388\u6743\u3001\u7D2F\u8BA1\u4FEE\u590D\u6B21\u6570\u3001\u72EC\u7ACB\u91C7\u6837\u548C\u4E0B\u6B21\u68C0\u67E5\u65F6\u95F4\u3002\u4E0D\u662F\u91CD\u65B0\u6267\u884C\u63A2\u9488\uFF0C\u4E0D\u63A5\u53D7\u6A21\u578B\u4F2A\u9020\u72B6\u6001\u3002",
    parameters: {},
    output: { schema: { type: "object", additionalProperties: true }, render },
  ...
  if (hooks.wait) disposers.push(agentCtx.tools.register(defineTool({
    name: "task_wait",
    description: "\u6301\u4E45\u5316\u7B49\u5F85\u5230\u6307\u5B9A\u65F6\u95F4\u518D\u7EE7\u7EED\u5F53\u524D\u5361\u3002\u7ED3\u675F\u672C\u6B21 worker\uFF0C\u5C4A\u65F6\u540C\u4E00 Task/Batch/\u5361\u4F1A\u521B\u5EFA\u65B0 Run\uFF1B\u4E0D\u662F\u5931\u8D25\uFF0C\u4E0D\u8017\u8FD4\u5DE5\u8F6E\u6B21\u3002\u5148\u8BB0\u5F55\u5DF2\u6709\u8BC1\u636E\u4E0E\u4E0B\u6B21\u68C0\u67E5\u76EE\u6807\uFF0C\u4E0D\u80FD\u7528\u7B49\u5F85\u4F2A\u9020\u7A33\u5B9A\u6027\u3002\u5B58\u5728\u672A\u5B8C\u6210\u540E\u53F0\u64CD\u4F5C\u65F6\u5E94\u7EE7\u7EED\u67E5\u8BE2\u539F\u56DE\u6267\uFF0C\u4E0D\u8C03\u7528\u672C\u5DE5\u5177\u3002",
    ...
    try {
      ...
      try {
        ...
        flight.disposeTools = await registerWorkerTools(flight.handle.agent.ctx, {
          ...task.design?.notifications && card.role === "planner" && this.notify ? { notify: (stage, exec) => this.notify({ task, batch, card, sessionId, profileId }, stage, async (args) => {
            ...
            const result = await runtime.execute({ name: tool.name, arguments: args, agent: flight.handle.agent, callId: `notify-${randomUUID6()}`, signal: exec.signal, parent: exec });
...
${JSON.stringify(plan.items)}`;
  ...
  async onTurnEnd(f, reason) {
    ...
    if ((run?.nudges ?? 0) < 1) {
      ...
      f.handle.agent.followup({ id: randomUUID6(), role: "user", content: [{ type: "text", text: NUDGE }], source: { kind: "user" } });
      return;
    }
...
function validateDesign(value) {
  ...
  if (d.evidenceContract === "browser-patrol-v2") {
    ...
    if (!p || p.scope !== "fleet-existing-authorized" || !Array.isArray(p.actions) || p.actions.some((a) => !["provision", "resume"].includes(a)) || !Number.isInteger(p.observationMinutes) || p.observationMinutes < 20 || p.observationMinutes > 60 || !Number.isInteger(p.minSamples) || p.minSamples < 4 || p.minSamples > 12)
...
var TaskCreator = class {
  ...
  queue = Promise.resolve();
  catalog() {
    return [...this.runner.store.tasks.values()].filter((t) => t.enabled && t.origin?.source === "task-chat").map(({ id, title, brief, participants, graphMode, workflowRecipe, design, trigger }) => ({
      id,
      title,
      brief,
      participants,
  ...
  async review(id, hash2, decision, reason) {
    const pending = this.queue.then(async () => {
      ...
      const selected = p.task.participants.map((a) => roster.find((r) => r.id === a.agentId) ?? null);
      ...
      if (!store.tasks.has(p.task.id)) await store.append({ t: "task/created", at: (/* @__PURE__ */ new Date()).toISOString(), taskId: p.task.id, task: { ...p.task, origin: { ...p.task.origin, reviewPlanId: id } } });
  ...
  async dispatch(raw, input, exec, cwd, directWorkflow = false, stageOnly = false) {
    ...
    try {
      ...
      if (proposal.design) {
        ...
        const reusableDesign = proposal.decision === "create" ? JSON.parse(ips.reduce((s, ip) => s.split(ip).join("{{target}}"), JSON.stringify(proposal.design))) : proposal.design;
      ...
      if (stageOnly) {
        ...
        if (!oldPlan) {
          const accepted = db2.prepare("SELECT id FROM dsh_task_plans WHERE request_id=? AND state IN ('approved','dispatched','scheduled')").get(input.requestId);
...
${scrub(input.text)}`,
  ...
  status(taskId, batchId) {
    ...
    return {
      taskId,
      batchId,
      outcome: batch.settled?.outcome ?? "running",
...
var BrowserPatrolWorkflow = class {
  ...
  plan(input, candidate) {
    ...
    for (const row of candidate) {
      ...
      if (keys.has(key) || !browser || !node.readAuthorized || !["verify", "provision", "resume"].includes(row.action) || typeof row.reason !== "string" || !row.reason.trim() || row.reason.length > 1e3) throw new Error("\u8F6E\u6B21\u76EE\u6807/\u52A8\u4F5C\u4E0D\u5728\u672C\u6B21\u771F\u5B9E\u53EF\u8BFB\u6E05\u5355\uFF0C\u6216\u7F3A\u5C11\u51B3\u7B56\u7406\u7531");
      ...
      if (row.action !== "verify") {
        const latest = db.prepare("SELECT state FROM dsh_patrol_observations WHERE batch_id=? AND target_key=? ORDER BY checked_at DESC LIMIT 1").get(input.batch.id, key);
  ...
  status(input, now = Date.now()) {
    ...
    for (const node of inventory.nodes) for (const browser of node.browsers) {
      ...
      const proof = db.prepare("SELECT * FROM dsh_patrol_observations WHERE batch_id=? AND target_key=? ORDER BY checked_at DESC LIMIT 1").get(input.batch.id, key);
      const samples = db.prepare("SELECT * FROM dsh_patrol_observations WHERE batch_id=? AND target_key=? AND role='reviewer' ORDER BY checked_at").all(input.batch.id, key);
      ...
      const lastChange = issue ? db.prepare("SELECT created_at FROM dsh_browser_operations WHERE issue_id=? ORDER BY created_at DESC LIMIT 1").get(issue.id) : null;
      ...
      const fresh = last && Date.parse(last.checked_at) <= now && Date.parse(last.expires_at) > now && now - Date.parse(last.checked_at) <= 15 * 6e4 && last.state === "verified";
      const stable = !issue || relevant.length >= cfg.minSamples && Date.parse(last?.checked_at) - Date.parse(first?.checked_at) >= cfg.observationMinutes * 6e4 && relevant.every((s) => s.state === "verified" && s.fingerprint === last.fingerprint);
      const accepted = node.readAuthorized === true && node.reachable === true && fresh && stable;
      const nextCheckAt = issue && first && !stable && relevant.every((s) => s.state === "verified") ? new Date(Math.max(now + 6e4, Date.parse(first.checked_at) + cfg.observationMinutes * 6e4 * Math.min(relevant.length, cfg.minSamples - 1) / (cfg.minSamples - 1))).toISOString() : null;
      items.push({
        ip: node.ip,
        instance: browser.instance,
    ...
    const uncovered = inventory.nodes.filter((n) => n.reachable !== true && !n.browsers.length).map((n) => ({ nodeId: n.nodeId, reason: "unreachable-no-browser-observation" }));
    ...
    return {
      ready: items.length > 0 && items.every((i) => i.accepted) && uncovered.length === 0,
      canCloseUnresolved,
      plan,
      items,
      uncovered,
      summary: `${items.length} \u4E2A\u5DF2\u89C2\u6D4B\u6D4F\u89C8\u5668\uFF1A${items.filter((i) => i.accepted).length} \u9A8C\u6536\u901A\u8FC7\uFF0C${items.filter((i) => !i.accepted).length} \u672A\u901A\u8FC7\uFF1B${uncovered.length} \u4E2A\u8282\u70B9\u65E0\u6CD5\u786E\u8BA4\u6D4F\u89C8\u5668\u8986\u76D6\u3002`
    };
  }
  complete(input) {
...
var TaskNotifications = class {
  ...
  async send(input, stage, report, deliver) {
    ...
    for (const chatId of config.chatIds) {
      const id = createHash10("sha256").update(JSON.stringify([input.batch.id, input.card.round, stage, chatId])).digest("hex").slice(0, 24);
      const markdown = [
        `## \u6D4F\u89C8\u5668\u5DE1\u67E5 \xB7 ${labels[stage]}`,
        `Task: ${input.task.id}`,
        `\u6267\u884C: ${input.batch.id} \xB7 \u7B2C ${input.card.round} \u8F6E`,
        report.summary || report.reason,
        ...(report.items ?? []).map((r) => `- ${r.ip}/browser-${r.instance}: ${r.accepted ? "\u5DF2\u9A8C\u6536" : r.state}\uFF1B\u4FEE\u590D\u5C1D\u8BD5 ${r.attempts}\uFF1B${r.reason}`),
      ...
      if (claim) {
        ...
        try {
          ...
          } else if (/^(no subscribers|指定的 chatid 不在已订阅列表里|no alerter designated|alerter node [\w-]+ unavailable)$/.test(String(failure?.error))) {
            state = "failed";
            reason = "\u53D1\u9001\u524D\u88AB\u670D\u52A1\u62D2\u7EDD\uFF1B\u914D\u7F6E\u6062\u590D\u540E\u6700\u591A\u91CD\u8BD53\u6B21\uFF0C\u4EC5\u91CD\u8BD5\u901A\u77E5";
          }
        } catch {
        }
...
async function browserJobs(deps) {
  return (deps.jobs || (async () => {
    const root = process.env.FLEET_BROWSER_STATE_DIR || join8(homedir6(), ".local/state/fleet-browser-manager");
    let names;
    try {
...
async function readBrowserAcceptance(id) {
  ...
  let file;
  try {
    file = await open2(join8(root, id + ".json"), constants.O_RDONLY | constants.O_NOFOLLOW);
...
async function validateWorkflowCompletion(input, deps = {}) {
  ...
  for (const id of ids) {
    ...
    const expected = createHash11("sha256").update(JSON.stringify([sessionId, job?.args?.requestId])).digest("hex").slice(0, 32);
...
\u4F01\u5FAE\uFF1A${notifications.length ? notifications.map((n) => n.state).join("\u3001") : "\u672C\u4EFB\u52A1\u672A\u914D\u7F6E\u901A\u77E5"}`, metadata: { ...report2.metadata, notifications } };
      ...
      notify: async (input, stage, deliver) => new TaskNotifications(this.runner.store).send(input, stage, (await this.patrolWorkflow(input)).snapshot(input), deliver)
    });
  ...
  async intakeAgents() {
    ...
    for (const preset of await presets.list()) {
      ...
      rows.push({
        ...
        profileHash: createHash12("sha256").update(JSON.stringify(spec)).digest("hex"),
  ...
  async tryRun(payload) {
    ...
    try {
      handle = await this.ctx.agents.create({
        sessionId,
        ...selection ? { agentOptions: selection } : {},
        meta: { cwd: homedir7(), agentPreset: preset.id },
        setup: async (agentCtx) => {
          await presets.mount(agentCtx, preset.id);
        }
      });
  ...
  async startAgentSession(payload) {
    ...
    const dir = cwd && cwd.trim() ? cwd.trim() : workspaces[0]?.path ?? homedir7();
