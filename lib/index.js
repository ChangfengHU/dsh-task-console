...
// src/service.ts
import { randomUUID as randomUUID10, createHash as createHash18 } from "node:crypto";
import { homedir as homedir9 } from "node:os";
import { dirname as dirname5, resolve as resolve7 } from "node:path";

// src/agent-action-store.ts
...
function validateActions(raw) {
  ...
  return raw.map((a) => {
    ...
    const parameters = a.parameters.map((p) => {
      ...
      const field2 = { key: p.key, label: text(p.label, "\u53C2\u6570\u540D\u79F0", 80), type: p.type, required: p.required };
      if (p.binding !== void 0) {
        if (p.type !== "text" || !["target-ip", "ssh-user", "ssh-password", "gemini-account"].includes(p.binding)) throw Error("\u53C2\u6570\u7528\u9014\u65E0\u6548");
        if (p.binding === "ssh-password" && (p.default !== void 0 || p.source || p.choices)) throw Error("\u5BC6\u7801\u53EA\u80FD\u672C\u6B21\u8F93\u5165\uFF0C\u4E0D\u5141\u8BB8\u4FDD\u5B58\u9ED8\u8BA4\u503C\u6216\u5019\u9009");
        field2.binding = p.binding;
      }
      for (const flag of ["acceptDefaultOnEnter", "integer"]) if (p[flag] !== void 0) {
      ...
      if (p.choices !== void 0) {
        ...
        field2.choices = p.choices.map((v) => text(v, "\u9009\u9879", 120));
      }
      if (p.source !== void 0) {
        if (p.type !== "text" || !["fleet.nodes", "fleet.gemini-accounts"].includes(p.source) || p.choices) throw new Error("\u5019\u9009\u6765\u6E90\u65E0\u6548");
        field2.source = p.source;
      }
      if (p.dependsOn !== void 0) {
        if (!Array.isArray(p.dependsOn) || p.dependsOn.length > 12 || new Set(p.dependsOn).size !== p.dependsOn.length || p.dependsOn.some((k) => !keys.has(k) || k === p.key)) throw new Error("\u4F9D\u8D56\u5FC5\u987B\u662F\u4E4B\u524D\u7684\u53C2\u6570\uFF0C\u4E0D\u5141\u8BB8\u5FAA\u73AF");
        field2.dependsOn = [...p.dependsOn];
      }
      if (p.visibleWhen !== void 0) {
        ...
        if (typeof p.visibleWhen.equals === "number" && !Number.isFinite(p.visibleWhen.equals) || typeof p.visibleWhen.equals === "string" && p.visibleWhen.equals.length > 4e3) throw new Error("\u663E\u793A\u6761\u4EF6\u503C\u65E0\u6548");
        field2.visibleWhen = { key: p.visibleWhen.key, equals: p.visibleWhen.equals };
        field2.inactiveValue = text(p.inactiveValue ?? "\u65E0\u9700\u6307\u5B9A", "\u672A\u542F\u7528\u65F6\u6587\u672C", 120, true);
      }
      if (p.default !== void 0) {
        validateValue(field2, p.default);
        field2.default = p.default;
      }
      if (field2.source === "fleet.gemini-accounts" && (!field2.dependsOn?.length || field2.default !== void 0)) throw new Error("\u8D26\u53F7\u6765\u6E90\u9700\u8981\u76EE\u6807\u53C2\u6570\u4F9D\u8D56\uFF0C\u4E0D\u80FD\u914D\u7F6E\u9ED8\u8BA4\u8D26\u53F7");
      return field2;
    });
    const template = text(a.template, "\u63D0\u793A\u8BCD\u6A21\u677F", 8e3);
    if (new Set(parameters.map((p) => p.label)).size !== parameters.length) throw new Error("\u53C2\u6570\u540D\u79F0\u5FC5\u987B\u552F\u4E00\uFF0C\u907F\u514D\u5360\u4F4D\u7B26\u6B67\u4E49");
    ...
    if (parameters.some((p) => !used.includes(p.key))) throw new Error("\u6BCF\u4E2A\u53C2\u6570\u90FD\u5FC5\u987B\u5728\u6A21\u677F\u4E2D\u4F7F\u7528");
    ...
    for (const flag of ["enabled", "isDefault"]) if (a[flag] !== void 0 && typeof a[flag] !== "boolean") throw Error("Action \u5F00\u5173\u65E0\u6548");
    return {
      id: a.id,
      name: text(a.name, "Action \u540D\u79F0", 80),
      description: text(a.description ?? "", "\u8BF4\u660E", 300, true),
      template,
      parameters,
      ...a.enabled !== void 0 ? { enabled: a.enabled } : {},
      ...a.isDefault !== void 0 ? { isDefault: a.isDefault } : {}
    };
  });
}
function validateValue(p, v) {
  if (p.type === "text" ? typeof v !== "string" || v.length > 4e3 : p.type === "number" ? typeof v !== "number" || !Number.isFinite(v) : typeof v !== "boolean") throw new Error(`${p.label}\uFF1A${p.type === "number" ? "\u8BF7\u586B\u5199\u6709\u6548\u6570\u5B57" : "\u53C2\u6570\u683C\u5F0F\u4E0D\u6B63\u786E"}`);
  ...
  if (p.type === "number" && (p.integer && !Number.isInteger(v) || p.min !== void 0 && Number(v) < p.min)) throw new Error(`${p.label}\uFF1A\u9700\u8981${p.integer ? "\u6574\u6570" : "\u6570\u5B57"}${p.min === void 0 ? "" : `\uFF0C\u6700\u5C0F ${p.min}`}`);
}
function parameterVisible(p, values) {
  return !p.visibleWhen || String(values[p.visibleWhen.key]) === String(p.visibleWhen.equals);
}
function renderAction(action, raw) {
  ...
  const typed = /* @__PURE__ */ Object.create(null);
  for (const p of action.parameters) {
    if (!parameterVisible(p, typed)) {
      resolved[p.key] = p.inactiveValue ?? "";
      continue;
    }
    const v = values[p.key] ?? (p.acceptDefaultOnEnter === false ? void 0 : p.default);
    if (v === void 0 || v === "") {
      if (p.required) throw new Error(`\u8BF7\u586B\u5199${p.label}`);
      resolved[p.key] = "";
      continue;
    }
    validateValue(p, v);
  ...
  return `[Action: ${action.name} \xB7 ${action.id}]
...
async function withPresetLock(dir, write) {
  const key = resolve(dir);
  const next = (pending.get(key) ?? Promise.resolve()).catch(() => void 0).then(write);
...
// src/agent-action-store.ts
var ACTION_FILE = "actions.json";
async function readActions(dir) {
  let content = "";
  try {
    content = await readFile(join(dir, ACTION_FILE), "utf8");
  } catch (e) {
...
async function saveActions(dir, raw, revision) {
...
function optionPage(items, search = "", page = 1, notice) {
  if (typeof search !== "string" || search.length > 500 || !Number.isInteger(page) || page < 1 || page > 1e4) throw Error("\u5019\u9009\u67E5\u8BE2\u53C2\u6570\u65E0\u6548");
...
function sourceTool(p) {
  const source = ACTION_SOURCES.find((s) => s.id === p.source);
  if (!source) throw Error("\u672A\u6CE8\u518C\u7684\u53EA\u8BFB\u5019\u9009\u6765\u6E90");
  return source.tool;
}

// src/fleet-action-options.ts
import { isAbsolute, dirname, join as join2, basename } from "node:path";
import { pathToFileURL } from "node:url";
var reads = /* @__PURE__ */ new Map();
function cachedRead(key, read) {
  const previous = reads.get(key);
  if (previous && Date.now() - previous.at < 5e3) return previous.promise;
  const entry = { at: Infinity, promise: Promise.resolve().then(read).then((value) => {
    entry.at = Date.now();
    return value;
  }) };
  reads.set(key, entry);
...
async function fleetActionOptions(config, p, values, load, intakeOnly = false) {
  ...
  const module = load ?? ((name2) => import(pathToFileURL(join2(dirname(entry), name2 + ".mjs")).href));
  const [{ request }, { policy }] = await Promise.all([module("transport"), module("runtime")]);
  const grants = await policy();
  const get = (path, authenticated = false) => load ? request(path, null, authenticated) : cachedRead(`${entry}:${path}`, () => request(path, null, authenticated));
  if (p.source === "fleet.nodes") {
    ...
    return { items: inventory.nodes.filter((n) => n.ip && n.readAuthorized).map((n) => ({ value: n.ip, label: n.ip, detail: `${n.nodeId} \xB7 ${n.reachable ? "\u5728\u7EBF" : "\u4E0D\u53EF\u8FBE"} \xB7 \u6D4F\u89C8\u5668 ${n.browsers.length}` })), notice: grants.scope === "registered-fleet" ? "\u5DF2\u767B\u8BB0\u542F\u7528\u7684\u673A\u5668\u7EDF\u4E00\u7531\u6D4F\u89C8\u5668 MCP \u7BA1\u7406\uFF0C\u65E0\u9700\u9010\u673A\u6388\u6743\uFF1B\u6267\u884C\u524D\u4ECD\u68C0\u67E5\u8D44\u6E90\u548C\u5B9E\u4F8B\u72B6\u6001\u3002" : "\u53EA\u8BFB\u540D\u518C\uFF1B\u4E5F\u53EF\u624B\u586B\u65B0\u673A\u5668 IP\u3002\u9009\u62E9\u4E0D\u4EE3\u8868\u5DF2\u6709\u64CD\u4F5C\u6743\u9650\u3002" };
  }
  ...
  if (vaultEnabled) {
    ...
    return {
      items: ranked.filter((c) => c.kind === "vault" && c.authorized && !c.reasons.includes("account-excluded") && c.label && /^gemini_[a-f0-9]{8,64}$/.test(c.accountId) && !seen.has(c.accountId) && seen.add(c.accountId)).map((c) => {
        const holders = accounts.find((a) => a.credential?.accountId === c.accountId)?.holders ?? [];
        const usage = holders.map((h) => `${h.ip ?? h.node} / browser-${h.instance}`).join("\u3001");
        return {
          value: `${c.label} \xB7 accountId=${c.accountId} \xB7 #${c.fingerprint}`,
          label: c.label,
          detail: `\u91D1\u5E93 v${c.version} \xB7 ${c.eligible ? "\u53EF\u4E0B\u53D1" : c.reasons.join("\u3001")} \xB7 \u5F53\u524D\u5DF2\u9A8C\u8BC1 ${c.currentHolders} \u4E2A\u6D4F\u89C8\u5668 \xB7 ${usage || "\u65E0\u5728\u7EBF\u6765\u6E90\uFF0C\u4E5F\u53EF\u4F7F\u7528\u6709\u6548\u5E93\u5B58"}`,
          disabled: !c.eligible
        };
      }),
  ...
  return {
    items: ranked.filter((c) => c.authorized && !c.reasons.includes("account-excluded") && c.label && c.sourceIp && Number.isInteger(c.sourceInstance)).map((c) => ({
      value: `${c.label} \xB7 ${c.sourceIp} / browser-${c.sourceInstance} \xB7 #${c.fingerprint}`,
      label: c.label,
      detail: `${c.sourceIp} \xB7 browser-${c.sourceInstance} \xB7 ${c.eligible ? "\u5DF2\u9A8C\u8BC1" : c.reasons.length === 1 && c.reasons[0] === "source-not-currently-verified" ? "\u5F85\u6267\u884C\u524D\u590D\u9A8C" : c.reasons.join("\u3001")}`,
      ...
      disabled: c.reasons.some((r) => r !== "source-not-currently-verified")
    })),
...
import { cp, mkdir, readFile as readFile2, readdir, rename as rename2, rm as rm2, stat, writeFile as writeFile2, chmod } from "node:fs/promises";
...
function publicToolName(serverName, rawName) {
  ...
  const hash3 = createHash2("sha256").update(`${serverName}\0${rawName}`).digest("hex").slice(0, 12);
  return `${normalized.slice(0, 51)}_${hash3}`;
}
...
async function registerWorkerTools(agentCtx, hooks, options = {}) {
  ...
  if (hooks.notify) disposers.push(agentCtx.tools.register(defineTool({
    ...
    parameters: { stage: { type: "string", required: true, enum: ["started", "findings", "rework", "restored", "unresolved", "blocked", "completed"] } },
...
function userPresetRoot(home = homedir()) {
  return join3(process.env.DSH_HOME ?? join3(home, ".dsh"), ".agent-presets");
}
function skillRoots(home = homedir()) {
  return [
    { root: join3(process.env.DSH_HOME ?? join3(home, ".dsh"), "skills"), label: "user-dsh" },
    { root: join3(process.env.DSH_AGENTS_HOME ?? join3(home, ".agents"), "skills"), label: "user-agents" }
  ];
}
function frontmatter(text3) {
  const m = /^---\r?\n([\s\S]*?)\r?\n---/.exec(text3);
  if (!m) return {};
  const out = {};
...
async function scanSkills(home = homedir()) {
  ...
  for (const { root, label } of skillRoots(home)) {
    ...
    for (const name2 of names.sort()) {
      ...
      const dir = join3(root, name2);
      try {
        if (!(await stat(dir)).isDirectory()) continue;
        const text3 = await readFile2(join3(dir, "SKILL.md"), "utf8");
        const fm = frontmatter(text3);
...
function indent(text3, n) {
  const pad = " ".repeat(n);
  return text3.split("\n").map((l) => l.length ? pad + l : l).join("\n");
}
function mask(text3) {
  return text3.replace(/(Authorization:\s*)(["']?)Bearer\s+\S+/gi, "$1$2Bearer \u2022\u2022\u2022\u2022").replace(/((?:token|secret|password|api[-_]?key)\s*:\s*)(["']?)[^\s"'#]+/gi, "$1$2\u2022\u2022\u2022\u2022").replace(/\/\/[^@\s/]+@/g, "//\u2022\u2022\u2022\u2022@");
}
...
function selectedSkill(specName, library) {
  return library.find((skill) => skill.name === specName) ?? library.find((skill) => basename2(skill.dir) === specName);
}
async function hashSkillTree(root) {
  const digest3 = createHash3("sha256");
  async function walk(dir, prefix) {
    ...
    for (const entry of entries) {
      ...
      if (info.isDirectory()) {
        digest3.update(`d\0${relative3}\0`);
        await walk(path, relative3);
      } else if (info.isFile()) {
        const body2 = await readFile2(path);
        digest3.update(`f\0${relative3}\0${body2.length}\0`);
        digest3.update(body2);
      }
    }
  }
  await walk(root, "");
...
async function verifyPresetSkills(spec, library, dir) {
  const lock = await readFile2(join3(dir, SKILL_LOCK_FILE), "utf8").then(parseSkillLock).catch(() => null);
  ...
  for (const name2 of spec.skills) {
    ...
    const copied = join3(dir, "skills", source ? basename2(source.dir) : name2);
...
async function syncPresetSkills(spec, library, dir) {
  ...
  const skillsDir = join3(dir, "skills");
  const lockPath = join3(dir, SKILL_LOCK_FILE);
  const staged = join3(dir, `.skills-next-${randomUUID3()}`);
  const stagedLock = join3(dir, `.skills-lock-next-${randomUUID3()}`);
  const backup = join3(dir, `.skills-backup-${randomUUID3()}`);
  const backupLock = join3(dir, `.skills-lock-backup-${randomUUID3()}`);
  ...
  await mkdir(staged, { recursive: true, mode: 448 });
  try {
    for (const { name: name2, entry } of sources) {
      const target = join3(staged, basename2(entry.dir));
      await cp(entry.dir, target, {
        recursive: true,
        dereference: true,
        filter: (source) => managedSkillEntry(basename2(source))
      });
      ...
      lock.skills.push({ name: name2, sourceRoot: entry.root, sourceBasename: basename2(entry.dir), sha256: sourceSha256 });
    }
    await writeFile2(stagedLock, JSON.stringify(lock, null, 2) + "\n", { mode: 384 });
    const hadSkills = await stat(skillsDir).then(() => true).catch(() => false);
    const hadLock = await stat(lockPath).then(() => true).catch(() => false);
    if (hadSkills) {
      await rename2(skillsDir, backup);
      skillsBackedUp = true;
    }
    if (hadLock) {
      await rename2(lockPath, backupLock);
      lockBackedUp = true;
    }
    try {
      if (sources.length) {
        await rename2(staged, skillsDir);
        newSkillsInstalled = true;
      } else await rm2(staged, { recursive: true, force: true });
      await rename2(stagedLock, lockPath);
      newLockInstalled = true;
    } catch (error) {
      if (newSkillsInstalled) await rm2(skillsDir, { recursive: true, force: true });
      if (newLockInstalled) await rm2(lockPath, { force: true });
      if (skillsBackedUp) {
        await rename2(backup, skillsDir);
        skillsBackedUp = false;
      }
      if (lockBackedUp) {
        await rename2(backupLock, lockPath);
        lockBackedUp = false;
      }
      throw error;
    }
    ...
    if (lockBackedUp) {
      await rm2(backupLock, { force: true });
      lockBackedUp = false;
    }
    return lock;
  } catch (error) {
    await rm2(staged, { recursive: true, force: true });
    await rm2(stagedLock, { force: true });
    if (!skillsBackedUp) await rm2(backup, { recursive: true, force: true });
    if (!lockBackedUp) await rm2(backupLock, { force: true });
    throw error;
  }
}
...
async function writePreset(spec, hostMcp, library, root = userPresetRoot(), inheritedTools = [], options = {}) {
  return withPresetLock(resolve2(root, spec.id), () => writePresetLocked(spec, hostMcp, library, root, inheritedTools, options));
}
async function writePresetLocked(spec, hostMcp, library, root, inheritedTools, options) {
  const dir = resolve2(root, spec.id);
  if (!dir.startsWith(resolve2(root) + "/")) throw new Error("\u975E\u6CD5 id");
  const availableSkills = spec.skills.filter((name2) => selectedSkill(name2, library));
  if (!options.allowMissingSkills) {
  ...
  const existed = await stat(dir).then(() => true).catch((error) => {
    if (error.code === "ENOENT") return false;
    throw error;
  });
  const createdAt = existed ? await readAgentCreatedAt(dir) : (/* @__PURE__ */ new Date()).toISOString();
  const staged = resolve2(root, `.${spec.id}-next-${randomUUID3()}`);
  const backup = resolve2(root, `.${spec.id}-backup-${randomUUID3()}`);
  let backedUp = false;
  await mkdir(staged, { recursive: true, mode: 448 });
  try {
    await writeFile2(join3(staged, "agent.cordis.yml"), preview2.yml, { mode: 384 });
    await writeFile2(join3(staged, "preset.yml"), `name: ${JSON.stringify(spec.name)}
description: ${JSON.stringify(spec.description)}
`, { mode: 384 });
    await writeFile2(join3(staged, SPEC_FILE), JSON.stringify(spec, null, 2) + "\n", { mode: 384 });
    await writeFile2(join3(staged, "agent-meta.json"), JSON.stringify({ createdAt }) + "\n", { mode: 384 });
    await syncPresetSkills({ skills: availableSkills }, library, staged);
    if (spec.skills.length) await mkdir(join3(staged, "skills"), { recursive: true, mode: 448 });
    if (existed) {
      try {
        await writeFile2(join3(staged, ACTION_FILE), await readFile2(join3(dir, ACTION_FILE)), { mode: 384 });
      } catch (error) {
    ...
    await rm2(staged, { recursive: true, force: true });
    if (!backedUp) await rm2(backup, { recursive: true, force: true });
    throw error;
  }
...
async function readSpec(dir) {
  try {
    return validateSpec(JSON.parse(await readFile2(join3(dir, SPEC_FILE), "utf8")));
  } catch {
    return null;
  }
}
async function readAgentCreatedAt(dir) {
  try {
    const { createdAt } = JSON.parse(await readFile2(join3(dir, "agent-meta.json"), "utf8"));
...
async function removePreset(id, root = userPresetRoot()) {
  ...
  const dir = resolve2(root, id);
  if (!dir.startsWith(resolve2(root) + "/")) throw new Error("\u975E\u6CD5 id");
  await withPresetLock(dir, () => rm2(dir, { recursive: true, force: true }));
}

// src/session-capabilities.ts
var CAPABILITY_TOOLS = ["session_capabilities", "environment_capabilities"];
var STANDARD_LIMIT = 24;
var REPEAT_LIMIT = 4;
var canonical = (v) => Array.isArray(v) ? v.map(canonical) : v && typeof v === "object" ? Object.fromEntries(Object.keys(v).sort().map((k) => [k, canonical(v[k])])) : v;
var digest = (v) => createHash4("sha256").update(JSON.stringify(canonical(v)) ?? "").digest("hex");
var standard = (agent) => !!agent?.session && !agent.session.id.startsWith("task-") && (!agent.session.header?.agentPreset || agent.session.header.agentPreset === "standard");
var roleOf = (agent) => agent?.session?.header?.agentPreset ?? "standard";
var toolText = (message) => (message?.content ?? []).flatMap((b) => b.type === "tool-result" ? b.content ?? [] : [b]).filter((b) => b.type === "text").map((b) => b.text).join("\n");
function resolveMcpToolIdentity(name2, declared, sources) {
  ...
  const exact = candidates.find((candidate) => candidate.name === name2);
  if (exact) return exact;
  if (!name2.startsWith("mcp__")) return void 0;
  return candidates.find((candidate) => {
    const suffix = `__${candidate.rawName}`;
    if (!name2.endsWith(suffix)) return false;
    const namespace = name2.slice(5, -suffix.length);
    const stable = candidate.server.replace(/[^A-Za-z0-9_-]/g, "_");
    return namespace === stable || namespace.startsWith(`${stable}-`);
  });
}
var ChatProgress = class {
  turn = -1;
  ...
  observe(event) {
    ...
    const hash3 = digest(toolText(d.message)), previous = this.results.get(call.key);
    const repeats = previous?.hash === hash3 ? previous.repeats + 1 : 1;
    this.results.set(call.key, { hash: hash3, repeats });
    if (repeats >= REPEAT_LIMIT) this.reason = "\u540C\u4E00\u5DE5\u5177\u548C\u53C2\u6570\u5DF2\u91CD\u590D\u8FD4\u56DE\u76F8\u540C\u7ED3\u679C\uFF0C\u5DF2\u505C\u6B62\u65E0\u8FDB\u5C55\u8C03\u7528\u3002\u8BF7\u6839\u636E\u5DF2\u6709\u7ED3\u679C\u56DE\u7B54\uFF1B\u9700\u8981\u65B0\u64CD\u4F5C\u65F6\u7531\u7528\u6237\u660E\u786E\u63D0\u51FA\u3002";
  }
  boundary(step, maxSteps = STANDARD_LIMIT) {
    return this.reason || (step > maxSteps ? `\u666E\u901A\u4F1A\u8BDD\u5DF2\u8FBE\u5230 ${maxSteps} \u6B65\u9884\u7B97\uFF0C\u5DF2\u505C\u6B62\u7EE7\u7EED\u8C03\u7528\u3002\u8BF7\u7F29\u5C0F\u672C\u6B21\u8303\u56F4\u6216\u8F6C\u4EA4\u5DF2\u914D\u7F6E\u7684\u4EFB\u52A1\u6D41\u7A0B\u3002` : "");
  }
};
var SessionCapabilities = class {
  constructor(ctx, environment, db, mcpSources = () => []) {
    this.ctx = ctx;
    this.environment = environment;
    this.db = db;
    this.mcpSources = mcpSources;
  ...
  state(agent) {
    ...
    if (!state) {
      state = new ChatProgress();
      for (const event of agent.session.events ?? []) state.observe(event);
      this.progress.set(agent.session, state);
    }
    return state;
  }
  restricted(schema, agent) {
    if (!standard(agent)) return false;
    return this.policy.standardExcludedTools?.includes(schema.name) || this.policy.standardMcpInheritance === "discover-only" && schema.name.startsWith("mcp__") || this.policy.standardSkillInheritance === "discover-only" && schema.name === "skill" || this.policy.standardExcludedMcpServers?.some((server) => /^[A-Za-z0-9_-]{1,32}$/.test(server) && schema.name.startsWith(`mcp__${server}__`)) || this.mcpSources().some((m) => this.policy.standardExcludedMcpServers?.includes(m.serverName) && m.tools.some((n) => publicToolName(m.serverName, n) === schema.name));
  }
  skillRestricted(name2, agent) {
    return standard(agent) && (this.policy.standardSkillInheritance === "discover-only" || this.policy.standardExcludedTools?.includes("skill") || this.policy.standardExcludedSkills?.includes(name2));
  }
  filterSkillMessages(messages, agent) {
    return messages.flatMap((message) => {
      ...
      const entries = source.entries.filter((s) => !this.skillRestricted(s.name, agent));
...
${entries.map((s) => JSON.stringify({ name: s.name, description: s.description })).join("\n")}
\u6267\u884C\u5339\u914D\u4EFB\u52A1\u524D\u4F7F\u7528 skill \u5DE5\u5177\u52A0\u8F7D\u5B8C\u6574\u8BF4\u660E\uFF1B\u76EE\u5F55\u4E0D\u662F\u6388\u6743\u7ED5\u8FC7\u5176\u4ED6\u5DE5\u5177\u9650\u5236\u3002` }] }];
    });
  }
  async describe(agent, exposed) {
    const session = agent.session, role = roleOf(agent);
    const spec = /^[a-zA-Z0-9_-]+$/.test(role) ? await readSpec(resolve3(userPresetRoot(), role)) : null;
    const schemas = this.ctx.tools.schemas(agent);
    const available = exposed ?? schemas.filter((s) => !this.restricted(s, agent));
    const availableNames = new Set(available.map((s) => s.name));
    const declared = new Set(spec ? [...spec.tools.flatMap((id) => NATIVE_TOOLS.find((t) => t.id === id)?.schemaNames ?? []), ...Object.entries(spec.mcpTools).flatMap(([server, names]) => names.map((n) => publicToolName(server, n))), ...spec.skills.length ? ["skill"] : []] : []);
    ...
    const errors = /* @__PURE__ */ new Map(), loaded = /* @__PURE__ */ new Set(), calls = /* @__PURE__ */ new Map();
    for (const event of session.events ?? []) {
      ...
      const text3 = toolText(d.message);
      const failed = /^error|"ok"\s*:\s*false|Authentication Fails/i.test(text3);
      if (failed) errors.set(call.name, /Authentication Fails|unauthorized|api key.*invalid/i.test(text3) ? "authentication-failed" : /session-required|not.granted|not.authorized|denied/i.test(text3) ? "permission-or-identity-denied" : "last-call-failed");
      else {
        errors.delete(call.name);
        if (call.name === "skill") {
          try {
            loaded.add(JSON.parse(call.arguments).name);
          } catch {
          }
        }
      }
    }
    let skills = [], skillDiscovery = "unavailable";
    if (this.ctx.get("skills")) {
      try {
        const catalog = await this.ctx.get("skills").snapshot({ scope: agent, cwd: session.header?.cwd });
        skills = catalog.skills.filter((s) => s.invocation?.modelInvocable !== false).map((s) => ({ name: s.name, source: spec?.skills.includes(s.name) ? "agent-definition" : "environment-inherited", state: this.skillRestricted(s.name, agent) ? "restricted" : loaded.has(s.name) ? "loaded-in-session-history" : availableNames.has("skill") ? "available-on-demand" : "not-callable", provider: s.provider }));
        skillDiscovery = catalog.complete ? "complete" : "partial";
      } catch {
        skillDiscovery = "discovery-failed";
      }
    }
    const registeredStable = /* @__PURE__ */ new Set();
    ...
    const out = {
      ...
      current: { tools, skills, skillDiscovery, permissionPreset: this.ctx.get("permissionPresets")?.current(session.events ?? []) ?? "not-observed", configuredButNotRegistered: [...declared].filter((n) => !schemas.some((s) => s.name === n) && !registeredStable.has(n)) },
      answerContract: { language: "\u7528\u6237\u5F53\u524D\u6D88\u606F\u7684\u8BED\u8A00", registeredIsNotAuthorized: true, cannotCall: tools.filter((t) => t.state === "restricted").map((t) => t.name), requiredCaveat: "\u5FC5\u987B\u5355\u72EC\u8BF4\u660E\u53D7\u9650\u5DE5\u5177\u4E0D\u53EF\u8C03\u7528\uFF1B\u5176\u4F59\u5DF2\u6CE8\u518C\u5DE5\u5177\u4E5F\u4E0D\u4FDD\u8BC1\u51ED\u636E\u6709\u6548\u6216\u5177\u4F53\u64CD\u4F5C\u83B7\u6388\u6743\u3002\u7981\u6B62\u603B\u7ED3\u4E3A\u5168\u90E8\u80FD\u529B\u90FD\u53EF\u4F7F\u7528\u3002Skill \u662F\u53EF\u6309\u9700\u52A0\u8F7D\uFF0C\u4E0D\u7B49\u4E8E\u5DF2\u52A0\u8F7D\u3002" },
  ...
  async install() {
    const disposers = [];
    const defineTool = process.env.NODE_ENV === "test" ? (s) => s : (await import("@deepseek-ai/dsh-tools")).defineTool;
    for (const name2 of CAPABILITY_TOOLS) disposers.push(this.ctx.tools.register(defineTool({
      name: name2,
      description: name2 === "session_capabilities" ? "\u53EA\u8BFB\u67E5\u8BE2\u5F53\u524D\u4F1A\u8BDD\u771F\u5B9E\u80FD\u529B\uFF1A\u89D2\u8272\u5B9A\u4E49\u3001\u5B9E\u9645\u6CE8\u518C\u5DE5\u5177\u3001\u6309\u9700 Skill\u3001\u73AF\u5883\u7EE7\u627F\u53CA\u6700\u8FD1\u5931\u8D25\u3002\u95EE\u4F60\u6709\u54EA\u4E9B\u80FD\u529B\u65F6\u5148\u4F7F\u7528\u672C\u5DE5\u5177\uFF1B\u4E0D\u8981\u8BD5\u8C03\u7528\u4E1A\u52A1\u5DE5\u5177\u3002" : "\u53EA\u8BFB\u67E5\u8BE2\u73AF\u5883\u5DF2\u5B89\u88C5\u80FD\u529B\u548C\u5DF2\u914D\u7F6E Agent\uFF1B\u76EE\u5F55\u4E0D\u6388\u4E88\u6267\u884C\u6743\u9650\uFF0C\u4E0D\u5B89\u88C5\u3001\u4E0D\u59D4\u6D3E\u3001\u4E0D\u8FD0\u884C\u4E1A\u52A1\u5DE5\u5177\u3002",
      parameters: {},
      output: { schema: { type: "object", additionalProperties: true }, render: (_, v) => [{ type: "text", text: JSON.stringify(v) }] },
      execute: async (_, exec) => {
        if (!exec.agent?.session) throw Error("Live session required");
        return name2 === "session_capabilities" ? this.describe(exec.agent) : this.environment();
      }
    })));
    disposers.push(this.ctx.tools.guard((exec) => {
      ...
      return this.state(exec.agent).reason || void 0;
    }));
    disposers.push(this.ctx.on("session/event", (session, event) => {
      if (event.type === "request/header") {
        ...
        const request = { checkedAt, provider: header.config?.provider, model: header.config?.model, tools: (header.tools ?? []).map((s) => s.name) };
        this.db.prepare("INSERT INTO dsh_session_capability_snapshots (session_id,checked_at,snapshot_json,request_json) VALUES (?,?,?,?) ON CONFLICT(session_id) DO UPDATE SET request_json=excluded.request_json").run(session.id, checkedAt, JSON.stringify({ sessionId: session.id, checkedAt, live: false, definition: { role: session.header?.agentPreset ?? "standard" } }), JSON.stringify(request));
      }
      const state = this.progress.get(session);
      if (state) state.observe(event);
    }));
    disposers.push(this.ctx.on("system-prompt/assemble", async (_assembly, context, next) => {
      const assembly = await next(), agent = context.scope;
      if (!agent?.session) return assembly;
      const tools = assembly.tools.filter((s) => !this.restricted(s, agent));
    ...
    disposers.push(this.ctx.on("agent/pre-step", async (input, next) => {
...
function foldTurns(sessionId, events, agentPreset) {
  ...
  let model = {};
  let pendingUser = "";
  const iso3 = (t) => new Date(t).toISOString();
  for (const e of events) {
    const d = e.data ?? {};
    switch (e.type) {
      ...
      case "request/context":
        ...
        break;
      case "turn/start":
        cur = { turn: d.turn, at: iso3(e.time), user: pendingUser, steps: [] };
        pendingUser = "";
        turns.push(cur);
        totals.turns++;
        break;
      case "step/start":
        if (!cur) {
          cur = { turn: d.turn ?? turns.length + 1, at: iso3(e.time), user: pendingUser, steps: [] };
          turns.push(cur);
          totals.turns++;
        }
        step = { step: d.step, ...model, at: iso3(e.time), ms: 0, usage: { input: 0, output: 0, reasoning: 0, cacheRead: 0 }, tools: [], text: "" };
        cur.steps.push(step);
        totals.steps++;
        break;
      ...
      case "tool/call": {
        ...
        const row = { callId: d.callId, name: m ? m[2] : name2, kind, server: m?.[1], args: preview(d.arguments, 240), result: "", ok: true, ms: 0, at: iso3(e.time), _t: e.time };
...
function validateDesign(value) {
  ...
  if (d.notifications !== void 0) {
    ...
    if (n.mode !== void 0 && n.mode !== "final-handoff") throw new Error("\u672A\u77E5\u901A\u77E5\u6A21\u5F0F");
  ...
  if (d.evidenceContract === "browser-patrol-v2") {
    ...
    if (p.resumeAfterCopyLimit !== void 0) {
  ...
  const branches = d.branches.map((b) => {
    ...
    return { id: b.id, when: text3(b.when, "when"), action: text3(b.action, "action"), evidence: text3(b.evidence, "evidence") };
  });
  ...
  return {
    ...
    ...proxy ? { proxy } : {},
    scope: text3(d.scope, "scope"),
    branches,
    coordination: text3(d.coordination, "coordination"),
...
import { copyFile, mkdir as mkdir2, readFile as readFile4, realpath as realpath2, stat as stat3 } from "node:fs/promises";
import { basename as basename4, extname, isAbsolute as isAbsolute3, join as join4, relative as relative2, resolve as resolve5, sep as sep2 } from "node:path";

// src/public-upload.ts
import { homedir as homedir2 } from "node:os";
import { readFile as readFile3, realpath, stat as stat2 } from "node:fs/promises";
import { basename as basename3, isAbsolute as isAbsolute2, relative, resolve as resolve4, sep } from "node:path";
...
async function readPublishableHtml(filePath) {
  ...
  const file = await realpath(resolve4(raw));
  const configured = (process.env.DSH_TASK_CONSOLE_PUBLISH_ROOTS ?? homedir2()).split(":").map((value) => value.trim()).filter(Boolean);
  const roots = await Promise.all(configured.map((root) => realpath(resolve4(root))));
  ...
  return { path: file, name: basename3(file), data: await readFile3(file) };
}
...
function inside2(child, parent) {
  ...
  return r === "" || !r.startsWith(`..${sep2}`) && r !== ".." && !isAbsolute3(r);
}
function safePart(value) {
  return value.normalize("NFKC").replace(/[^\p{L}\p{N}._-]+/gu, "-").replace(/^[-.]+|[-.]+$/g, "").slice(0, 100) || "artifact";
}
async function sha256(path) {
  return createHash5("sha256").update(await readFile4(path)).digest("hex");
}
async function captureArtifacts(ctx, requested) {
  ...
  for (const raw of [...new Set(requested.map((x) => String(x).trim()).filter(Boolean))]) {
    const candidate = resolve5(ctx.task.cwd, raw);
    let original;
    try {
    ...
    const info = await stat3(original);
    if (!info.isFile()) throw new Error(`\u4EA7\u7269\u4E0D\u662F\u666E\u901A\u6587\u4EF6:${raw}`);
    if (info.size > MAX_CAPTURE_BYTES) throw new Error(`\u4EA7\u7269\u8D85\u8FC7 20 MiB:${raw}`);
    checked.push({ original, name: safePart(basename4(original)), size: info.size, sha: await sha256(original) });
  }
  const dir = join4(ctx.root, "artifacts", safePart(ctx.task.id), safePart(ctx.batchId), safePart(ctx.runId));
  await mkdir2(dir, { recursive: true, mode: 448 });
  const out = [];
  for (const file of checked) {
    const id = `a-${randomUUID4()}`;
    const storagePath = join4(dir, `${id.slice(2, 10)}-${file.name}`);
...
async function discoverLegacyArtifacts(task, runs, knownOriginals) {
  ...
  for (const { run, path } of found.values()) {
    try {
      ...
      out.push({
        id: `legacy-${createHash5("sha256").update(`${run.id}\0${original}`).digest("hex").slice(0, 24)}`,
        taskId: task.id,
        batchId: run.batchId,
        cardId: run.cardId,
        runId: run.id,
        sessionId: run.sessionId,
        name: safePart(basename4(original)),
...
async function readArtifact(root, task, artifact) {
  ...
  const allowed = artifact.legacy ? await realpath2(task.cwd) : await realpath2(join4(root, "artifacts"));
...
// src/task-list.ts
function taskListIndex(store, input = {}, names = /* @__PURE__ */ new Map()) {
  ...
  for (const n of [input.page, input.pageSize]) if (n !== void 0 && (!Number.isSafeInteger(n) || n < 1)) throw Error("\u65E0\u6548\u5206\u9875\u53C2\u6570");
  const pageSize = Math.min(10, input.pageSize ?? 10);
  const latest = /* @__PURE__ */ new Map(), counts = /* @__PURE__ */ new Map();
  ...
  const order = { park: 0, review: 1, bad: 2, run: 3, schedule: 4, idle: 5, done: 6 };
  ...
  const metrics = {
    total: all.length,
    active: all.filter((r) => r.state === "run").length,
    attention: all.filter((r) => ["park", "review", "bad"].includes(r.state)).length,
    schedules: all.filter((r) => r.task.trigger.kind === "cron" && r.task.enabled).length,
    ended: all.filter((r) => ["done", "bad"].includes(r.state)).length
  };
  const term = query.trim().toLowerCase();
  const rows = all.filter((r) => (!term || `${r.task.title} ${r.task.brief} ${r.task.participants.map((p) => `${p.agentId} ${names.get(p.agentId) ?? ""}`).join(" ")}`.toLowerCase().includes(term)) && (filter === "all" || filter === "active" && r.state === "run" || filter === "attention" && ["park", "review", "bad"].includes(r.state) || filter === "done" && r.state === "done" || filter === "schedule" && r.task.trigger.kind === "cron" || filter === "ended" && ["done", "bad"].includes(r.state))).sort((a, b) => order[a.state] - order[b.state] || (b.batch?.firedAt ?? b.task.createdAt).localeCompare(a.batch?.firedAt ?? a.task.createdAt) || a.task.id.localeCompare(b.task.id));
...
// src/tasks.ts
import { mkdir as mkdir3, readFile as readFile5 } from "node:fs/promises";
...
var HermesKernel = class {
  ...
  constructor(path, options = {}) {
    ...
    this.claimer = options.claimer ?? (() => `${hostname()}:${process.pid}:${randomUUID5()}`);
  ...
  blockTask(taskId, options) {
    ...
    return this.write(() => {
      ...
      const status = kind === "dependency" && !this.parentsSatisfied(taskId) ? "todo" : recurrences >= BLOCK_RECURRENCE_LIMIT ? "triage" : "blocked";
  ...
  buildWorkerContext(taskId) {
    ...
    const cap = (value, limit = 8e3) => {
      ...
      return text3.length <= limit ? text3 : `${text3.slice(0, limit)}\u2026 [truncated]`;
    };
...
function storeDir(home = homedir3()) {
  return join5(process.env.DSH_HOME ?? join5(home, ".dsh"), "task-console");
}
var EventStore = class {
  ...
  get file() {
    return join5(this.dir, "task.db");
  }
  get legacyFile() {
    return join5(this.dir, "events.jsonl");
  }
  get root() {
    return this.dir;
  }
  ...
  async importLegacyJsonl() {
    let text3 = "";
    try {
      text3 = await readFile5(this.legacyFile, "utf8");
    } catch {
      return;
    }
  ...
  async createNotification(task, batch, planner, stage, report) {
    ...
    await this.transition(() => {
      ...
      const card = { id, agentId, kind: "agent", role: "notifier", round: planner.round, deps: parents, brief };
      ...
      this.kernel.recordEvent(id, "created", { title: `\u4F01\u5FAE\u901A\u77E5 \xB7 ${stage}`, body: brief, assignee: agentId, status: "todo", parents, tenant: batch.id, node_kind: "agent", round: planner.round, role: "notifier", created_at: epoch });
  ...
  claim(task, now) {
    ...
    return this.store.kernel.write(() => {
      ...
      if (!row && state.next_at !== null && state.next_at <= now) {
        ...
        const id = createHash6("sha256").update(`${task.id}:${due}`).digest("hex").slice(0, 24);
...
async function browserJobs(deps) {
  return (deps.jobs || (async () => {
    const root = process.env.FLEET_BROWSER_STATE_DIR || join6(homedir4(), ".local/state/fleet-browser-manager");
    let names;
    try {
...
async function validateWorkflowBlock(input, deps = {}) {
  ...
  const requested = input.metadata?.requestedBlock;
...
async function readBrowserAcceptance(id) {
  ...
  let file;
  try {
    file = await open(join6(root, id + ".json"), constants.O_RDONLY | constants.O_NOFOLLOW);
...
async function validateWorkflowCompletion(input, deps = {}) {
  ...
  for (const id of ids) {
    ...
    const expected = createHash7("sha256").update(JSON.stringify([sessionId, job?.args?.requestId])).digest("hex").slice(0, 32);
...
var TaskRunner = class {
  ...
  async expireBlockedPatrols() {
    for (const batch of this.store.s.batches.values()) {
      ...
      const expired = cards.filter((c) => c.status === "blocked" && c.startedAt && this.clock() >= Date.parse(c.startedAt) + task.timeoutSec * 1e3);
      if (!expired.length) continue;
      const db = this.store.kernel.db;
      if (db.prepare("SELECT 1 FROM sqlite_master WHERE type='table' AND name='dsh_proxy_calls'").get() && db.prepare("SELECT 1 FROM dsh_proxy_calls WHERE batch_id=? AND state IN ('running','unknown') LIMIT 1").get(batch.id)) continue;
      let pending2 = false;
      if (db.prepare("SELECT 1 FROM sqlite_master WHERE type='table' AND name='dsh_browser_operations'").get()) {
        const operations = db.prepare("SELECT operation_id FROM dsh_browser_operations WHERE batch_id=?").all(batch.id);
        for (const row of operations) {
          try {
            const receipt = await readBrowserAcceptance(row.operation_id);
            if (!["complete", "blocked"].includes(receipt.phase)) pending2 = true;
          } catch {
            pending2 = true;
          }
        }
      ...
      for (const card of cards) {
        ...
        if (!run) continue;
        try {
          if (await this.pendingOperation?.({ task, batch, card, sessionId: run.sessionId, profileId: run.profileId ?? card.agentId })) pending2 = true;
        } catch {
          pending2 = true;
        }
      }
      ...
      for (const card of expired) await this.store.transition(
        ...
        (ok) => ok ? { t: "card/gave_up", at: this.now(), taskId: task.id, cardId: card.id, error: "\u5B9A\u65F6\u5DE1\u67E5\u963B\u585E\u8D85\u65F6\uFF0C\u975E\u4E1A\u52A1\u6210\u529F" } : void 0
      );
      await this.settleBatches();
    }
  }
  ...
  async startRun(task, batch, card) {
    ...
    const resumeFacts = card.runIds.length ? await this.operationOutcome?.({ task, batch, card, sessionId, profileId }) : void 0;
    const text3 = `[DSH SESSION]
    ...
    try {
      ...
      try {
        const submit = async (kind, summary, paths, metadata, reviewer) => {
          ...
          const pending2 = await this.pendingOperation?.({ task, batch, card, sessionId, profileId });
          if (pending2) throw new Error(`\u540E\u53F0\u64CD\u4F5C\u4ECD\u5728\u8FD0\u884C\uFF0C\u7EE7\u7EED\u8BFB\u53D6\u7EC8\u6001\u56DE\u6267\uFF0C\u4E0D\u80FD\u63D0\u524D\u63D0\u4EA4\u9A8C\u6536\uFF1A${pending2}`);
        ...
        flight.disposeTools = await registerWorkerTools(flight.handle.agent.ctx, {
          ...task.design?.notifications && (["planner", "notifier"].includes(card.role ?? "") || profileId === task.design.notifications.agentId) && this.notify ? { notify: (stage, exec) => this.notify({ task, batch, card, sessionId, profileId }, stage, async (args) => {
...
${JSON.stringify(plan.items)}`;
          ...
          finalize: async (summary, artifactPath, disposition = "passed") => {
            ...
            if (artifactPath) {
              let originalPath;
              try {
                originalPath = await realpath3(resolve6(task.cwd, artifactPath));
              } catch {
      ...
      flight.handle.agent.followup({ id: messageId, role: "user", content: [{ type: "text", text: text3 }], source: { kind: "user" } });
  ...
  async displayName(id) {
    ...
    try {
      ...
      const spec = await readSpec(dirname3(String(p.path)));
  ...
  async onTurnEnd(f, reason) {
    ...
    if (card && batch && base && this.pendingOperation) {
      try {
        const pending2 = await this.pendingOperation({ task: taskForBatch(base, batch), batch, card, sessionId: f.sessionId, profileId: f.profileId });
        if (!this.flights.has(f.sessionId)) return;
        if (pending2) {
    ...
    if ((run?.nudges ?? 0) < maxNudges) {
      ...
      f.handle.agent.followup({ id: randomUUID7(), role: "user", content: [{ type: "text", text: outcomeNotice ? `${outcomeNotice}
...
${correction}` : correction }], source: { kind: "user" } });
  ...
  async finishBlocked(f, reason, kind) {
    ...
    if (ok && this.afterBlock) {
      ...
      try {
        await this.afterBlock({ task: taskForBatch(base, batch), batch, card, sessionId: f.sessionId, profileId: f.profileId });
      } catch {
        console.warn("[task-console] blocked notification could not be queued; original block retained");
      }
    }
    await this.tick();
  }
...
function stringId(value, field2) {
  ...
  if (!ID2.test(out)) throw new Error(`${field2} \u4E0D\u5408\u6CD5`);
  return out;
}
function validateTaskSignal(raw) {
  ...
  if (goalKey && !ID2.test(goalKey)) throw new Error("goal.key \u4E0D\u5408\u6CD5");
  let incident;
...
var TaskIntakeCoordinator = class {
  ...
  workspace() {
    return this.options.workspace ?? process.env.DSH_TASK_INTAKE_WORKSPACE ?? join7(homedir5(), ".dsh", "task-console", "intake-workspace");
  }
  async start() {
    ...
    const pending2 = db.prepare(`SELECT signal_id FROM dsh_task_signals WHERE status='received' AND parent_signal_id IS NULL ORDER BY received_at`).all();
    for (const row of pending2) this.kick(row.signal_id);
  }
  async submit(raw) {
...
async function decideTaskSignalWithAgent(ctx, signal, context, options = {}) {
  ...
  const digest3 = createHash9("sha256").update(signal.id).digest("hex").slice(0, 12);
  const sessionId = `task-intake-${digest3}-${Date.now().toString(36)}`;
  let handle;
  let disposeTools;
  let proposed;
  ...
  const listener = ctx.on("session/event", (session, event) => {
    ...
    if (!nudged) {
      nudged = true;
      handle?.agent.followup({ id: randomUUID8(), role: "user", content: [{ type: "text", text: "\u4F60\u5C1A\u672A\u63D0\u4EA4\u8DEF\u7531\u51B3\u5B9A\u3002\u5FC5\u987B\u5148\u8C03\u7528 task_intake_context\uFF0C\u518D\u8C03\u7528 task_intake_decide\uFF1B\u4E0D\u8981\u76F4\u63A5\u56DE\u7B54\u6587\u5B57\u3002" }], source: { kind: "user" } });
      return;
    }
...
import { createHash as createHash10, createHmac, timingSafeEqual } from "node:crypto";
import { constants as fsConstants } from "node:fs";
import { access, chmod as chmod2, lstat as lstat2, mkdtemp, open as open2, readFile as readFile7, realpath as realpath4, rm as rm3, stat as stat5, writeFile as writeFile3 } from "node:fs/promises";
...
function messageIpv4s(text3) {
  const values = [];
  for (const match of text3.matchAll(/(?:^|[^\d.])((?:\d{1,3}\.){3}\d{1,3})(?=$|[^\d.])/g)) {
    if (validIpv4(match[1])) values.push(match[1]);
  }
  return [...new Set(values)];
}
function labeledCredential(text3) {
  return {
    username: /(?:^|\s)(?:username|user|账号|用户)(?:\s*(?:[:=：]|is|是)\s*|\s+)([a-z_][a-z0-9_-]{0,31})/i.exec(text3)?.[1],
    password: /(?:^|\s)(?:password|passwd|密码)(?:\s*(?:[:=：]|is|是)\s*|\s+)(\S{1,256})/i.exec(text3)?.[1]
  };
}
function conventionalCredential(text3, ip) {
  const tokens = text3.trim().split(/\s+/);
...
function credentialFromSession(ip, exec) {
  ...
  if (anchorIps.some((value) => value !== ip) || rows.slice(anchor + 1).some((text3) => messageIpv4s(text3).some((value) => value !== ip))) {
    return { available: false, missing: ["ssh_username", "ssh_credential"] };
  }
  let username;
  let password;
  for (let index = anchor; index < rows.length; index += 1) {
    const text3 = rows[index];
    if (!text3) continue;
    const labeled = labeledCredential(text3);
    if (labeled.username) {
    ...
    if (index === anchor) {
      const conventional = conventionalCredential(text3, ip);
...
function validateTaskActions(raw) {
...
function assertTaskActionLogin(snapshot, raw, args, resumed) {
  if (!snapshot) return;
  ...
  const accountId = typeof value === "string" ? value.match(/(?:^|accountId=)(gemini_[a-f0-9]{8,64})(?:$|\s)/)?.[1] : void 0;
  if (!accountId) return;
...
var TaskActions = class {
  ...
  read(id) {
    const task = this.task(id), row = this.db().prepare("SELECT * FROM dsh_task_action_catalog WHERE task_id=?").get(id);
  ...
  save(id, raw, revision) {
    ...
    return db.transaction(() => {
      ...
      const next = hash2([revision, actions]);
  ...
  resolve(input) {
    ...
    for (const p of action.parameters) {
      ...
      const raw = input.values[p.key] ?? (p.acceptDefaultOnEnter === false ? void 0 : p.default);
      values[p.key] = raw === void 0 ? "" : raw;
    }
    const rendered = renderAction(action, values);
    const bound = (binding) => values[action.parameters.find((p) => p.binding === binding)?.key ?? ""];
    const ip = bound("target-ip"), username = bound("ssh-user"), password = bound("ssh-password");
    if (ip !== void 0 && !isIPv4(ip)) throw Error("\u8BF7\u586B\u5199\u4E00\u4E2A\u6709\u6548\u7684\u76EE\u6807 IPv4 \u5730\u5740");
    ...
    const redact = (text3) => password ? text3.split(password).join("[credential supplied privately]") : text3;
    const snapshot = { id: action.id, name: action.name, revision: catalog.revision, template: action.template, parameters: action.parameters, values: Object.fromEntries(Object.entries(safe).map(([k, v]) => [k, typeof v === "string" ? redact(v) : v])) };
    return {
      text: redact(rendered),
      snapshot,
      ips: ip ? [ip] : [],
      accountId,
      credential: password ? { schema: 1, ip, username, password } : void 0
    };
  }
};

// presets/fleet-task-actions.json
var fleet_task_actions_default = [
  {
    id: "onboard-node",
    name: "\u88C5\u673A\u4E0E\u8D26\u53F7\u9A8C\u6536",
    description: "\u590D\u7528\u672C\u5DE5\u4F5C\u6D41\uFF0C\u586B\u5199\u672C\u6B21\u673A\u5668\u548C\u8D26\u53F7\u610F\u56FE\uFF1B\u4FDD\u7559\u539F\u4E09\u89D2\u8272\u4E0E\u53CC\u6D4F\u89C8\u5668\u7A33\u5B9A\u6027\u9A8C\u6536\u3002",
    enabled: true,
    isDefault: true,
    template: "\u8BF7\u5BF9 {{ip}} \u6267\u884C\u672C Task \u5DF2\u5BA1\u67E5\u7684 Fleet \u57FA\u7840\u8282\u70B9\u5E42\u7B49\u63A5\u5165\u4E0E Gemini \u8D26\u53F7\u9A8C\u6536\u6D41\u7A0B\u3002\u9996\u6B21\u63A5\u5165\u65B9\u5F0F\uFF1A{{ssh_mode}}\uFF1BSSH \u7528\u6237\u540D\uFF1A{{username}}\uFF1BSSH \u5BC6\u7801\uFF1A{{password}}\u3002\u5DF2\u6709\u91D1\u5E93\u63A5\u5165\u65B9\u5F0F\u65F6\u4E0D\u91CD\u590D\u7D22\u8981\u51ED\u636E\u3002\u8D26\u53F7\u9009\u62E9\uFF1A{{login_mode}}\uFF1B\u6307\u5B9A\u91D1\u5E93\u8D26\u53F7\uFF1A{{account}}\u3002\u5065\u5EB7\u7EC4\u4EF6\u4E0E\u5DF2\u6709\u5065\u5EB7\u767B\u5F55\u4FDD\u7559\uFF0C\u4E0D\u5378\u8F7D\u91CD\u88C5\uFF0C\u4E0D\u4FEE\u6539\u4EE4\u724C\u3002\u672A\u767B\u5F55\u6D4F\u89C8\u5668\u6309\u672C\u6B21\u9009\u62E9\u8865\u9F50\uFF1B\u6307\u5B9A\u8D26\u53F7\u5FC5\u987B\u5C06 accountId \u4F20\u5165 browser_login_provision\uFF0C\u4F7F\u7528\u91D1\u5E93\u5E93\u5B58\uFF0C\u4E0D\u4F9D\u8D56\u6765\u6E90\u6D4F\u89C8\u5668\u5728\u7EBF\uFF0C\u4E0D\u9759\u9ED8\u6362\u53F7\u3002\u81EA\u52A8\u5206\u914D\u6CBF\u7528 MCP \u65E2\u6709\u5019\u9009\u4E0E\u6392\u9664\u7B56\u7565\u3002\u5DE5\u4F5C\u6D41\u89D2\u8272\u3001\u6743\u9650\u3001\u4EA4\u63A5\u548C\u9A8C\u6536\u4FDD\u6301\u539F\u5B9A\u4E49\uFF1B\u5B8C\u6210\u540E\u8F93\u51FA\u672C\u6B21\u4E09\u4E2A\u89D2\u8272\u7684\u8BC1\u636E\u4E0E\u6700\u7EC8\u62A5\u544A\uFF0C\u4E0D\u628A\u77ED\u671F\u767B\u5F55\u5F53\u4F5C\u539F\u53CC\u6D4F\u89C8\u566820\u5206\u949F\u9A8C\u6536\u901A\u8FC7\u3002",
    parameters: [
      { key: "ip", label: "\u673A\u5668 IP", type: "text", required: true, source: "fleet.nodes", binding: "target-ip" },
      { key: "ssh_mode", label: "SSH \u63A5\u5165\u65B9\u5F0F", type: "text", required: true, choices: ["\u4F7F\u7528\u91D1\u5E93\u63A5\u5165", "\u63D0\u4F9B\u9996\u6B21\u51ED\u636E"], default: "\u4F7F\u7528\u91D1\u5E93\u63A5\u5165" },
      { key: "username", label: "SSH \u7528\u6237\u540D", type: "text", required: true, default: "root", binding: "ssh-user", dependsOn: ["ssh_mode"], visibleWhen: { key: "ssh_mode", equals: "\u63D0\u4F9B\u9996\u6B21\u51ED\u636E" }, inactiveValue: "\u7531\u91D1\u5E93\u89E3\u6790" },
      { key: "password", label: "\u9996\u6B21 SSH \u5BC6\u7801", type: "text", required: true, binding: "ssh-password", dependsOn: ["ssh_mode"], visibleWhen: { key: "ssh_mode", equals: "\u63D0\u4F9B\u9996\u6B21\u51ED\u636E" }, inactiveValue: "\u7531\u91D1\u5E93\u89E3\u6790" },
      { key: "login_mode", label: "\u8D26\u53F7\u9009\u62E9", type: "text", required: true, choices: ["\u6309\u8D26\u53F7\u5206\u914D\u7B56\u7565", "\u6307\u5B9A\u8D26\u53F7"], default: "\u6309\u8D26\u53F7\u5206\u914D\u7B56\u7565" },
      { key: "account", label: "\u91D1\u5E93\u8D26\u53F7", type: "text", required: true, source: "fleet.gemini-accounts", binding: "gemini-account", dependsOn: ["ip", "login_mode"], visibleWhen: { key: "login_mode", equals: "\u6307\u5B9A\u8D26\u53F7" }, inactiveValue: "\u7531\u8D26\u53F7\u5206\u914D\u7B56\u7565\u9009\u62E9" }
    ]
  }
];

// src/task-create.ts
var digest2 = (value) => createHash12("sha256").update(JSON.stringify(value)).digest("hex");
function userInput(exec) {
  const messages = exec.agent?.session?.deriveMessages?.();
  const users = (messages ?? []).filter((m) => m.role === "user" && (!m.source || m.source.kind === "user"));
  const last = users.at(-1);
  const text3 = typeof last?.content === "string" ? last.content : (last?.content ?? []).filter((p) => p.type === "text").map((p) => p.text).join("\n");
  const sessionId = String(exec.agent?.session?.id ?? exec.agent?.session?.header?.id ?? "");
  if (!sessionId || !text3?.trim()) throw new Error("\u9700\u8981\u771F\u5B9E\u7528\u6237\u6D88\u606F\uFF0C\u4E0D\u80FD\u7528\u6A21\u578B\u7F16\u9020\u7684\u8F93\u5165\u521B\u5EFA\u4EFB\u52A1");
  return { text: text3, sessionId, requestId: digest2([sessionId, last.id ?? users.length, text3]) };
}
var TaskCreator = class {
  ...
  queue = Promise.resolve();
  get actions() {
    return new TaskActions(this.runner);
  }
  catalog() {
  ...
  async prepare(proposal, exec, cwd) {
    ...
    const pending2 = this.queue.then(() => this.dispatch(proposal, input, exec, cwd, false, true));
    this.queue = pending2.catch(() => void 0);
    return pending2;
  }
  plansDb() {
    const db = this.runner.store.kernel.db;
  ...
  async scheduledTurn(task, occurrenceId) {
    ...
    if (digest2(selected) !== row.roster_hash) throw new Error("\u5B9A\u65F6\u4EFB\u52A1\u89D2\u8272\u914D\u7F6E\u5DF2\u53D8\u5316\uFF0C\u9700\u91CD\u65B0\u5BA1\u67E5");
    const turn = JSON.parse(row.turn_json);
    if (digest2(workflowDefinition(task)) !== digest2(turn.workflow.definition)) throw new Error("\u5B9A\u65F6\u4EFB\u52A1\u5B9A\u4E49\u4E0E\u5BA1\u67E5\u5FEB\u7167\u4E0D\u4E00\u81F4");
  ...
  async assertScheduleActivation(task) {
    ...
    if (!manual || manual.settled?.outcome !== "done" && !completedPatrol || !manual.turn?.workflow || digest2(manual.turn.workflow.definition) !== digest2(workflowDefinition(task)))
  ...
  async review(id, hash3, decision, reason) {
    const pending2 = this.queue.then(async () => {
      const db = this.plansDb(), row = db.prepare("SELECT * FROM dsh_task_plans WHERE id=?").get(id);
      if (!row || row.hash !== hash3) throw new Error("\u8BA1\u5212\u4E0D\u5B58\u5728\u6216\u6307\u7EB9\u53D8\u5316\uFF0C\u8BF7\u91CD\u65B0\u5BA1\u67E5");
      ...
      if (digest2(selected) !== p.rosterHash) throw new Error("\u53C2\u4E0E Agent \u7684\u80FD\u529B\u6216\u914D\u7F6E\u5DF2\u53D8\u5316\uFF0C\u9700\u521B\u5EFA\u5E76\u5BA1\u67E5\u65B0\u8BA1\u5212");
      if (p.decision === "revise") {
        const definition2 = workflowDefinition(p.task);
        const turn2 = {
          objective: p.recurringObjective ?? `${p.task.brief}

[THIS EXECUTION \u2014 USER REQUEST]
${p.input.text}`,
          participants: p.task.participants,
          userRequest: p.recurringObjective ?? p.input.text,
          workflow: { id: digest2(definition2), definition: definition2 },
          ...p.cwd ? { cwd: p.cwd } : {},
          targets: p.targets,
      ...
      if (p.decision === "reuse") {
        ...
        if (!current || current.archivedAt || !current.enabled && current.trigger.kind !== "cron" || digest2(workflowDefinition(current)) !== digest2(workflowDefinition(p.task))) throw new Error("\u5F85\u590D\u7528\u5DE5\u4F5C\u6D41\u5DF2\u53D8\u5316\uFF0C\u9700\u91CD\u65B0\u5BA1\u67E5");
      }
...
${p.input.text}`,
        participants: p.task.participants,
        userRequest: p.recurringObjective ?? p.input.text,
        workflow: { id: digest2(definition), definition },
        ...p.cwd ? { cwd: p.cwd } : {},
        targets: p.targets,
  ...
  async submit(proposal, exec, cwd) {
    const input = userInput(exec);
    const pending2 = this.queue.then(() => this.dispatch(proposal, input, exec, cwd));
    this.queue = pending2.catch(() => void 0);
    return pending2;
  }
  async launch(taskId, text3, requestId, cwd) {
    if (typeof text3 !== "string" || text3.trim().length < 2 || text3.length > 32e3) throw new Error("\u8BF7\u8F93\u5165\u672C\u6B21\u4EFB\u52A1\u53C2\u6570\uFF08\u6700\u591A 32000 \u5B57\u7B26\uFF09");
    if (!/^[a-zA-Z0-9-]{16,80}$/.test(requestId)) throw new Error("\u9700\u8981\u7A33\u5B9A\u7684\u63D0\u4EA4 ID");
    const exec = { agent: { session: { id: `workflow-${requestId}`, deriveMessages: () => [{ id: requestId, role: "user", content: [{ type: "text", text: text3 }] }] } } };
    const input = { ...userInput(exec), requestId: digest2(["workflow", requestId]) };
    const pending2 = this.queue.then(() => this.dispatch({ decision: "reuse", taskId, reason: "\u7528\u6237\u901A\u8FC7 @ \u9009\u62E9\u5DF2\u6709\u5DE5\u4F5C\u6D41\uFF0C\u63D0\u4EA4\u672C\u6B21\u53C2\u6570" }, input, exec, cwd, true));
    this.queue = pending2.catch(() => void 0);
    return pending2;
  }
  async launchAction(query) {
    ...
    const pending2 = this.queue.then(async () => {
      ...
      const hash3 = digest2([query.taskId, query.actionId, query.revision, query.values, query.cwd]), requestId = digest2(["task-action", query.requestId]), batchId = `b-chat-${requestId.slice(0, 20)}`;
      const old = db.prepare("SELECT * FROM dsh_task_action_requests WHERE id=?").get(requestId);
      if (old && old.hash !== hash3) throw Error("\u540C\u4E00\u63D0\u4EA4\u4E0D\u80FD\u66FF\u6362\u53C2\u6570\uFF0C\u8BF7\u65B0\u5EFA\u4E00\u6B21\u660E\u786E\u7684\u6267\u884C");
      ...
      if (!old) db.prepare("INSERT INTO dsh_task_action_requests VALUES (?,?,?,?)").run(requestId, hash3, query.taskId, batchId);
  ...
  async dispatch(raw, input, exec, cwd, directWorkflow = false, stageOnly = false, actionInput) {
    ...
    try {
      ...
      const actions = proposal.actions === void 0 ? void 0 : validateTaskActions(proposal.actions);
      ...
      if (proposal.decision === "reuse" || proposal.decision === "revise") {
        ...
        if (proposal.decision === "revise") {
          ...
          previous = found;
          const reusable = (text3) => ips.reduce((s, ip) => s.split(ip).join("{{target}}"), text3);
          task = {
            ...found,
            ...validateTask({
              ...found,
      ...
      if (proposal.recurringObjective !== void 0 && (!stageOnly || proposal.decision === "reuse" || task.trigger.kind !== "cron" || typeof proposal.recurringObjective !== "string" || proposal.recurringObjective.trim().length < 2 || proposal.recurringObjective.length > 32e3))
      ...
      const hash3 = digest2({ proposal, text: scrub(input.text), cwd, ...actionInput ? { action: actionInput.snapshot } : {} });
      if (old && old.payload_hash !== hash3) throw new Error("\u540C\u4E00\u63D0\u4EA4\u5DF2\u88AB\u63A5\u53D7\uFF1B\u4E0D\u80FD\u66FF\u6362\u5C1A\u672A\u6D3E\u53D1\u7684\u8BA1\u5212");
      ...
      if (leases.length) {
        const root = join10(store.root, "private-inputs");
        await mkdir5(root, { recursive: true, mode: 448 });
        await writeFile4(join10(root, `${batchId}.json`), JSON.stringify({
          expiresAt: Date.now() + 24 * 36e5,
          credentials: leases.map((l) => JSON.parse(Buffer.from(l.material).toString()))
        }), { mode: 384 });
      }
      if (stageOnly) {
        const planId = `P-chat-${digest2([input.requestId, hash3]).slice(0, 20)}`, db2 = this.plansDb();
        const payload = JSON.stringify({
          ...
          rosterHash: digest2(taskAgentIds(task).map((id) => roster.find((r) => r.id === id)))
        });
        ...
        if (!oldPlan) {
          ...
          db2.transaction(() => {
            ...
            db2.prepare("INSERT INTO dsh_task_plans(id,request_id,source_session,hash,state,title,payload,created_at) VALUES (?,?,?,?,'pending',?,?,?)").run(planId, input.requestId, input.sessionId, hash3, task.title, payload, (/* @__PURE__ */ new Date()).toISOString());
          })();
        }
        return this.plan(planId);
      }
      if (!old) db.prepare("INSERT INTO dsh_task_requests VALUES (?, ?, ?, ?, ?, ?)").run(input.requestId, hash3, task.id, batchId, input.sessionId, (/* @__PURE__ */ new Date()).toISOString());
...
function collectBrowserEvidence(input, events) {
  ...
  for (const server of ["fleet-browser", `fleet-browser-${input.profileId}`]) for (const raw of ["browser_fleet_inventory", "browser_inspect", "browser_status"]) {
    ...
    names.set(full.length <= 64 ? full : `${full.slice(0, 51)}_${createHash13("sha256").update(`${server}\0${raw}`).digest("hex").slice(0, 12)}`, raw);
  }
...
function patrolItemView(row) {
  ...
  if (["signed_out", "unknown"].includes(row.state) && row.loginDelivery?.reason === "interactive-verification-required") return {
    kind: row.state === "signed_out" ? "signed_out" : "unknown",
    state,
    freshness,
    verdict: "\u5F85\u4EBA\u5DE5\u5B89\u5168\u9A8C\u8BC1",
  ...
  if (row.state === "signed_out" && !row.verificationFailure && row.repairExhausted) return {
    ...
    freshness,
    verdict: "\u5DF2\u5904\u7406\uFF0C\u672A\u89E3\u51B3",
    reason: row.loginDelivery?.mutation === "imported" ? `\u5DF2\u5C1D\u8BD5\u4FEE\u590D ${row.attempts} \u6B21\uFF1B\u6700\u8FD1\u4E00\u6B21\u5DF2\u5BFC\u5165\u767B\u5F55\u8D44\u6599\uFF0C\u4F46\u9A8C\u8BC1\u4ECD\u8981\u6C42\u767B\u5F55` : `\u5DF2\u5C1D\u8BD5\u4FEE\u590D ${row.attempts} \u6B21\uFF0C\u4ECD\u672A\u767B\u5F55`,
...
function proxyRequestId(sessionId, requestId) {
  ...
  return createHash14("sha256").update(sessionId + "\0" + requestId).digest("hex");
}
...
var ProxyWorkflow = class {
  ...
  proof(input, ip, now = Date.now(), independent = false) {
    ...
    const pending2 = db.prepare("SELECT 1 FROM dsh_proxy_calls WHERE ip=? AND state IN ('running','unknown') LIMIT 1").get(ip);
    const chosen = independent ? rows.find((r) => r.role === "reviewer" && r.accepted) : latest;
    const adverse = rows.find((r) => !r.accepted);
    if (!chosen || pending2 || mutation && Date.parse(chosen.checked_at) < Date.parse(mutation.created_at)) return null;
...
${JSON.stringify(current)}`, metadata: { proxy: status } };
  ...
  pending(input) {
    if (!input.task.design?.proxy) return void 0;
    const rows = this.db().prepare("SELECT operation_id,ip FROM dsh_proxy_calls WHERE spec_id=? AND card_id=? AND state='running'").all(input.task.id, input.card.id);
    if (!rows.length) return void 0;
    let local;
    try {
      local = new Database3(join11(process.env.DSH_PROXY_STATE_DIR ?? join11(homedir7(), ".local/state/dsh-proxy"), "proxy.db"), { readonly: true, fileMustExist: true });
      for (const row of rows) {
...
var BrowserPatrolWorkflow = class {
  ...
  plan(input, candidate) {
    ...
    for (const row of candidate) {
      ...
      if (row.action !== "verify") {
        const budget = this.status(input).items.find((r) => r.ip === row.ip && r.instance === row.instance);
        if (input.task.design.browserPatrol?.resumeAfterCopyLimit === 1 && row.action === "resume" && !budget?.canResume) throw Error("\u6B63\u5E38\u7EED\u63A5\u9700\u8981\u65B0\u9C9C\u672A\u767B\u5F55\u8BC1\u636E\u3001\u540C\u6545\u969C\u5DF2\u786E\u8BA4\u5BFC\u5165\u53CA\u5269\u4F59\u4E00\u6B21\u7EED\u63A5\u9884\u7B97\uFF1B\u4E0D\u80FD\u6362\u53F7\u6216\u7ED5\u8FC7\u6311\u6218");
        if (row.action === "provision" && budget?.canResume) throw Error("\u5DF2\u6709\u5BFC\u5165\u4F46\u4ECD\u672A\u767B\u5F55\uFF0C\u5148\u6309\u539F\u8D26\u53F7\u5B89\u6392\u4E00\u6B21\u6B63\u5E38 resume \u8BCA\u65AD\uFF1B\u4E0D\u80FD\u7528\u91CD\u590D\u590D\u5236\u66FF\u4EE3\u7EED\u63A5");
    ...
    if (input.card.round > 1 && items.every((row) => row.action === "verify") && input.task.design.browserPatrol.actions.includes("provision")) {
      const repairable = this.status(input).items.filter((row) => !row.accepted && row.readAuthorized && inventory.nodes.some((n) => n.ip === row.ip && n.reachable) && row.state === "signed_out" && row.loginAuthorized && row.attempts < input.task.design.failurePolicy.maxAttempts && row.preflightFailures < input.task.design.failurePolicy.maxAttempts);
  ...
  status(input, now = Date.now()) {
    ...
    for (const node of nodes) for (const browser of node.browsers) {
      ...
      const issue = db.prepare("SELECT * FROM dsh_browser_issues WHERE spec_id=? AND target_key=? AND (status='open' OR resolved_at>=?) ORDER BY (status='open') DESC,id DESC LIMIT 1").get(input.task.id, key, input.batch.firedAt);
      const notStarted = issue ? db.prepare("SELECT COUNT(*) n FROM dsh_browser_operations o JOIN dsh_browser_operation_outcomes r ON r.operation_id=o.operation_id WHERE o.issue_id=? AND r.mutation='not_started'").get(issue.id).n : 0;
      const preflightFailures = db.prepare("SELECT COUNT(*) n FROM dsh_browser_operations o JOIN dsh_browser_issues i ON i.id=o.issue_id JOIN dsh_browser_operation_outcomes r ON r.operation_id=o.operation_id WHERE o.batch_id=? AND i.target_key=? AND r.mutation='not_started'").get(input.batch.id, key).n;
      const changedThisBatch = db.prepare("SELECT COUNT(*) n FROM dsh_browser_operations o JOIN dsh_browser_issues i ON i.id=o.issue_id WHERE o.batch_id=? AND i.target_key=? AND NOT EXISTS (SELECT 1 FROM dsh_browser_operation_outcomes r WHERE r.operation_id=o.operation_id AND r.mutation='not_started')").get(input.batch.id, key).n;
      ...
      const confirmedImport = issue && db.prepare("SELECT o.operation_id FROM dsh_browser_operations o JOIN dsh_browser_operation_outcomes r ON r.operation_id=o.operation_id WHERE o.issue_id=? AND o.action='login-provision' AND r.mutation='imported' ORDER BY o.created_at DESC LIMIT 1").get(issue.id);
      const canResume = cfg.resumeAfterCopyLimit === 1 && cfg.actions.includes("resume") && resumeAttempts < 1 && !!confirmedImport && proof?.state === "signed_out" && Date.parse(proof.expires_at) > now && (!failure || Date.parse(failure.at) < Date.parse(proof.checked_at));
      ...
      const nextCheckAt = needsStability && first && !stable ? new Date(Date.parse(first.checked_at) + cfg.observationMinutes * 6e4 * Math.min(relevant.length, cfg.minSamples - 1) / (cfg.minSamples - 1)).toISOString() : null;
      items.push({
        ip: node.ip,
        instance: browser.instance,
        ...
        totalAttempts,
        resumeAttempts,
        resumeLimit: cfg.resumeAfterCopyLimit ?? 0,
        canResume,
        repairLimit: input.task.design.failurePolicy.maxAttempts,
        repairExhausted: attempts >= input.task.design.failurePolicy.maxAttempts,
    ...
    const canHandoffForRework = (input.card.round ?? 0) < input.task.design.failurePolicy.maxAttempts && items.some((i) => !i.accepted && i.readAuthorized && inventory.nodes.some((n) => n.ip === i.ip && n.reachable) && i.loginAuthorized && (i.canResume || i.attempts < input.task.design.failurePolicy.maxAttempts) && i.preflightFailures < input.task.design.failurePolicy.maxAttempts && (i.checkedAt && i.state !== "verified" || i.verificationFailure));
    const canCloseUnresolved = items.every((i) => i.accepted || !i.readAuthorized || !inventory.nodes.find((n) => n.ip === i.ip)?.reachable || i.independentlyObserved > 0 && ((input.card.round ?? 0) > input.task.design.failurePolicy.maxAttempts || !i.canResume && i.attempts >= input.task.design.failurePolicy.maxAttempts || i.preflightFailures >= input.task.design.failurePolicy.maxAttempts || i.state === "unknown" && i.independentlyObserved >= 2)) && pendingStability.length === 0 && (items.length > 0 || uncovered.length > 0);
    ...
    return {
      ...
      ready,
      canCloseUnresolved: !ready && canCloseUnresolved,
      plan,
...
var labels = { started: "\u5F00\u59CB\u5DE1\u67E5", findings: "\u5DE1\u67E5\u53D1\u73B0", rework: "\u7EE7\u7EED\u8FD4\u5DE5", restored: "\u72EC\u7ACB\u9A8C\u6536\u901A\u8FC7", unresolved: "\u4ECD\u6709\u672A\u89E3\u51B3\u9879", blocked: "\u5DE1\u67E5\u5904\u7406\u53D7\u963B", completed: "\u4EFB\u52A1\u6267\u884C\u7ED3\u679C" };
function taskNotificationMarkdown(report, taskId, batchId, id) {
...
function patrolNotificationMarkdown(stage, report, taskId, batchId, id) {
  const problems = (report.items ?? []).filter((r) => !r.accepted);
  const lines = [`## \u6D4F\u89C8\u5668\u5DE1\u67E5 \xB7 ${labels[stage]}`, report.summary || report.reason];
...
**${row.ip} / browser-${row.instance}\uFF1A${view.verdict}**`,
  ...
  const expired = (report.items ?? []).filter((r) => r.accepted && r.freshness === "expired");
...
var TaskNotifications = class {
  ...
  job(input) {
    ...
    if (!row?.payload && config.mode === "final-handoff") {
      const upstream = this.store.kernel.db.prepare(`SELECT tr.task_id,tr.summary,tr.ended_at FROM task_links l JOIN task_runs tr ON tr.task_id=l.parent_id
        WHERE l.child_id=? AND tr.status='done' AND tr.outcome='completed' ORDER BY tr.ended_at DESC LIMIT 1`).get(input.card.id);
      if (!upstream?.summary) throw new Error("\u4E0A\u6E38\u5C1A\u672A\u7559\u4E0B\u53EF\u901A\u77E5\u7684\u5B8C\u6210\u4EA4\u63A5");
      const payload = { stage: "completed", report: { title: input.task.title, summary: upstream.summary }, source_card_id: upstream.task_id };
  ...
  async request(input, stage, report) {
    ...
    return {
      state: "queued",
      cardId,
      agentId: input.task.design?.notifications?.agentId,
  ...
  async send(input, stage, report, deliver) {
    ...
    for (const chatId of config.chatIds) {
      const id = createHash15("sha256").update(JSON.stringify([input.batch.id, input.card.round, stage, chatId, ...stage === "blocked" ? [input.card.id] : []])).digest("hex").slice(0, 24);
      const markdown = config.mode === "final-handoff" ? taskNotificationMarkdown(report, input.task.id, input.batch.id, id) : patrolNotificationMarkdown(stage, report, input.task.id, input.batch.id, id);
...
var iso2 = (seconds) => seconds ? new Date(seconds * 1e3).toISOString() : null;
var idOK = (id) => typeof id === "string" && /^[A-Za-z0-9][A-Za-z0-9_-]{0,150}$/.test(id);
function patrolFollowup(db, now = Date.now()) {
  ...
  if (required.some((name2) => !tables.has(name2))) return { schemaVersion: 1, capturedAt: new Date(now).toISOString(), tasks: [], items: [] };
  const tasks = [], items = [];
  for (const saved of db.prepare("SELECT id,spec_json,enabled FROM dsh_task_specs").all()) {
    ...
    tasks.push({
      ...
      state,
      startedAt: iso2(batch.fired_at),
      endedAt: iso2(batch.settled_at),
      notification,
      blockedRoles: blocked.map((c) => ["executor", "reviewer", "planner", "proxy", "notifier"].includes(c.role) ? c.role : "unknown")
    });
    for (const node of parse(inventory?.inventory_json)?.nodes ?? []) for (const browser of node.browsers ?? []) {
      if (!/^(\d{1,3}\.){3}\d{1,3}$/.test(node.ip) || node.ip.split(".").some((v) => +v > 255) || !Number.isInteger(browser.instance) || browser.instance < 1 || browser.instance > 128) continue;
      const key = `${node.ip}:${browser.instance}`;
      ...
      const recorded = parse(snapshot?.payload)?.items?.find((r) => r.ip === node.ip && r.instance === browser.instance && r.checkedAt === proof.checked_at);
      const row = {
        state: proof?.state ?? "unknown",
        accepted: recorded?.accepted === true && issue?.status !== "open",
        ...
        canResume: recorded?.canResume === true && resumeLimit === 1 && resumeAttempts < 1
      };
      const view = patrolItemView(row);
      if (row.accepted) {
        view.verdict = "\u6700\u8FD1\u72EC\u7ACB\u68C0\u67E5\u901A\u8FC7";
        view.reason = "\u6700\u8FD1\u8BB0\u5F55\u5DF2\u6709\u72EC\u7ACB\u9A8C\u6536\u8BC1\u636E\uFF0C\u4E0D\u4EE3\u8868\u5F53\u524D\u4ECD\u7136\u6709\u6548";
      }
      items.push({
        ip: node.ip,
        instance: browser.instance,
        ...
        taskState: state,
        checkedAt: proof?.checked_at ?? null,
        observationBatchId: proof?.batch_id ?? null,
...
var METHODS = [
  ...
  ["agentHistory", 1],
  ["sessionShortcuts", 0],
  ["setSessionShortcut", 1],
  ["sessionCapabilities", 1],
  ["exportConfig", 0],
  ["createConfigBootstrap", 1],
  ["previewConfigImport", 1],
  ["applyConfigImport", 1],
  ["installConfigRuntime", 1],
  ["configRuntimeStatus", 1],
  ["agentActions", 1],
  ["saveAgentActions", 1],
  ["prepareAgentAction", 1],
...
// src/session-shortcuts.ts
function shortcutChange(raw) {
  const q = raw;
...
var SessionShortcuts = class {
  ...
  list(hidden = /* @__PURE__ */ new Set()) {
    return this.db.prepare("SELECT * FROM dsh_session_shortcuts ORDER BY session_id").all().filter((r) => !hidden.has(r.session_id)).map((r) => ({ sessionId: r.session_id, pinned: r.pinned_at > 0, favorite: r.favorite_at > 0, pinnedAt: r.pinned_at, favoriteAt: r.favorite_at }));
  }
  set(raw, eligible, now = Date.now()) {
    const q = shortcutChange(raw);
    if (!eligible.has(q.sessionId)) throw Error("\u4F1A\u8BDD\u4E0D\u5B58\u5728\u3001\u5DF2\u5F52\u6863\u6216\u5C5E\u4E8E\u4EFB\u52A1\u5185\u90E8\u4F1A\u8BDD\uFF0C\u4E0D\u80FD\u8BBE\u7F6E\u5FEB\u6377\u5165\u53E3");
    this.db.transaction(() => {
      const row = this.db.prepare("SELECT * FROM dsh_session_shortcuts WHERE session_id=?").get(q.sessionId);
      const field2 = q.kind === "pinned" ? "pinned_at" : "favorite_at";
      const current = (row?.[field2] ?? 0) > 0;
      ...
      this.db.prepare(`UPDATE dsh_session_shortcuts SET ${field2}=? WHERE session_id=?`).run(q.value ? Math.max(1, now) : 0, q.sessionId);
...
var CONFIG_SCHEMA = "dsh-task-console/config-v1";
var MAX_CONFIG_BYTES = 5 * 1024 * 1024;
var FLEET_RUNTIME = {
...
function payloadDigest(payload) {
  return createHash16("sha256").update(canonical2(payload)).digest("hex");
}
function taskConfig(task, actions) {
  return {
    id: task.id,
    title: task.title,
    brief: task.brief,
    ...
    participants: task.participants,
    ...task.graphMode ? { graphMode: task.graphMode } : {},
    ...task.workflowRecipe ? { workflowRecipe: task.workflowRecipe } : {},
    ...task.design ? { design: task.design } : {},
    cwd: task.cwd,
    timeoutSec: task.timeoutSec,
    onFail: task.onFail,
    maxTries: task.maxTries,
...
function createEnvelope(payload, version, now = /* @__PURE__ */ new Date(), runtime) {
  const normalized = {
...
function text2(value, name2, max = 8e3) {
  if (typeof value !== "string" || !value.trim() || value.length > max) throw Error(`${name2}\u65E0\u6548`);
  return value;
}
function parseEnvelope(raw) {
  ...
  if (!e.payload || !Array.isArray(e.payload.agents) || !Array.isArray(e.payload.tasks) || e.payload.agents.length > 500 || e.payload.tasks.length > 1e3) throw Error("\u914D\u7F6E\u5305\u6E05\u5355\u65E0\u6548\u6216\u8D85\u8FC7\u6570\u91CF\u9650\u5236");
  const agentIds = /* @__PURE__ */ new Set(), taskIds = /* @__PURE__ */ new Set();
  const agents = e.payload.agents.map((row) => {
  ...
  const tasks = e.payload.tasks.map((row) => {
    const id = text2(row?.id, "Task id", 160);
    ...
    if (trigger.kind === "cron" && (!parseCron(String(trigger.expr ?? "")) || trigger.timeZone !== void 0 && !validTimeZone(trigger.timeZone))) throw Error(`Task ${id} \u7684\u5B9A\u65F6\u89C4\u5219\u65E0\u6548`);
    if (!Array.isArray(row.participants) || !row.participants.length || row.participants.length > 32) throw Error(`Task ${id} \u7684\u53C2\u4E0E\u8005\u65E0\u6548`);
    if (row.participants.some((p) => !p || typeof p.agentId !== "string" || !p.agentId.trim() || p.agentId.length > 160 || p.brief !== void 0 && (typeof p.brief !== "string" || p.brief.length > 1e4))) throw Error(`Task ${id} \u7684\u53C2\u4E0E\u8005\u65E0\u6548`);
    if (!Number.isFinite(row.timeoutSec) || row.timeoutSec < 60 || row.timeoutSec > 21600 || !Number.isInteger(row.maxTries) || row.maxTries < 1 || row.maxTries > 5 || !["stop", "retry"].includes(row.onFail)) throw Error(`Task ${id} \u7684\u8FD0\u884C\u7B56\u7565\u65E0\u6548`);
    return taskConfig({
      id,
      title: text2(row.title, "Task \u6807\u9898", 300),
      brief: text2(row.brief, "Task \u4EFB\u52A1\u4E66", 5e4),
      trigger,
      participants: row.participants,
      graphMode: row.graphMode,
      workflowRecipe: row.workflowRecipe,
      design: row.design,
      cwd: text2(row.cwd, "Task \u5DE5\u4F5C\u76EE\u5F55", 4096),
  ...
  if (e.runtime !== void 0) {
    ...
    const mcps = e.runtime.mcps.map((row) => ({ serverName: text2(row?.serverName, "MCP \u540D\u79F0", 120), transport: row?.transport === "stdio" || row?.transport === "streamable-http" ? row.transport : (() => {
      throw Error("MCP transport \u65E0\u6548");
    })(), credentialRef: row?.credentialRef === "fleet-admin" || row?.credentialRef === "onboard-vault-resolve" ? row.credentialRef : (() => {
      throw Error("MCP \u51ED\u636E\u5F15\u7528\u65E0\u6548");
    })(), runtime: text2(row?.runtime, "MCP runtime", 160) }));
    const skills = e.runtime.skills.map((row) => ({ id: text2(row?.id, "Skill \u540D\u79F0", 120), runtime: row?.runtime === "bootstrap-bundle" ? row.runtime : (() => {
      throw Error("Skill runtime \u65E0\u6548");
    })() }));
    let bootstrap;
    if (e.runtime.bootstrap !== void 0) {
  ...
  return { schema: CONFIG_SCHEMA, exportedAt: new Date(e.exportedAt).toISOString(), source: { plugin: "dsh-task-console", version: text2(e.source.version, "\u7248\u672C", 80) }, digest: e.digest, payload, ...runtime ? { runtime } : {} };
}
...
function assertPublicConfigUrl(value, publicDomain) {
  let url, base;
  try {
    url = new URL(value);
    base = new URL(publicDomain);
  } catch {
    throw Error("R2 \u5730\u5740\u65E0\u6548");
  }
...
async function uploadConfig(envelope, config, fixedName) {
  if (!config.token) throw Error("\u5BBF\u4E3B\u672A\u914D\u7F6E\u914D\u7F6E\u5BFC\u51FA\u6240\u9700\u7684 R2 \u51ED\u636E");
  const data = encodeEnvelope(envelope), name2 = fixedName ?? `dsh-config-${Date.now()}-${randomUUID9().slice(0, 8)}.json`;
  if (!/^dsh-config-[A-Za-z0-9._-]+\.json$/.test(name2)) throw Error("R2 \u914D\u7F6E\u6587\u4EF6\u540D\u65E0\u6548");
  const form = new FormData();
  form.set("file", new Blob([data], { type: "application/json" }), name2);
  form.set("domain", config.domain);
  form.set("name", name2);
  form.set("path", "dsh-task-console/config-exports");
  const response = await fetch(config.endpoint, { method: "POST", headers: { Authorization: `Bearer ${config.token}` }, body: form, signal: AbortSignal.timeout(6e4) });
  if (!response.ok) throw Error(`R2 \u4E0A\u4F20\u5931\u8D25\uFF08HTTP ${response.status}\uFF09`);
  const body2 = await response.text();
  let publicUrl = "";
  try {
    const parsed = JSON.parse(body2);
    publicUrl = String(parsed.url ?? parsed.image_url ?? parsed.data?.url ?? parsed.result?.url ?? "");
  } catch {
    publicUrl = body2.trim();
  }
  if (!publicUrl.startsWith(`${config.domain.replace(/\/$/, "")}/`)) publicUrl = `${config.domain.replace(/\/$/, "")}/dsh-task-console/config-exports/${name2}`;
  assertPublicConfigUrl(publicUrl, config.domain);
  const check = await fetch(`${publicUrl}?verify=${createHash16("sha256").update(data).digest("hex").slice(0, 16)}`, { redirect: "error", signal: AbortSignal.timeout(3e4), cache: "no-store" });
  ...
  return { publicUrl, bytes: data.byteLength, sha256: createHash16("sha256").update(data).digest("hex") };
}
async function downloadConfig(value, publicDomain) {
  const url = assertPublicConfigUrl(value, publicDomain);
  const response = await fetch(url, { redirect: "error", signal: AbortSignal.timeout(3e4), headers: { accept: "application/json" } });
  ...
  try {
    ...
    throw Error("\u914D\u7F6E\u5305\u4E0D\u662F\u6709\u6548 JSON");
  }
  return { envelope: parseEnvelope(parsed), bytes: data.byteLength, fileSha256: createHash16("sha256").update(data).digest("hex") };
}
async function createBootstrapCommand(value, config) {
  const url = assertPublicConfigUrl(value, config.domain);
  if (!config.token) throw Error("\u5BBF\u4E3B\u672A\u914D\u7F6E\u65B0\u673A\u5668\u5F15\u5BFC\u6388\u6743");
  const response = await fetch(config.endpoint, {
    method: "POST",
    headers: { Authorization: `Bearer ${config.token}`, "content-type": "application/json" },
    body: JSON.stringify({ configUrl: url.toString() }),
    redirect: "error",
  ...
  if (!response.ok || !body2?.ok || typeof body2.command !== "string" || !/^[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+$/.test(body2.bootstrapToken ?? "")) throw Error(body2?.error || `\u65B0\u673A\u5668\u5F15\u5BFC\u547D\u4EE4\u751F\u6210\u5931\u8D25\uFF08HTTP ${response.status}\uFF09`);
  if (!/^bash <\(curl -fsSL https:\/\/skill\.vyibc\.com\/dsh-config-bootstrap\/release\/install-dsh-config-bootstrap\.sh\) --bootstrap-token \S+ --config-url /.test(body2.command)) throw Error("\u65B0\u673A\u5668\u5F15\u5BFC\u547D\u4EE4\u683C\u5F0F\u65E0\u6548");
  return { command: body2.command, expiresInSeconds: Number(body2.expiresInSeconds) || 0, bootstrapToken: body2.bootstrapToken };
}

// src/config-runtime-bootstrap.ts
import { createHash as createHash17 } from "node:crypto";
import { spawn as spawn2 } from "node:child_process";
import { closeSync, openSync } from "node:fs";
import { chmod as chmod3, mkdir as mkdir6, readFile as readFile8, writeFile as writeFile5 } from "node:fs/promises";
import { homedir as homedir8 } from "node:os";
import { join as join12 } from "node:path";
var INSTALLER = "https://skill.vyibc.com/dsh-config-bootstrap/release/install-dsh-config-bootstrap.sh";
var ROOT = join12(homedir8(), ".config", "dsh-config-bootstrap", "imports");
function shell(value) {
  return `'${value.replaceAll("'", "'\\''")}'`;
}
function jobPath(jobId) {
  if (!/^[a-f0-9]{24}$/.test(jobId)) throw Error("\u8FD0\u884C\u65F6\u5B89\u88C5\u4EFB\u52A1\u7F16\u53F7\u65E0\u6548");
  return join12(ROOT, jobId);
}
async function startRuntimeBootstrap(configUrl, token) {
  if (!/^[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+$/.test(token)) throw Error("\u914D\u7F6E\u5305\u672A\u643A\u5E26\u6709\u6548\u7684\u8FD0\u884C\u65F6\u5F15\u5BFC\u6388\u6743");
  const jobId = createHash17("sha256").update(`${configUrl}\0${Date.now()}\0${Math.random()}`).digest("hex").slice(0, 24);
  const dir = jobPath(jobId), status = join12(dir, "status.json"), installer = join12(dir, "installer.sh"), tokenFile = join12(dir, "token");
  await mkdir6(dir, { recursive: true, mode: 448 });
  const response = await fetch(INSTALLER, { redirect: "error", signal: AbortSignal.timeout(3e4) });
  ...
  await writeFile5(installer, source, { mode: 448 });
  await chmod3(installer, 448);
  await writeFile5(tokenFile, `${token}
`, { mode: 384 });
  await writeFile5(status, JSON.stringify({ jobId, state: "running" }), { mode: 384 });
  const runner = join12(dir, "run.sh");
  const failedStatus = join12(dir, "failed.json"), completeStatus = join12(dir, "complete.json");
  await writeFile5(failedStatus, JSON.stringify({ jobId, state: "failed", message: "\u8FD0\u884C\u65F6\u5B89\u88C5\u6216\u914D\u7F6E\u5BFC\u5165\u5931\u8D25\uFF0C\u8BF7\u67E5\u770B\u4EFB\u52A1\u65E5\u5FD7" }), { mode: 384 });
  await writeFile5(completeStatus, JSON.stringify({ jobId, state: "complete" }), { mode: 384 });
  await writeFile5(runner, `#!/usr/bin/env bash
set -euo pipefail
status=${shell(status)}
...
rm -f "$token_file"
`, { mode: 448 });
  const logFd = openSync(join12(dir, "install.log"), "a", 384);
  const child = spawn2("/bin/bash", [runner], { detached: true, stdio: ["ignore", logFd, logFd] });
  closeSync(logFd);
  child.unref();
  return { jobId, state: "running" };
}
async function runtimeBootstrapStatus(jobId) {
  let parsed;
  try {
    parsed = JSON.parse(await readFile8(join12(jobPath(jobId), "status.json"), "utf8"));
  } catch {
    throw Error("\u8FD0\u884C\u65F6\u5B89\u88C5\u4EFB\u52A1\u4E0D\u5B58\u5728");
  }
...
var TaskConsoleService = class extends TypertRemoteService {
  ...
  headerCache;
  headerRead;
  pendingConfigImports = /* @__PURE__ */ new Map();
  shortcutHidden() {
    const registry = this.ctx.get("workspaceRegistry");
    if (!registry) throw Error("\u4F1A\u8BDD\u53EF\u89C1\u6027\u670D\u52A1\u5C1A\u672A\u5C31\u7EEA");
    return /* @__PURE__ */ new Set([...registry.archivedSessionIds ?? [], ...registry.internalSessionIds ?? []]);
  }
  async sessionShortcuts() {
    await this.ready;
    return JSON.stringify(new SessionShortcuts(this.runner.store.kernel.db).list(this.shortcutHidden()));
  }
  async setSessionShortcut(payload) {
...
\u4F01\u5FAE\uFF1A${notifications.length ? notifications.map((n) => n.state).join("\u3001") : "\u672C\u4EFB\u52A1\u672A\u914D\u7F6E\u901A\u77E5"}`, metadata: { ...report2.metadata, notifications } };
      ...
      notify: async (input, stage, deliver) => {
        const outbox = new TaskNotifications(this.runner.store);
        if (input.card.role === "notifier" || input.profileId === input.task.design?.notifications?.agentId) return outbox.send(input, stage, void 0, deliver);
        const report = (await this.patrolWorkflow(input)).snapshot(input);
    ...
    this.ready = this.runner.start().then(() => {
      this.capabilities = new SessionCapabilities(ctx, async () => ({
        checkedAt: (/* @__PURE__ */ new Date()).toISOString(),
        scope: "environment-directory-not-execution-grant",
    ...
    }).then(() => this.markExistingTaskSessionsInternal()).then(() => this.intake.start());
  ...
  async scopedMcp(raw, args, exec, invoke) {
    ...
    if (/^browser_login_(copy|provision|resume)$/.test(raw) && batch.turn?.action) {
      const resumed = raw === "browser_login_resume" ? await readBrowserAcceptance(args.operationId) : void 0;
  ...
  async intakeAgents() {
    ...
    for (const preset of await presets.list()) {
      ...
      const spec = await readSpec(dirname5(String(preset.path)));
      ...
      rows.push({
        ...
        profileHash: createHash18("sha256").update(JSON.stringify(spec)).digest("hex"),
  ...
  async agentPage(payload) {
    const q = JSON.parse(payload);
    if (!q || typeof q !== "object" || Array.isArray(q) || q.query !== void 0 && typeof q.query !== "string" || q.page !== void 0 && (!Number.isSafeInteger(q.page) || q.page < 1)) throw Error("\u65E0\u6548 Agent \u5206\u9875");
    const presets = this.ctx.get("agentPresets"), all = presets ? await presets.list() : [];
    const created = new Map(await Promise.all(all.map(async (p) => [p.id, await readAgentCreatedAt(dirname5(String(p.path)))])));
    ...
    const load = async (p, detail = false) => {
      const dir = dirname5(String(p.path)), spec = p.trust === "user" ? await readSpec(dir) : null;
      return {
        id: p.id,
        name: spec?.name ?? p.name ?? p.id,
        description: spec?.description ?? p.description ?? "",
        trust: p.trust,
        broken: p.broken,
        path: dir,
        createdAt: created.get(p.id),
        firstUsedAt: null,
        permission: spec ? spec.tools.some((t) => ["bash", "fs", "str-replace-editor"].includes(t)) ? "write" : Object.values(spec.mcpTools).some((t) => t.length) ? "limited-write" : "read-only" : null,
        spec: detail ? spec : null
      };
    };
    const detailId = q.id === "new" ? void 0 : q.id ?? selected[0]?.id, detailPreset = detailId ? all.find((p) => p.id === detailId) : void 0;
  ...
  async agents() {
    ...
    for (const p of await presets.list()) {
      const dir = dirname5(String(p.path));
      const spec = p.trust === "user" ? await readSpec(dir) : null;
      let name2 = p.name ?? p.id, description = p.description ?? "";
      if (!p.name || !p.description) {
        try {
          const text3 = await (await import("node:fs/promises")).readFile(`${dir}/preset.yml`, "utf8");
          const n = /^name:\s*(.*)$/m.exec(text3)?.[1];
          const d = /^description:\s*(.*)$/m.exec(text3)?.[1];
      ...
      rows.push({ id: p.id, name: name2, description, trust: p.trust, broken: p.broken, path: dir, spec, actionCount, createdAt: await readAgentCreatedAt(dir), firstUsedAt: firstUsed.get(p.id) ?? null });
    }
  ...
  async exportConfig() {
    ...
    const presets = this.ctx.get("agentPresets");
    const agents = [];
    for (const preset of presets ? await presets.list() : []) {
      if (preset.trust !== "user") continue;
      const dir = dirname5(String(preset.path)), spec = await readSpec(dir);
      if (!spec) continue;
    ...
    let version = "unknown";
    try {
      version = JSON.parse(await (await import("node:fs/promises")).readFile(new URL("../package.json", import.meta.url), "utf8")).version ?? version;
    } catch {
    }
  ...
  async createConfigBootstrap(payload) {
    await this.ready;
  ...
  configImportView(envelope) {
    const existingAgents = /* @__PURE__ */ new Set(), existingTasks = new Set(this.runner.store.tasks.keys());
    const presets = this.ctx.get("agentPresets");
    const skills = /* @__PURE__ */ new Set(), mcp = new Set(this.hostMcp().map((row) => row.serverName));
    return Promise.all([presets ? presets.list() : [], scanSkills()]).then(([rows, skillRows]) => {
      for (const row of rows) existingAgents.add(String(row.id));
      for (const row of skillRows) skills.add(row.name);
      const agents = envelope.payload.agents.map((row) => {
        const missingSkills = row.spec.skills.filter((name2) => !skills.has(name2));
        const missingMcp = Object.keys(row.spec.mcpTools).filter((name2) => !mcp.has(name2));
      ...
      const runtime = envelope.runtime ? {
        missingMcp: envelope.runtime.mcps.map((row) => row.serverName).filter((name2) => !mcp.has(name2)),
        missingSkills: envelope.runtime.skills.map((row) => row.id).filter((name2) => !skills.has(name2)),
  ...
  async previewConfigImport(payload) {
    await this.ready;
    const { url } = JSON.parse(payload);
    if (!url) throw Error("\u8BF7\u8F93\u5165 R2 \u914D\u7F6E\u5730\u5740");
    const downloaded = await downloadConfig(url, this.configR2().domain);
    const importId = randomUUID10();
    const now = Date.now();
    for (const [id, row] of this.pendingConfigImports) if (row.expiresAt <= now) this.pendingConfigImports.delete(id);
    this.pendingConfigImports.set(importId, { envelope: downloaded.envelope, expiresAt: now + 10 * 6e4 });
    return JSON.stringify({ importId, expiresAt: new Date(now + 10 * 6e4).toISOString(), bytes: downloaded.bytes, fileSha256: downloaded.fileSha256, ...await this.configImportView(downloaded.envelope) });
  }
  async applyConfigImport(payload) {
    await this.ready;
    const { importId } = JSON.parse(payload);
    const pending2 = importId ? this.pendingConfigImports.get(importId) : void 0;
    if (!pending2 || pending2.expiresAt <= Date.now()) throw Error("\u5BFC\u5165\u9884\u89C8\u5DF2\u8FC7\u671F\uFF0C\u8BF7\u91CD\u65B0\u6821\u9A8C R2 \u5730\u5740");
    this.pendingConfigImports.delete(importId);
    const view = await this.configImportView(pending2.envelope);
    const importedAgents = [], skippedAgents = [];
    const importedTasks = [], skippedTasks = [];
    const library = await scanSkills(), hostMcp = this.hostMcp(), hostTools = this.hostToolNames();
    for (const row of pending2.envelope.payload.agents) {
    ...
    const availableAgents = new Set(importedAgents);
    for (const row of pending2.envelope.payload.tasks) {
      if (this.runner.store.tasks.has(row.id)) {
        skippedTasks.push({ id: row.id, reason: "\u540C ID Task \u5DF2\u5B58\u5728" });
        continue;
      }
      const missing = row.participants.map((p) => p.agentId).filter((id) => !availableAgents.has(id));
      if (missing.length) {
        skippedTasks.push({ id: row.id, reason: `\u7F3A\u5C11 Agent:${missing.join("\u3001")}` });
        continue;
      }
      const task = { ...row, actions: void 0, enabled: false, createdAt: (/* @__PURE__ */ new Date()).toISOString() };
      delete task.actions;
  ...
  async installConfigRuntime(payload) {
    await this.ready;
    const { url } = JSON.parse(payload);
    if (!url) throw Error("\u8BF7\u8F93\u5165 R2 \u914D\u7F6E\u5730\u5740");
  ...
  async configRuntimeStatus(payload) {
  ...
  async agentActions(payload) {
    ...
    if (!agentId) return JSON.stringify({ agentId: null, name: "", revision: "", actions: [], writable: false });
    const presets = this.ctx.get("agentPresets");
    const preset = (await presets?.list() ?? []).find((p) => p.id === agentId && !p.broken);
    if (!preset) throw new Error("Agent \u4E0D\u5B58\u5728\u6216\u4E0D\u53EF\u7528");
    const dir = dirname5(String(preset.path));
    const catalog = await readActions(dir);
    const spec = await readSpec(dir);
    return JSON.stringify({ ...catalog, agentId, name: spec?.name ?? preset.name ?? agentId, writable: preset.trust === "user" && presets.authorable !== false && resolve7(dir) === resolve7(userPresetRoot(), agentId) });
  }
  async saveAgentActions(payload) {
    const query = JSON.parse(payload);
    const catalog = JSON.parse(await this.agentActions(JSON.stringify({ agentId: query.agentId })));
    if (!catalog.agentId || !catalog.writable) throw new Error("\u8FD9\u4E2A Agent \u7684 Actions \u4E0D\u53EF\u5199");
    await saveActions(resolve7(userPresetRoot(), catalog.agentId), query.actions, query.revision);
  ...
  async agentActionOptions(payload) {
    const query = JSON.parse(payload);
    await this.ready;
    const catalog = query.taskId ? this.creator.actions.read(query.taskId) : JSON.parse(await this.agentActions(payload));
    ...
    const values = Object.fromEntries((parameter.dependsOn ?? []).map((k) => [k, typeof raw[k] === "string" ? raw[k].slice(0, 4e3) : ""]));
    ...
    const presets = (await this.ctx.get("agentPresets").list()).filter((p) => participantIds.includes(p.id));
    const specs = await Promise.all(presets.map((p) => readSpec(dirname5(String(p.path)))));
    if (!specs.some((spec) => (spec?.mcpTools?.["fleet-browser"] ?? []).some((t) => t === "*" || t === tool))) throw Error("\u5F53\u524D\u89D2\u8272\u6CA1\u6709\u6B64\u53EA\u8BFB\u5019\u9009\u80FD\u529B\uFF1B\u672A\u6269\u5927\u5DE5\u5177\u6743\u9650");
    if (!host) throw Error("\u5019\u9009 MCP \u5C1A\u672A\u5C31\u7EEA\uFF0C\u8BF7\u7A0D\u540E\u91CD\u8BD5\uFF1B\u6CA1\u6709\u6267\u884C\u4EFB\u4F55\u76EE\u6807\u64CD\u4F5C");
    try {
      const result = await fleetActionOptions(host.config, parameter, values, void 0, Boolean(catalog.taskId && this.creator.actions.task(catalog.taskId).workflowRecipe?.id === "fleet-base-v2"));
      const latest = catalog.taskId ? this.creator.actions.read(catalog.taskId) : JSON.parse(await this.agentActions(payload));
  ...
  async prepareAgentAction(payload) {
    const query = JSON.parse(payload);
    const catalog = JSON.parse(await this.agentActions(payload));
  ...
  async tryRun(payload) {
    ...
    try {
      handle = await this.ctx.agents.create({
        sessionId,
        ...selection ? { agentOptions: selection } : {},
        meta: { cwd: homedir9(), agentPreset: preset.id },
        setup: async (agentCtx) => {
          await presets.mount(agentCtx, preset.id);
        }
      });
  ...
  async startAgentSession(payload) {
    const { agentId, text: text3, cwd } = JSON.parse(payload);
    const presets = this.ctx.get("agentPresets");
    if (!presets) throw new Error("\u8FD9\u4E2A\u90E8\u7F72\u6CA1\u6709 preset \u670D\u52A1");
    ...
    const dir = cwd && cwd.trim() ? cwd.trim() : workspaces[0]?.path ?? homedir9();
    ...
    if (text3 && text3.trim()) handle.agent.followup({ id: randomUUID10(), role: "user", content: [{ type: "text", text: text3.trim() }], source: { kind: "user" } });
    return JSON.stringify({ sessionId, agentPreset: preset.id, name: name2 });
  }
  // ── turn ledger ────────────────────────────────────────────────────────
  async sessionCapabilities(payload) {
  ...
  async reviewTaskPlan(payload) {
    await this.ready;
    const { id, hash: hash3, decision, reason } = JSON.parse(payload);
    return JSON.stringify(await this.creator.review(id, hash3, decision, reason));
  }
  async launchWorkflow(payload) {
    await this.ready;
    const { taskId, text: text3, requestId, cwd } = JSON.parse(payload);
    return JSON.stringify(await this.creator.launch(taskId, text3, requestId, cwd));
  }
  async taskActions(payload) {
    await this.ready;
    return JSON.stringify(this.creator.actions.read(JSON.parse(payload).taskId));
  }
  async saveTaskActions(payload) {
    await this.ready;
    const { taskId, actions, revision } = JSON.parse(payload);
    return JSON.stringify(this.creator.actions.save(taskId, actions, revision));
  }
  async launchTaskAction(payload) {
    await this.ready;
   ...
   */
  taskPageCache = /* @__PURE__ */ new Map();
  async taskPage(payload) {
    ...
    if (cached?.stamp === stamp && Date.now() - cached.at < 2e3) return cached.value;
    const presets = this.ctx.get("agentPresets");
    const labels2 = new Map((presets ? await presets.list() : []).map((p) => [p.id, p.name ?? p.id]));
    const index = taskListIndex(store, input, labels2);
    const rows = index.rows.map(({ task, batch, state, history }) => {
      ...
      const cards = batch.cardIds.map((id2) => st.cards.get(id2)).filter(Boolean);
      const legs = cards.map((c) => {
        const r = cardRun(st, c);
        return {
          agentId: c.kind === "gate" ? "\u7CFB\u7EDF\u95F8\u95E8" : c.agentId,
          status: c.status === "ready" || c.status === "scheduled" ? "queued" : c.status,
          tries: c.runIds.length,
          question: c.wakeAt ? `\u5B9A\u65F6\u7B49\u5F85\uFF0C${c.wakeAt} \u81EA\u52A8\u7EE7\u7EED\uFF1A${r?.question ?? ""}` : r?.status === "blocked" ? r.question : void 0,
          error: c.error?.slice(0, 400)
        };
      });
      ...
      return { task: projected, state, history, latest: {
        id: batch.id,
        taskId: id,
        firedAt: batch.firedAt,
        by: batch.by,
        ...
        ...result ? { resultArtifact: this.artifactView(result) } : {},
        rounds,
        reworks: Math.max(0, rounds - 1)
      } };
    });
    const used = new Set(rows.flatMap((r) => r.task.participants.map((p) => p.agentId)));
    const value = JSON.stringify({ ...index, rows, agents: [...used].map((id) => ({ id, name: labels2.get(id) ?? id })) });
  ...
  async createTask(payload) {
    ...
    for (const p of rows) {
      const spec = p.trust === "user" ? await readSpec(dirname5(String(p.path))) : null;
...
async function handleTaskSignalHttp(req, res, service, token = process.env.DSH_TASK_INTAKE_TOKEN ?? "") {
  ...
  try {
    if (url.pathname === "/dsh-task-console/api/patrol-followup") {
      if (req.method !== "GET") {
        reply(res, 405, { ok: false, error: "read only" });
        return;
      }
      reply(res, 200, { ok: true, ...await service.patrolFollowup() });
      return;
    }
    if (req.method === "POST") {
...
var Config = z2.object({
  standardMaxSteps: z2.natural().min(1).default(24),
  standardMcpInheritance: z2.union(["inherit", "discover-only"]).default("inherit"),
  standardSkillInheritance: z2.union(["inherit", "discover-only"]).default("inherit"),
  standardExcludedSkills: z2.array(z2.string()).default([]),
  standardExcludedMcpServers: z2.array(z2.string()).default([]),
  standardExcludedTools: z2.array(z2.string()).default([])
});
async function apply(ctx, config = {}) {
  await ctx.plugin(TaskConsoleService);
  await ctx.get("taskConsole").ready;
  ctx.get("taskConsole").capabilities.policy = config;
  ctx.effect(() => ctx.get("taskConsole").capabilities.install(), "task-console: session capability facts and progress guard");
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        const body2 = await readFile9(new URL("./client-heavy.js", import.meta.url));
