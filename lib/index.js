...
function validateTaskSignal(raw) {
  ...
  const objective = String(goalInput.objective ?? "").trim().slice(0, 12e3);
  ...
  if (input.incident !== void 0) {
    ...
    incident = {
      ...
      ...oneLine(value.summary, 1e3) ? { summary: oneLine(value.summary, 1e3) } : {}
    };
  }
  ...
  const facts = (Array.isArray(input.facts) ? input.facts : []).slice(0, 64).map((rawFact, index) => {
    ...
    return { name: name2, value: typeof candidate === "string" ? oneLine(candidate, 1e3) : candidate };
  });
  return {
    schemaVersion: 1,
