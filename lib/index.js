...
var TaskIntakeCoordinator = class {
  ...
  workspace() {
    return this.options.workspace ?? process.env.DSH_TASK_INTAKE_WORKSPACE ?? join6(homedir4(), ".dsh", "task-console", "intake-workspace");
  }
  async start() {
...
async function decideTaskSignalWithAgent(ctx, signal, context, options = {}) {
  ...
  const digest2 = createHash7("sha256").update(signal.id).digest("hex").slice(0, 12);
...
import { createHash as createHash8, createHmac, timingSafeEqual } from "node:crypto";
import { constants as fsConstants } from "node:fs";
import { access, chmod as chmod2, lstat as lstat2, mkdtemp, open, readFile as readFile7, realpath as realpath4, rm as rm3, stat as stat5, writeFile as writeFile3 } from "node:fs/promises";
...
// src/task-create.ts
var digest = (value) => createHash9("sha256").update(JSON.stringify(value)).digest("hex");
...
${p.input.text}`,
  ...
  async dispatch(raw, input, exec, cwd, directWorkflow = false, stageOnly = false) {
    ...
    try {
      ...
      if (leases.length) {
        const root = join9(store.root, "private-inputs");
        await mkdir5(root, { recursive: true, mode: 448 });
        await writeFile4(join9(root, `${batchId}.json`), JSON.stringify({
...
function collectBrowserEvidence(input, events) {
  ...
  for (const server of ["fleet-browser", `fleet-browser-${input.profileId}`]) for (const raw of ["browser_fleet_inventory", "browser_inspect", "browser_status"]) {
    ...
    names.set(full.length <= 64 ? full : `${full.slice(0, 51)}_${createHash10("sha256").update(`${server}\0${raw}`).digest("hex").slice(0, 12)}`, raw);
  }
...
function proxyRequestId(sessionId, requestId) {
  ...
  return createHash11("sha256").update(sessionId + "\0" + requestId).digest("hex");
}
...
${JSON.stringify(current)}`, metadata: { proxy: status } };
  ...
  pending(input) {
    ...
    let local;
    try {
      local = new Database3(join10(process.env.DSH_PROXY_STATE_DIR ?? join10(homedir6(), ".local/state/dsh-proxy"), "proxy.db"), { readonly: true, fileMustExist: true });
      for (const row of rows) {
...
var TaskNotifications = class {
  ...
  async send(input, stage, report, deliver) {
    ...
    for (const chatId of config.chatIds) {
      const id = createHash12("sha256").update(JSON.stringify([input.batch.id, input.card.round, stage, chatId])).digest("hex").slice(0, 24);
      const markdown = [
...
async function browserJobs(deps) {
  return (deps.jobs || (async () => {
    const root = process.env.FLEET_BROWSER_STATE_DIR || join11(homedir7(), ".local/state/fleet-browser-manager");
    let names;
    try {
...
async function readBrowserAcceptance(id) {
  ...
  let file;
  try {
    file = await open2(join11(root, id + ".json"), constants.O_RDONLY | constants.O_NOFOLLOW);
...
async function validateWorkflowCompletion(input, deps = {}) {
  ...
  for (const id of ids) {
    ...
    const expected = createHash13("sha256").update(JSON.stringify([sessionId, job?.args?.requestId])).digest("hex").slice(0, 32);
