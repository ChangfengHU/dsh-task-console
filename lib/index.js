...
async function captureArtifacts(ctx, requested) {
  ...
  for (const raw of [...new Set(requested.map((x) => String(x).trim()).filter(Boolean))]) {
    ...
    let original;
    try {
      original = await realpath(candidate);
    } catch {
...
async function discoverLegacyArtifacts(task, runs, knownOriginals) {
  let workspace;
  try {
    workspace = await realpath(task.cwd);
  } catch {
    return [];
  }
  ...
  for (const { run, path } of found.values()) {
    try {
      ...
      out.push({
        ...
        name: safePart(basename(original)),
...
// src/presets.ts
import { cp, mkdir as mkdir2, readFile as readFile2, readdir, rm, stat as stat2, writeFile, chmod } from "node:fs/promises";
  ...
  for (const { root, label } of skillRoots(home)) {
    ...
    for (const name2 of names.sort()) {
      ...
      try {
        ...
        const text = await readFile2(join2(dir, "SKILL.md"), "utf8");
...
`, { mode: 384 });
  ...
  if (spec.skills.length) {
    ...
    for (const name2 of spec.skills) {
      const entry = library.find((s) => s.name === name2) ?? library.find((s) => basename2(s.dir) === name2);
      if (!entry) continue;
      await cp(entry.dir, join2(skillsDir, basename2(entry.dir)), { recursive: true, dereference: true });
    }
  }
...
async function readSpec(dir) {
  try {
    return validateSpec(JSON.parse(await readFile2(join2(dir, SPEC_FILE), "utf8")));
  } catch {
    return null;
  }
}
...
var EventStore = class {
  ...
  async load() {
    ...
    let text = "";
    try {
      text = await readFile3(this.file, "utf8");
    } catch {
    }
...
var TaskRunner = class {
  ...
  async preflight(task) {
    ...
    try {
      ...
      if (!(await stat3(task.cwd)).isDirectory()) return `\u5DE5\u4F5C\u76EE\u5F55\u4E0D\u5B58\u5728:${task.cwd}`;
    } catch {
  ...
  async reviewCard(cardId, decision, note = "") {
...
var TaskConsoleService = class extends TypertRemoteService {
  ...
  async reviewCard(payload) {
    const { cardId, decision, note } = JSON.parse(payload);
