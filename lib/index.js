  ...
  async function walk(dir, prefix) {
    ...
    for (const entry of entries) {
      ...
      if (info.isDirectory()) {
        ...
        digest.update(`f\0${relative3}\0${body.length}\0`);
        digest.update(body);
      }
    }
  }
...
async function syncPresetSkills(spec, library, dir) {
  ...
  const staged = join2(dir, `.skills-next-${randomUUID2()}`);
  const stagedLock = join2(dir, `.skills-lock-next-${randomUUID2()}`);
  const backup = join2(dir, `.skills-backup-${randomUUID2()}`);
  const backupLock = join2(dir, `.skills-lock-backup-${randomUUID2()}`);
...
async function writePreset(spec, hostMcp, library, root = userPresetRoot(), inheritedTools = []) {
  ...
  const staged = resolve3(root, `.${spec.id}-next-${randomUUID2()}`);
  const backup = resolve3(root, `.${spec.id}-backup-${randomUUID2()}`);
  let backedUp = false;
  await mkdir2(staged, { recursive: true, mode: 448 });
  try {
...
var HermesKernel = class {
  ...
  constructor(path, options = {}) {
    ...
    this.claimer = options.claimer ?? (() => `${hostname()}:${process.pid}:${randomUUID3()}`);
...
${cardMessage(task, card, batch.id, upstream)}`;
  ...
  async onTurnEnd(f, reason) {
    ...
    if ((run?.nudges ?? 0) < 1) {
      ...
      f.handle.agent.followup({ id: randomUUID4(), role: "user", content: [{ type: "text", text: NUDGE }], source: { kind: "user" } });
      return;
    }
...
var TaskConsoleService = class extends TypertRemoteService {
  ...
  constructor(ctx) {
    ...
    void this.runner.start().then(() => this.markExistingTaskSessionsInternal()).catch((err) => console.error("[task-console] runner failed to start:", err));
  }
  ...
  async startAgentSession(payload) {
    ...
    if (text && text.trim()) handle.agent.followup({ id: randomUUID5(), role: "user", content: [{ type: "text", text: text.trim() }], source: { kind: "user" } });
...
async function apply(ctx) {
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        const body = await readFile5(new URL("./client-heavy.js", import.meta.url));
