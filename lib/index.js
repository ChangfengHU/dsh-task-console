...
var TaskRunner = class {
  ...
  async startRun(task, batch, card) {
    ...
    const flight = { runId, cardId: card.id, taskId: task.id, sessionId, messageId, consumed: false, handle: void 0, lastText: "" };
    ...
    try {
      ...
      flight.timer = setTimeout(() => {
        void this.finish(flight, "run/timed_out", "timed_out", `${task.timeoutSec} \u79D2\u6CA1\u4EA4\u5377`);
      }, task.timeoutSec * 1e3);
      flight.timer.unref?.();
    } catch (error) {
