...
var BrowserPatrolWorkflow = class {
  ...
  status(input, now = Date.now()) {
    ...
    const canHandoffForRework = (input.card.round ?? 0) < input.task.design.failurePolicy.maxAttempts && items.some((i) => !i.accepted && i.readAuthorized && inventory.nodes.some((n) => n.ip === i.ip && n.reachable) && i.loginAuthorized && i.attempts < input.task.design.failurePolicy.maxAttempts && i.preflightFailures < input.task.design.failurePolicy.maxAttempts && (i.checkedAt && i.state !== "verified" || i.verificationFailure));
    const canCloseUnresolved = items.every((i) => i.accepted || !i.readAuthorized || !inventory.nodes.find((n) => n.ip === i.ip)?.reachable || i.independentlyObserved > 0 && ((input.card.round ?? 0) > input.task.design.failurePolicy.maxAttempts || i.attempts >= input.task.design.failurePolicy.maxAttempts || i.preflightFailures >= input.task.design.failurePolicy.maxAttempts || i.state === "unknown" && i.independentlyObserved >= 2)) && pendingStability.length === 0 && (items.length > 0 || uncovered.length > 0);
...
var TaskNotifications = class {
  ...
  async send(input, stage, report, deliver) {
    ...
    for (const chatId of config.chatIds) {
      const id = createHash14("sha256").update(JSON.stringify([input.batch.id, input.card.round, stage, chatId])).digest("hex").slice(0, 24);
      const markdown = [
