...
// src/service.ts
import { randomUUID as randomUUID9, createHash as createHash15 } from "node:crypto";
...
import { copyFile, mkdir, readFile as readFile3, realpath as realpath2, stat as stat2 } from "node:fs/promises";
import { basename as basename3, extname, isAbsolute as isAbsolute3, join as join3, relative as relative2, resolve as resolve3, sep as sep2 } from "node:path";

// src/public-upload.ts
import { homedir } from "node:os";
import { readFile as readFile2, realpath, stat } from "node:fs/promises";
import { basename as basename2, isAbsolute as isAbsolute2, relative, resolve as resolve2, sep } from "node:path";
...
async function readPublishableHtml(filePath) {
  ...
  const file = await realpath(resolve2(raw));
  const configured = (process.env.DSH_TASK_CONSOLE_PUBLISH_ROOTS ?? homedir()).split(":").map((value) => value.trim()).filter(Boolean);
  const roots = await Promise.all(configured.map((root) => realpath(resolve2(root))));
  ...
  return { path: file, name: basename2(file), data: await readFile2(file) };
}
...
async function captureArtifacts(ctx, requested) {
  ...
  for (const raw of [...new Set(requested.map((x) => String(x).trim()).filter(Boolean))]) {
    const candidate = resolve3(ctx.task.cwd, raw);
    let original;
    try {
    ...
    checked.push({ original, name: safePart(basename3(original)), size: info.size, sha: await sha256(original) });
  }
  const dir = join3(ctx.root, "artifacts", safePart(ctx.task.id), safePart(ctx.batchId), safePart(ctx.runId));
  await mkdir(dir, { recursive: true, mode: 448 });
  const out = [];
  for (const file of checked) {
    const id = `a-${randomUUID2()}`;
    const storagePath = join3(dir, `${id.slice(2, 10)}-${file.name}`);
...
async function discoverLegacyArtifacts(task, runs, knownOriginals) {
  ...
  for (const { run, path } of found.values()) {
    try {
      ...
      out.push({
        id: `legacy-${createHash2("sha256").update(`${run.id}\0${original}`).digest("hex").slice(0, 24)}`,
        taskId: task.id,
        batchId: run.batchId,
        cardId: run.cardId,
        runId: run.id,
        sessionId: run.sessionId,
        name: safePart(basename3(original)),
...
async function readArtifact(root, task, artifact) {
  ...
  const allowed = artifact.legacy ? await realpath2(task.cwd) : await realpath2(join3(root, "artifacts"));
...
import { cp, mkdir as mkdir2, readFile as readFile4, readdir, rename as rename2, rm as rm2, stat as stat3, writeFile as writeFile2, chmod } from "node:fs/promises";
import { homedir as homedir2 } from "node:os";
import { basename as basename4, join as join4, resolve as resolve4 } from "node:path";
...
function publicToolName(serverName, rawName) {
  ...
  const hash3 = createHash3("sha256").update(`${serverName}\0${rawName}`).digest("hex").slice(0, 12);
...
function userPresetRoot(home = homedir2()) {
  return join4(process.env.DSH_HOME ?? join4(home, ".dsh"), ".agent-presets");
}
function skillRoots(home = homedir2()) {
  return [
    { root: join4(process.env.DSH_HOME ?? join4(home, ".dsh"), "skills"), label: "user-dsh" },
    { root: join4(process.env.DSH_AGENTS_HOME ?? join4(home, ".agents"), "skills"), label: "user-agents" }
  ];
}
...
async function scanSkills(home = homedir2()) {
  ...
  for (const { root, label } of skillRoots(home)) {
    ...
    for (const name2 of names.sort()) {
      ...
      const dir = join4(root, name2);
      try {
        if (!(await stat3(dir)).isDirectory()) continue;
        const text2 = await readFile4(join4(dir, "SKILL.md"), "utf8");
...
function selectedSkill(specName, library) {
  return library.find((skill) => skill.name === specName) ?? library.find((skill) => basename4(skill.dir) === specName);
}
  ...
  async function walk(dir, prefix) {
    ...
    for (const entry of entries) {
      ...
      if (info.isDirectory()) {
        ...
        const body2 = await readFile4(path);
...
async function verifyPresetSkills(spec, library, dir) {
  const lock = await readFile4(join4(dir, SKILL_LOCK_FILE), "utf8").then(parseSkillLock).catch(() => null);
  ...
  for (const name2 of spec.skills) {
    ...
    const copied = join4(dir, "skills", source ? basename4(source.dir) : name2);
...
async function syncPresetSkills(spec, library, dir) {
  ...
  const skillsDir = join4(dir, "skills");
  const lockPath = join4(dir, SKILL_LOCK_FILE);
  const staged = join4(dir, `.skills-next-${randomUUID4()}`);
  const stagedLock = join4(dir, `.skills-lock-next-${randomUUID4()}`);
  const backup = join4(dir, `.skills-backup-${randomUUID4()}`);
  const backupLock = join4(dir, `.skills-lock-backup-${randomUUID4()}`);
  ...
  try {
    for (const { name: name2, entry } of sources) {
      const target = join4(staged, basename4(entry.dir));
      await cp(entry.dir, target, {
        recursive: true,
        dereference: true,
        filter: (source) => managedSkillEntry(basename4(source))
      });
      ...
      lock.skills.push({ name: name2, sourceRoot: entry.root, sourceBasename: basename4(entry.dir), sha256: sourceSha256 });
    }
...
async function writePreset(spec, hostMcp, library, root = userPresetRoot(), inheritedTools = []) {
  return withPresetLock(resolve4(root, spec.id), () => writePresetLocked(spec, hostMcp, library, root, inheritedTools));
}
async function writePresetLocked(spec, hostMcp, library, root, inheritedTools) {
  const dir = resolve4(root, spec.id);
  if (!dir.startsWith(resolve4(root) + "/")) throw new Error("\u975E\u6CD5 id");
  ...
  const staged = resolve4(root, `.${spec.id}-next-${randomUUID4()}`);
  const backup = resolve4(root, `.${spec.id}-backup-${randomUUID4()}`);
  let backedUp = false;
  await mkdir2(staged, { recursive: true, mode: 448 });
  try {
    await writeFile2(join4(staged, "agent.cordis.yml"), preview2.yml, { mode: 384 });
    await writeFile2(join4(staged, "preset.yml"), `name: ${JSON.stringify(spec.name)}
description: ${JSON.stringify(spec.description)}
`, { mode: 384 });
    await writeFile2(join4(staged, SPEC_FILE), JSON.stringify(spec, null, 2) + "\n", { mode: 384 });
    await writeFile2(join4(staged, "agent-meta.json"), JSON.stringify({ createdAt }) + "\n", { mode: 384 });
    await syncPresetSkills(spec, library, staged);
    if (existed) {
      try {
        await writeFile2(join4(staged, ACTION_FILE), await readFile4(join4(dir, ACTION_FILE)), { mode: 384 });
      } catch (error) {
...
async function readSpec(dir) {
  try {
    return validateSpec(JSON.parse(await readFile4(join4(dir, SPEC_FILE), "utf8")));
  } catch {
    return null;
  }
}
async function readAgentCreatedAt(dir) {
  try {
    const { createdAt } = JSON.parse(await readFile4(join4(dir, "agent-meta.json"), "utf8"));
  ...
  claim(task, now) {
    ...
    return this.store.kernel.write(() => {
      ...
      if (!row && state.next_at !== null && state.next_at <= now) {
        ...
        const id = createHash5("sha256").update(`${task.id}:${due}`).digest("hex").slice(0, 24);
...
async function validateWorkflowCompletion(input, deps = {}) {
  ...
  for (const id of ids) {
    ...
    const expected = createHash6("sha256").update(JSON.stringify([sessionId, job?.args?.requestId])).digest("hex").slice(0, 32);
...
${JSON.stringify(plan.items)}`;
          ...
          finalize: async (summary, artifactPath, disposition = "passed") => {
            ...
            if (artifactPath) {
              let originalPath;
              try {
                originalPath = await realpath3(resolve5(task.cwd, artifactPath));
              } catch {
...
async function decideTaskSignalWithAgent(ctx, signal, context, options = {}) {
  ...
  const digest2 = createHash8("sha256").update(signal.id).digest("hex").slice(0, 12);
...
import { createHash as createHash9, createHmac, timingSafeEqual } from "node:crypto";
...
// src/task-create.ts
var digest = (value) => createHash11("sha256").update(JSON.stringify(value)).digest("hex");
function userInput(exec) {
  ...
  return { text: text2, sessionId, requestId: digest([sessionId, last.id ?? users.length, text2]) };
}
...
${p.input.text}`,
  ...
  async launchAction(query) {
    ...
    const pending2 = this.queue.then(async () => {
      ...
      const hash3 = digest([query.taskId, query.actionId, query.revision, query.values, query.cwd]), requestId = digest(["task-action", query.requestId]), batchId = `b-chat-${requestId.slice(0, 20)}`;
  ...
  async dispatch(raw, input, exec, cwd, directWorkflow = false, stageOnly = false, actionInput) {
    ...
    try {
      ...
      const hash3 = digest({ proposal, text: scrub(input.text), cwd, ...actionInput ? { action: actionInput.snapshot } : {} });
      ...
      if (stageOnly) {
        const planId = `P-chat-${digest([input.requestId, hash3]).slice(0, 20)}`, db2 = this.plansDb();
...
function collectBrowserEvidence(input, events) {
  ...
  for (const server of ["fleet-browser", `fleet-browser-${input.profileId}`]) for (const raw of ["browser_fleet_inventory", "browser_inspect", "browser_status"]) {
    ...
    names.set(full.length <= 64 ? full : `${full.slice(0, 51)}_${createHash12("sha256").update(`${server}\0${raw}`).digest("hex").slice(0, 12)}`, raw);
  }
...
function proxyRequestId(sessionId, requestId) {
  ...
  return createHash13("sha256").update(sessionId + "\0" + requestId).digest("hex");
}
...
var TaskNotifications = class {
  ...
  async send(input, stage, report, deliver) {
    ...
    for (const chatId of config.chatIds) {
      const id = createHash14("sha256").update(JSON.stringify([input.batch.id, input.card.round, stage, chatId, ...stage === "blocked" ? [input.card.id] : []])).digest("hex").slice(0, 24);
...
\u4F01\u5FAE\uFF1A${notifications.length ? notifications.map((n) => n.state).join("\u3001") : "\u672C\u4EFB\u52A1\u672A\u914D\u7F6E\u901A\u77E5"}`, metadata: { ...report2.metadata, notifications } };
  ...
  async intakeAgents() {
    ...
    for (const preset of await presets.list()) {
      ...
      rows.push({
        ...
        profileHash: createHash15("sha256").update(JSON.stringify(spec)).digest("hex"),
  ...
  async agentActions(payload) {
    ...
    return JSON.stringify({ ...catalog, agentId, name: spec?.name ?? preset.name ?? agentId, writable: preset.trust === "user" && presets.authorable !== false && resolve6(dir) === resolve6(userPresetRoot(), agentId) });
  }
  async saveAgentActions(payload) {
    ...
    await saveActions(resolve6(userPresetRoot(), catalog.agentId), query.actions, query.revision);
