...
import type { TasksApi, ConfigBootstrapResult, ConfigExportResult, ConfigImportPreview, ConfigImportResult, ConfigRuntimeJob } from './TasksView.tsx'
import { go } from './Console.tsx'

export function ConfigMigration({ api, toast }: { api: TasksApi; toast: (text: string) => void }) {
  const [url, setUrl] = useState('')
  const [busy, setBusy] = useState<'export' | 'bootstrap' | 'preview' | 'apply' | 'runtime' | ''>('')
  ...
  const [runtimeJob, setRuntimeJob] = useState<ConfigRuntimeJob | null>(null)
  ...
  const installRuntime = () => void run('runtime', () => api.installConfigRuntime(url.trim()), value => { setRuntimeJob(value); toast('已开始安装依赖；DSH 重启后会自动完成导入') })
  ...
  useEffect(() => {
    if (!runtimeJob || runtimeJob.state !== 'running') return
    const timer = window.setInterval(() => { void api.configRuntimeStatus(runtimeJob.jobId).then(value => {
      setRuntimeJob(value)
      if (value.state === 'complete') { toast('依赖、Agent 与 Task 已全部导入'); void api.previewConfigImport(url.trim()).then(setPreview).catch(() => {}) }
      if (value.state === 'failed') setError(value.message ?? '运行时安装失败')
    }).catch(() => { /* DSH restart is expected while the detached installer continues. */ }) }, 2500)
    return () => window.clearInterval(timer)
  }, [api, runtimeJob?.jobId, runtimeJob?.state, url, toast])
    ...
    <div className="dtc-migration-grid">
      ...
      <section className="dtc-migration-card">
        ...
        {preview ? <div className="dtc-migration-preview"><b>导入预览</b><p>{preview.counts.agents} Agents · {preview.counts.tasks} Tasks · 来源版本 {preview.sourceVersion}</p>
          {preview.runtime && (preview.runtime.missingMcp.length || preview.runtime.missingSkills.length) ? <div className="dtc-migration-runtime"><b>新机器缺少运行依赖</b><p>{preview.runtime.missingMcp.length} MCP · {preview.runtime.missingSkills.length} Skills</p>{preview.runtime.bootstrapAvailable ? <button className="dtc-btn pri" disabled={!!busy || runtimeJob?.state === 'running'} onClick={installRuntime}>{runtimeJob?.state === 'running' ? '安装并导入中，DSH 将重启…' : '一键安装依赖并自动导入'}</button> : <small>引导授权已过期，请在源机器重新导出配置包。</small>}</div> : null}
          ...
          {preview.runtime && (preview.runtime.missingMcp.length || preview.runtime.missingSkills.length) ? null : <button className="dtc-btn pri" disabled={!!busy} onClick={apply}>{busy === 'apply' ? '导入中…' : '确认导入可用配置'}</button>}</div> : null}
