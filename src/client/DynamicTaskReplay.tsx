...
function DbDag({ frame, selected, current, onSelect, nameOf }: { frame: GraphFrame; selected: string | null; current?: GraphEventRow; onSelect: (id: string) => void; nameOf: (id: string | null) => string }) {
  ...
  const width = ordered.length > 4 ? 1180 : Math.max(700, ordered.length * 226 + 50)
...
export function DynamicTaskReplay({ api, agents, task, batches, batchId, sessionId, toast }: { api: TasksApi; agents: AgentRow[]; task: TaskSpec; batches: Batch[]; batchId: string; sessionId?: string; toast: (text: string) => void }) {
  ...
  return <div className="dtc-cartoon dtc-dbtruth">
    <header className="dtc-cartoon-head"><button className="dtc-cartoon-back" onClick={() => go('tasks')}>←</button><div className="dtc-cartoon-logo">▦</div><div className="dtc-cartoon-title"><span>数据库回放</span><h1>{task.title}</h1><small>{batchId} · tasks / task_links / task_runs / task_events</small></div><div className="dtc-cartoon-live"><i />{cursor === null ? 'SQLite 实时态' : `历史事件 #${step}`}</div><button className="dtc-btn dtc-session-trigger" onClick={() => setSessionsOpen(true)}><i>{sessionCount}</i> Sessions</button>{workflowDone && !final ? <button className="dtc-btn sm" onClick={() => document.getElementById("dtc-final-delivery")?.scrollIntoView({ behavior: "smooth" })}>查看执行报告 ↓</button> : <FinalArtifactActions compact api={api} taskId={task.id} batchId={batchId} group={final} toast={toast} refresh={refreshArtifacts} />}<div className="dtc-cartoon-actions">{batches.length > 1 ? <select value={batchId} onChange={e => go(`tasks/${task.id}/runs/${e.target.value}`)}>{batches.map(batch => <option key={batch.id} value={batch.id}>{batch.id}</option>)}</select> : null}<TaskRunAction task={task} api={api} toast={toast} />{!data.batch.outcome ? <button className="dtc-btn" onClick={async () => { await api.cancelRun(batchId); toast('已取消运行') }}>取消</button> : null}<button className="dtc-btn danger" onClick={async () => { if (!window.confirm('删除任务和它的运行记录?会话本身不删。')) return; await api.deleteTask(task.id); go('tasks') }}>删除</button></div></header>
    {error ? <div className="dtc-err">{error}</div> : null}
    {origin ? <details className="dtc-intake-origin" open={task.origin?.source === "task-chat" ? undefined : true}><summary>任务来源与本次参数</summary><div className="dtc-intake-origin-head"><span>Task Agent</span><b>{origin.decision === 'reuse' ? '复用已有 Task，创建新 Turn' : '创建新 Task 与首个 Turn'}</b>{origin.intakeSessionId ? <button className="dtc-btn sm" onClick={() => void api.openSession(origin.intakeSessionId!)}>查看路由会话 ↗</button> : null}</div><p>{origin.reason || '已通过受限 Task Agent 完成路由决策。'}</p><dl><div><dt>Source</dt><dd>{origin.source}</dd></div><div><dt>Signal</dt><dd className="dtc-mono">{origin.signalId}</dd></div>{origin.incidentId ? <div><dt>Incident</dt><dd className="dtc-mono">{origin.incidentId}</dd></div> : null}<div><dt>本轮目标</dt><dd>{objective}</dd></div>{turn?.targets?.length ? <div><dt>Targets（仅元数据）</dt><dd>{turn.targets.map(target => <code key={`${target.kind}:${target.id}`}>{target.kind}:{target.label || target.id}</code>)}</dd></div> : null}</dl></details> : null}
