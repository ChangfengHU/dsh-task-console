...
function SessionDrawer({ api, frame, taskId, batchId, selectedSessionId, nameOf, onSelectNode, onClose }: { api: TasksApi; frame: GraphFrame; taskId: string; batchId: string; selectedSessionId?: string; nameOf: (id: string | null) => string; onSelectNode: (id: string) => void; onClose: () => void }) {
  ...
  return <div className="dtc-session-layer"><button className="dtc-session-scrim" aria-label="关闭 Sessions" onClick={onClose} /><aside className="dtc-session-drawer"><header><div><b>{selectedSessionId ? 'Session Trace' : 'Related Sessions'}</b><small>{selectedSessionId ? 'LLM · Skills · MCP · Tools 运行审计' : `任务关联的 ${rows.length} 个真实 DSH 会话`}</small></div><button className="dtc-close" onClick={onClose}>×</button></header><div className="dtc-session-drawer-body">
    {selectedSessionId ? <><button className="dtc-session-back" onClick={back}>← 返回 {rows.length} Sessions</button><SessionTrace api={api} sessionId={selectedSessionId} onOpen={() => void api.openSession(selectedSessionId)} /></> : rows.length ? <><div className="dtc-session-note">会话只是任务证据的辅助索引，不改变 DAG 和任务页面布局。</div><div className="dtc-session-rounds">{[...rounds.entries()].sort(([a], [b]) => a - b).map(([round, group]) => <div className="dtc-session-round" key={round}><b>{round === maxRound && group.every(run => taskById.get(run.task_id)?.role === 'planner') ? 'FINALIZE' : `ROUND ${String(round || 1).padStart(2, '0')}`}</b>{group.map(run => {
...
export function DynamicTaskReplay({ api, agents, task, batches, batchId, sessionId, toast }: { api: TasksApi; agents: AgentRow[]; task: TaskSpec; batches: Batch[]; batchId: string; sessionId?: string; toast: (text: string) => void }) {
  ...
  const sessionCount = data.live.runs.filter(run => run.session_id).length
  ...
  return <div className="dtc-cartoon dtc-dbtruth">
    <header className="dtc-cartoon-head"><button className="dtc-cartoon-back" onClick={() => go('tasks')}>←</button><div className="dtc-cartoon-logo">▦</div><div className="dtc-cartoon-title"><span>数据库回放</span><h1>{task.title}</h1><small>{batchId} · tasks / task_links / task_runs / task_events</small></div><div className="dtc-cartoon-live"><i />{cursor === null ? 'SQLite 实时态' : `历史事件 #${step}`}</div><button className="dtc-btn dtc-session-trigger" onClick={() => setSessionsOpen(true)}><i>{sessionCount}</i> Sessions</button><FinalArtifactActions compact api={api} taskId={task.id} batchId={batchId} group={final} toast={toast} refresh={refreshArtifacts} /><div className="dtc-cartoon-actions">{batches.length > 1 ? <select value={batchId} onChange={e => go(`tasks/${task.id}/runs/${e.target.value}`)}>{batches.map(batch => <option key={batch.id} value={batch.id}>{batch.id}</option>)}</select> : null}<button className="dtc-btn pri" disabled={Boolean(turn?.origin?.signalId || task.origin?.signalId)} title="外部任务请从来源系统重新提交，由 Task Agent 核对目标与角色" onClick={async () => { const next = await api.fireTask(task.id); toast('已触发'); go(`tasks/${task.id}/runs/${next.runId}`) }}>{turn?.origin?.signalId || task.origin?.signalId ? '从来源重试' : '▶ 再跑一次'}</button>{!data.batch.outcome ? <button className="dtc-btn" onClick={async () => { await api.cancelRun(batchId); toast('已取消运行') }}>取消</button> : null}<button className="dtc-btn danger" onClick={async () => { if (!window.confirm('删除任务和它的运行记录?会话本身不删。')) return; await api.deleteTask(task.id); go('tasks') }}>删除</button></div></header>
    {error ? <div className="dtc-err">{error}</div> : null}
    {origin ? <section className="dtc-intake-origin"><div className="dtc-intake-origin-head"><span>Task Agent</span><b>{origin.decision === 'reuse' ? '复用已有 Task，创建新 Turn' : '创建新 Task 与首个 Turn'}</b>{origin.intakeSessionId ? <button className="dtc-btn sm" onClick={() => void api.openSession(origin.intakeSessionId!)}>查看路由会话 ↗</button> : null}</div><p>{origin.reason || '已通过受限 Task Agent 完成路由决策。'}</p><dl><div><dt>Source</dt><dd>{origin.source}</dd></div><div><dt>Signal</dt><dd className="dtc-mono">{origin.signalId}</dd></div>{origin.incidentId ? <div><dt>Incident</dt><dd className="dtc-mono">{origin.incidentId}</dd></div> : null}<div><dt>本轮目标</dt><dd>{objective}</dd></div>{turn?.targets?.length ? <div><dt>Targets（仅元数据）</dt><dd>{turn.targets.map(target => <code key={`${target.kind}:${target.id}`}>{target.kind}:{target.label || target.id}</code>)}</dd></div> : null}</dl></section> : null}
    ...
    <details className="dtc-cpanel dtc-cartoon-details"><summary>任务书与运行边界</summary><div className="dtc-hand">{objective}</div><div className="dtc-kv"><span className="k">工作区</span><span className="dtc-mono">{turn?.cwd ?? task.cwd}</span><span className="k">超时</span><span>{Math.round(task.timeoutSec / 60)} 分钟 / 角色</span><span className="k">失败策略</span><span>{task.onFail === 'retry' ? `最多重试 ${task.maxTries} 次` : '失败后停止'}</span></div></details>
    {sessionsOpen ? <SessionDrawer api={api} frame={data.live} taskId={task.id} batchId={batchId} selectedSessionId={sessionId} nameOf={nameOf} onSelectNode={setSelected} onClose={closeSessions} /> : null}
  </div>
}
