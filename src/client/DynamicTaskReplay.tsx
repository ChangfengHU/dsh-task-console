...
function DbDag({ frame, selected, onSelect, nameOf }: { frame: GraphFrame; selected: string | null; onSelect: (id: string) => void; nameOf: (id: string | null) => string }) {
  const width = Math.max(760, frame.tasks.length * 226 + 48)
  const height = 250
  const positions = new Map(frame.tasks.map((task, index) => [task.id, { x: 28 + index * 226, y: task.node_kind === 'gate' ? 91 : 75 }]))
  return <div className="dtc-dbdag-scroll"><div className="dtc-dbdag" style={{ width, height }}>
    <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`} aria-label={`${frame.links.length} 条数据库依赖`}>
      ...
      {frame.links.map(link => { const a = positions.get(link.parent_id); const b = positions.get(link.child_id); if (!a || !b) return null; const ay = a.y + 48; const by = b.y + 48; return <g key={`${link.parent_id}>${link.child_id}`}><path className="dtc-dbdag-edge" d={`M${a.x + 178} ${ay} C${a.x + 199} ${ay},${b.x - 21} ${by},${b.x} ${by}`} markerEnd="url(#db-arrow)" /><text x={(a.x + b.x + 178) / 2} y={Math.min(ay, by) - 9}>depends</text></g> })}
    </svg>
    {frame.tasks.map(task => { const pos = positions.get(task.id)!; const role = ROLE[task.role ?? task.node_kind] ?? { label: task.role ?? '角色', icon: '●' }; const runs = frame.runs.filter(run => run.task_id === task.id); return <button key={task.id} className={`dtc-dbnode k-${task.node_kind} s-${task.status} ${selected === task.id ? 'selected' : ''}`} style={{ left: pos.x, top: pos.y }} onClick={() => onSelect(task.id)}>
...
export function DynamicTaskReplay({ api, agents, task, batches, batchId, toast }: { api: TasksApi; agents: AgentRow[]; task: TaskSpec; batches: Batch[]; batchId: string; toast: (text: string) => void }) {
  ...
  const rounds = Math.max(0, ...frame.tasks.map(row => row.round ?? 0))
