...
function DbDag({ frame, selected, current, onSelect, nameOf }: { frame: GraphFrame; selected: string | null; current?: GraphEventRow; onSelect: (id: string) => void; nameOf: (id: string | null) => string }) {
  ...
  return <div className="dtc-dbdag-scroll"><div className="dtc-dbdag" style={{ width, height }}>
    ...
    {ordered.map(task => { const pos = positions.get(task.id)!; const role = ROLE[task.role ?? task.node_kind] ?? { label: task.role ?? '角色', icon: '●' }; const runs = frame.runs.filter(run => run.task_id === task.id); const run = runs.at(-1); const focused = current?.task_id === task.id; return <button key={task.id} className={`dtc-dbnode k-${task.node_kind} s-${task.status} ${selected === task.id ? 'selected' : ''} ${focused ? `event-focus ev-${current.kind}` : ''}`} style={{ left: pos.x, top: pos.y }} onClick={() => onSelect(task.id)}>
      <i>{role.icon}</i><span><b>{role.label}{task.round ? ` ${task.round}` : ''}</b><small>{task.node_kind === 'gate' ? 'System task · no Agent Run' : nameOf(task.assignee)}</small><em>{run ? `Run #${run.id} · ${PHASES.find(phase => phase.id === run.phase)?.label}` : '尚未生成 Agent Run'}</em>{run ? <span className="dtc-run-phases" aria-label={`Run #${run.id} 运行阶段`}>{PHASES.map(phase => <i key={phase.id} className={`${run.evidence.includes(phase.id) ? 'reached' : ''} ${run.phase === phase.id ? 'current' : ''}`} title={phase.label}>{phase.short}</i>)}</span> : null}</span><strong>{STATUS[task.status] ?? task.status}</strong>
    </button> })}
  </div></div>
}
...
export function DynamicTaskReplay({ api, agents, task, batches, batchId, toast }: { api: TasksApi; agents: AgentRow[]; task: TaskSpec; batches: Batch[]; batchId: string; toast: (text: string) => void }) {
  ...
  useEffect(() => {
    let stop = false
    const load = () => Promise.all([api.taskGraph(task.id, batchId), api.taskArtifacts(task.id, batchId)]).then(([next, nextArtifacts]) => { if (!stop) { setData(next); setArtifacts(nextArtifacts); setError(''); setSelected(cur => cur && next.live.tasks.some(row => row.id === cur) ? cur : next.live.tasks.find(row => ['running', 'blocked', 'ready'].includes(row.status))?.id ?? next.live.tasks.at(-1)?.id ?? null) } }).catch(e => { if (!stop) setError(String((e as Error).message ?? e)) })
