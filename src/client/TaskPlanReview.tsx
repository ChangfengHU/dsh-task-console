...
import type { WorkflowRecipe } from '../workflow-recipes.ts'
...
export function TaskDesignView({ design, recipe }: { design?: TaskDesign; recipe?: WorkflowRecipe }) {
  if (!design) return null
  const evidenceContract = recipe?.id === 'fleet-base-v3' ? 'fleet-base-v3 · 三角色原始证据与 Fleet 独立回读'
    : recipe?.id === 'fleet-base-v2' && recipe.login === 'provision-gemini' ? 'fleet-base-v2 · 双浏览器20分钟独立登录验收' : design.evidenceContract
  return <section aria-label="条件与验收设计">
    ...
    {evidenceContract ? <p>宿主证据闸门：{evidenceContract}（依据实际工具事件核对交卷，不接受模型自报统计）</p> : <p className="dtc-workflow-muted">未选择专用宿主证据闸门；结构化计划本身不保证执行结果正确。</p>}
...
export function TaskPlanReview({ api, id }: { api: Api; id?: string }) {
  ...
  return <section className="dtc-workflow" aria-label="任务计划审查">
    ...
    <div className="dtc-workflow-body">
      ...
      {plan ? <><div className="dtc-workflow-meta"><span>{plan.state === 'awaiting_trial' && plan.decision === 'revise' && plan.definition.trigger?.kind !== 'cron' ? '已更新 · 仍暂停 · 待手动验收' : states[plan.state] || plan.state}</span><span>指纹 {plan.hash.slice(0, 12)}</span></div><h2>{plan.definition.title}</h2>
        ...
        <h3>原始目标</h3><pre>{plan.request}</pre><p>{plan.definition.brief}</p><TaskDesignView design={plan.definition.design} recipe={plan.definition.workflowRecipe} />
        ...
        {['pending', 'approved'].includes(plan.state) ? <section><h3>独立审查</h3><p>请核对范围、权限、条件、失败处理和验收标准。批准仅执行本计划，不增加节点或工具授权。</p>
          ...
          <div className="dtc-workflow-actions"><button className="dtc-btn" disabled={busy || !reason.trim() || plan.state !== 'pending'} onClick={() => review('reject')}>退回，不执行</button><button className="dtc-btn pri" disabled={busy || !checked || !reason.trim()} onClick={() => review('approve')}>{busy ? '提交中…' : plan.decision === 'revise' ? '批准更新，保持暂停' : plan.definition.trigger?.kind === 'cron' ? '批准计划，等待手动验收' : '批准并执行'}</button></div>
        </section> : null}
        ...
        {plan.taskId && !plan.batchId ? <p><button className="dtc-btn" onClick={() => go(`tasks/${plan.taskId}`)}>查看任务与执行记录 ↗</button></p> : null}
      </> : null}
    </div>
  </section>
}
