...
const ACTOR = { dispatcher: { label: '宿主', cls: 'dtc-p-grey' }, agent: { label: 'Agent', cls: 'dtc-p-acc' }, person: { label: '人', cls: 'dtc-p-warn' }, clock: { label: '时钟', cls: 'dtc-p-park' } } as const
...
export function TaskReplay({ api, agents, id, runId, toast }: { api: TasksApi & LedgerApi; agents: AgentRow[]; id: string; runId?: string; toast: (m: string) => void }) {
  ...
  const stepTo = (n: number) => { setPlaying(false); const v = Math.max(1, Math.min(n, events.length)); setCursor(v >= events.length ? null : v); setDrawer(true) }
  ...
  return (
    <>
      <div className="dtc-crumb"><a onClick={() => go('tasks')}>任务</a><span>/</span><span className="dtc-mono">{task.id}</span></div>
      ...
      <div className="dtc-sub">{batchFull ? `${({ cron: '到点', manual: '手动', retry: '重试' })[batchFull.by]}触发 · ${cardsFull.length} 张卡${total ? ` · 共 ${total}` : ''}${asks ? ` · ${asks} 次停车等人` : ''}` : '还没跑过'}</div>
      ...
      <div className="dtc-two">
        <div>
          ...
          <div className="dtc-panel">
            <h3>看板状态 <span className="dtc-faint" style={{ fontWeight: 400 }}>由事件流重放得出{stopAt ? <> · <span className="dtc-chip stale">停在第 {upto} 条 · {fmt(stopAt)}</span></> : ' · 现在'}</span></h3>
            {cardsNow.length ? <>
                ...
                {cardsNow.map((c, i) => {
                  ...
                  return (
                    <span key={c.id} style={{ display: 'contents' }}>
                      {up ? <div className={`dtc-link ${open ? 'open' : ''}`}><span className="lbl">{open ? '交接单已交' : '按住'}</span><div className="bar" /><span className="lbl">{open ? '放行' : '等上游'}</span></div> : null}
                      <div className={`dtc-node s-${c.status}`}>
                        <div className="role"><div className="av">{agentName(c.agentId)[0]}</div><div><div className="nm">{agentName(c.agentId)}</div><div className="id dtc-mono">第 {c.index + 1} 张 · {c.agentId}</div></div></div>
              ...
              <div className="dtc-chain-cap"><span className="dtc-chip">{cardsFull.filter(c => c.runIds.length).length} 个真实会话</span><span>紫边 = 上游没完成,按住下游;绿边 = 交接单已交,放行</span></div>
            </> : <div className="dtc-empty">这个时刻还没有运行。</div>}
          </div>

          {/* ── run ledger ── */}
          {batchFull ? <div className="dtc-panel"><h3>运行台账 <span className="dtc-faint" style={{ fontWeight: 400 }}>谁在干活 · 每张卡拿到什么、交出什么;点开看交接单和回合账本</span></h3>
                ...
                return (
                  <div key={c.id} className={`dtc-leg ${isOpen ? 'open' : ''}`}>
                    <div className="head" onClick={() => setOpenCard(isOpen ? null : c.id)}>
                      ...
                      <div className="who">{agentName(c.agentId)}<small className="dtc-mono">{r?.sessionId ?? '—'}</small></div>
                      ...
                      <div className="acts">{r?.sessionId ? <button className="dtc-btn sm" onClick={e => { e.stopPropagation(); closeConsole(); void api.openSession(r.sessionId) }}>打开会话</button> : null}</div>
                    </div>
        ...
        <div>
          ...
          <div className="dtc-panel"><h3>角色 <span className="dtc-faint" style={{ fontWeight: 400 }}>每张卡带自己的 preset</span></h3>
      ...
      <div className="dtc-evbar" onClick={() => setDrawer(d => !d)}>
        <b>事件流</b><span className="dtc-muted">{events.length} 条 · 只追加</span>
        {last ? <span className="dtc-muted">最新 {fmt(last.at)} · {describe(last, full, agentName)}</span> : null}
        <span className="sp" /><span className="dtc-faint">{drawer ? '收起 ▾' : '▴ 展开看回放'}</span>
      </div>
      {drawer ? <div className="dtc-drawer">
        <div className="dh"><b>事件流</b><span className="dtc-muted">只追加 · 真实时刻 · 谁动的手</span><span className="sp" /><span className="dtc-muted">{upto} / {events.length}</span><button className="dtc-btn sm" onClick={() => setDrawer(false)}>收起 ▾</button></div>
        <div className="dbody">
          {events.map((e, i) => { const a = ACTOR[actorOf(e)]; return (
            <div key={i} className={`dtc-ev ${i < upto ? '' : 'off'} ${i === upto - 1 ? 'cur' : ''}`} onClick={() => stepTo(i + 1)}>
              ...
              <span className={`dtc-pill ${a.cls}`}>{a.label}</span><span className="dtc-mono t">{e.t}</span><span className="d">{describe(e, full, agentName)}</span>
            </div>) })}
        </div>
        <div className="dtc-scrub">
          ...
          <span className="dtc-faint" style={{ fontSize: 12 }}>上面的看板跟着走</span>
        </div>
      </div> : null}
    </>
  )
