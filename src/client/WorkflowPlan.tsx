...
export function WorkflowPlan({ task, batch, nameOf, openSession, trace, expanded: controlledExpanded, onExpandedChange }: {
  ...
  if (!view.origin && !plan.design?.studioStages) return null
  return <section className="dtc-workflow" aria-label="创建与编排">
    <header><div><b>创建与编排</b><small>{!view.origin ? '任务书已保存 · 运行时建立真实阶段与依赖' : view.directReuse ? '本次直接复用工作流 · 未再次调用创建 Agent' : view.origin?.decision === 'reuse' ? 'Agent 选择复用工作流 · 新执行记录' : 'Agent 已生成工作流 · 执行由下方 DAG 跟踪'}</small></div>
    ...
    {expanded ? <div className="dtc-workflow-body">
      ...
      <div className="dtc-workflow-meta"><span>{view.planId ? `计划指纹 ${view.planId.slice(0, 12)}` : batch ? '历史记录 · 未保存独立计划快照' : (view.origin ? '已审查工作流 · 尚未执行' : '已保存任务书 · 尚未执行')}</span><span>{plan.graphMode === 'dynamic-rounds' ? '动态返工协议' : '角色交接 / static-chain'}</span>{view.origin?.reviewPlanId ? <a href={`#/tc/tasks/plans/${view.origin?.reviewPlanId}`}>查看放行记录</a> : null}{plan.workflowRecipe ? <span>预制配方 {plan.workflowRecipe.id} · {plan.workflowRecipe.login === 'provision-gemini' ? '验证并补齐 Gemini 登录' : '保持登录现状'}</span> : null}{view.sessions.map(s => <button key={s.id} className="dtc-btn sm" onClick={() => trace(s.id)}>{s.label} Trace</button>)}</div>
      {tab === 'input' && batch?.turn?.action ? <section aria-label="本次 Task Action"><h4>{batch.turn.action.name} · 版本 {batch.turn.action.revision.slice(0, 10)}</h4><dl>{Object.entries(batch.turn.action.values).map(([key,value]) => <div key={key}><dt>{batch.turn!.action!.parameters.find(p => p.key === key)?.label ?? key}</dt><dd>{String(value)}</dd></div>)}</dl><details><summary>本次 Action 快照</summary><pre>{JSON.stringify(batch.turn.action, null, 2)}</pre></details></section> : null}
      {tab === 'plan' ? <TaskDesignView design={plan.design} recipe={plan.workflowRecipe} /> : null}
      {tab === 'plan' ? <div role="tabpanel"><h3>{plan.title}</h3><p className="dtc-workflow-copy">{plan.brief}</p><p className="dtc-workflow-muted">决策依据：{view.origin?.reason || '历史记录未提供'}</p>
        <ol className="dtc-workflow-roles">{plan.participants.map((role, index) => <li key={`${role.agentId}-${index}`}><header><b>{index + 1}. {nameOf(role.agentId)}</b><span>{plan.graphMode === 'dynamic-rounds' ? ['初始规划 / 后续收口', '每轮执行', '每轮评估'][index] : index ? `依赖 ${nameOf(plan.participants[index - 1].agentId)} 完成交接` : '首个执行角色'}</span></header><p>{role.brief || '未单独设置角色任务书，使用总体目标。'}</p><code>{role.agentId}</code></li>)}{plan.design?.studioStages?.map(stage=><li key={stage.id}><header><b>{nameOf(stage.agentId)}</b><span>{stage.id==='storyboard'?'编导完成后执行':'分镜完成后执行；两路共同交接合成'}</span></header><p>{stage.brief}</p><code>{stage.agentId}</code></li>)}{plan.design?.notifications?.agentId && plan.design.notifications.mode !== 'final-handoff' ? <li><header><b>{nameOf(plan.design.notifications.agentId)}</b><span>辅助参与者 · 独立通知支线</span></header><p>按阶段创建自己的执行卡与会话，使用冻结报告发送；不阻塞浏览器修复。</p><code>{plan.design.notifications.agentId}</code></li> : null}</ol>
      ...
      : <div role="tabpanel"><p className="dtc-workflow-muted">{!batch ? '当前工作流定义；启动时才生成执行快照和真实 DAG。' : view.legacy ? '按已有记录展示的定义，并非补造历史模型输出。' : '本次执行已保存的工作流定义；不会随 Agent 预设修改而改变。'}模型原始提交请查看创建 Agent 的 task_create_submit 工具调用。</p><pre>{JSON.stringify({ planId: view.planId, decision: view.origin?.decision, reason: view.origin?.reason, ...plan }, null, 2)}</pre></div>}
    </div> : null}
  </section>
}
