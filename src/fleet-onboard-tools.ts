...
export class ScopedFleetMcpClient<Tool extends string> {
  ...
  async call(tool: Tool, args: Record<string, unknown>, signal?: AbortSignal): Promise<Record<string, unknown>> {
    ...
    const knownError = LEDGER_DIAGNOSTICS.get((payload as Record<string, unknown>).error)
    if (knownError && ((payload as Record<string, unknown>).ok === false || envelope.result.isError === true)) {
...
// Only fixed protocol messages may cross into Agent diagnostics, never arbitrary
// upstream error text (which can contain credentials or request bodies).
const LEDGER_DIAGNOSTICS = new Map<unknown, string>([
...
class LedgerDiagnosticError extends Error {}
  ...
  private async drive(run: FleetLedgerRun, inventory: Record<string, unknown>, assessment: Record<string, unknown>, history: Array<Record<string, unknown>>, env: NodeJS.ProcessEnv, signal?: AbortSignal): Promise<{ run: FleetLedgerRun; reason?: string; needsInput?: boolean }> {
    ...
    while (run.currentStage < 10) {
      ...
      const unattendedProbeRepair = stage.stage === 9 && probeGate && executionConfigured
        && observed.disposition === 'needs-user' && observed.reason === 'probe-gate-not-satisfied'
      ...
      if (effectiveDisposition !== 'repairable' || !executionConfigured || (probeGate && !unattendedProbeRepair)) {
        const reason = probeGate && !unattendedProbeRepair && effectiveDisposition === 'repairable'
          ? 'probe-gate-not-satisfied'
          : !executionConfigured && effectiveDisposition === 'repairable'
        ...
        const classified = probeGate && !unattendedProbeRepair && effectiveDisposition === 'repairable'
          ? 'needs-user'
          : !executionConfigured && effectiveDisposition === 'repairable' ? 'repairable' : failureClass(effectiveDisposition)
    ...
    try {
      return await withCredentialFile(lease, this.config, async env => {
        ...
        if (operation === 'start') {
          ...
          if (prior.run && ['running', 'blocked'].includes(prior.run.status)
            && prior.run.sessionId !== sessionId && prior.run.contractVersion === this.config.contractSha256
            && prior.run.executorVersion === this.config.executorVersion) {
            run = checkedRun(prior.run, this.config, ip, prior.run.sessionId)
            assertRunBinding(run, inventory, this.config)
            // Starting a new Agent session is not authority to steal its old
            // lease. Surface the existing transaction; resume owns the CAS.
            const expires = Date.parse(run.leaseExpiresAt ?? '')
            const waiting = run.status === 'running' && Number.isFinite(expires) && expires > Date.now()
            return {
              schema: 1, ok: false, operation, ip, phase: 'blocked', needs_input: false,
              execution_available: this.executionAvailable, probe_executed: true, run_created: false,
              run_id: run.id, revision: run.revision, current_stage: run.currentStage,
              reason: waiting ? 'ledger-session-lease-active' : 'ledger-explicit-handoff-required',
              can_resume: !waiting && this.executionAvailable, next_tool: waiting ? 'fleet_onboard_status' : 'fleet_onboard_resume',
              next_action: waiting ? 'Wait for the existing session lease to expire, then explicitly resume this transaction. Do not create another run.'
                : 'Call fleet_onboard_resume to claim this existing transaction with fresh inventory and CAS. Do not create another run.',
      ...
      return {
        ...
        diagnostic: { boundary, code: error instanceof AdapterProcessError ? error.diagnostic
          : error instanceof LedgerDiagnosticError ? error.message : 'adapter-result-validation-failed' },
