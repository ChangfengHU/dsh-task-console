...
export function registerStudioSkillGate(ctx:any,options:{input:any;isActive:()=>boolean;record:(receipt:any)=>void}){
 const {input}=options
 if(input.task?.design?.evidenceContract!=='studio-video-v1'||input.card?.role!=='executor')return()=>{}
 ...
 const stopResult=ctx.on('tools/result',(exec:any,result:any)=>{
  ...
  if(!REQUIRED_PRODUCTION_SKILLS.includes(name)||typeof exec.callId!=='string'||!exec.callId)return
 ...
 const stopGuard=ctx.tools.guard((exec:any)=>{
  ...
  const missing=REQUIRED_PRODUCTION_SKILLS.filter(name=>!loaded.has(name))
