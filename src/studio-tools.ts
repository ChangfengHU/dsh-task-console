...
export const STUDIO_TOOL_NAMES=['studio_status','studio_register_candidate','studio_read_text','studio_inspect_frames','studio_inspect_probe','studio_inspect_audio','studio_submit_review','studio_reference_frames','studio_reference_audio','studio_character_image','studio_preview_audio'] as const
...
export async function registerStudioTools(agentCtx:any,options:StudioToolOptions):Promise<()=>void>{
 ...
 register('studio_reference_frames','Planner/producer/reviewer: see 8 actual frames of the host-frozen reference film, up to 2 seconds. Reference observations never count as candidate evidence.',{start:{type:'number',required:true},end:{type:'number',required:true}},async args=>{
  ...
  if(await fileSha256(ref.path)!==ref.sha256)throw Error('studio-reference-file-changed');check();const receipt=await options.referenceReceipt?.({referenceSha256:ref.sha256,kind:'frames',ranges:[[start,end]],sha256:hash(JSON.stringify(frames))});return {scope:'reference-only',referenceSha256:ref.sha256,frames,images,referenceReceipt:receipt,sampling:'8 ordered samples; not continuous coverage'}
 },true)
 ...
 register('studio_character_image','Planner/producer/reviewer: view a host-locked character reference by id. No arbitrary path access.',{id:{type:'string',required:true}},async args=>{
  requireRole(['planner','executor','reviewer']);const ref=options.characterReferences?.find(v=>v.id===args.id);if(!ref||!/^[a-f0-9]{64}$/.test(ref.sha256))throw Error('studio-character-lock-required');const attachments=attachmentStore();const path=await realpath(ref.path);if(!(await stat(path)).isFile()||(await stat(path)).size>20*1024*1024)throw Error('studio-character-image-invalid');const bytes=await readFile(path);if(hash(bytes)!==ref.sha256)throw Error('studio-character-file-changed');const mediaType=bytes[0]===255&&bytes[1]===216?'image/jpeg':bytes.subarray(0,8).equals(Buffer.from([137,80,78,71,13,10,26,10]))?'image/png':null;if(!mediaType)throw Error('studio-character-image-invalid');check();const attachment=await attachments.saveImage({data:bytes,mediaType,name:basename(path)});check();return {scope:'character-reference-only',id:ref.id,sha256:ref.sha256,images:[attachment]}
 },true)
