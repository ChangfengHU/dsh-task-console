...
export async function registerStudioTools(agentCtx:any,options:StudioToolOptions):Promise<()=>void>{
 ...
 register('studio_render_start','Executor only: submit a real composition to the pinned host HyperFrames runtime. Returns a durable jobId immediately; poll studio_render_status. Same input/output reuses the original job. Uses host rendering dependencies, not npx or system FFmpeg. No placeholder output and no quality approval.',{composition:{type:'string',required:true},output:{type:'string',required:true}},async args=>{requireRole(['executor']);if(!options.renderJob)throw Error('studio-render-host-not-configured');return options.renderJob('start',args)})
 register('studio_render_status','Executor only: inspect an existing host render job by jobId, including after chat reconnect. Do not resubmit a running or unknown job. Completed means technical rendering only; register and review the actual MP4 next.',{jobId:{type:'string',required:true}},async args=>{requireRole(['executor']);if(!options.renderJob)throw Error('studio-render-host-not-configured');return options.renderJob('status',args)})
 register('studio_download_asset','Producer/specialist: download an existing archived asset by its exact library ID to a new project-relative path. Host handles authentication and checks actual size/SHA; do not read or print credentials or invent /file URLs. Source-only cards have no archived file: use the original source and verify permission separately. No synthesis, overwrite, or quality approval.',{id:{type:'string',required:true},path:{type:'string',required:true}},async args=>{requireRole(['executor','studio-stage']);if(!options.downloadAsset)throw Error('studio-asset-download-host-not-configured');return options.downloadAsset(args)})
 register('studio_register_stage','Preparation specialist only: verify actual stage manifest and files. Use stages/r<round>/<stage>/manifest.json with {stage,round,outputs:[relative files],summary}. Does not approve final film.',{path:{type:'string',required:true}},async args=>{requireRole(['studio-stage']);if(!options.registerStage)throw Error('studio-stage-capability-required');return options.registerStage(args.path)})
 ...
 register('studio_preview_image','Producer only: inspect actual project PNG/JPEG via the host visual observer. No independent QA receipt.',{path:{type:'string',required:true}},async args=>{
  requireRole(['executor','studio-stage']);if(!options.visionObserve)throw Error('studio-vision-capability-required');const path=await studioPath(input.task.cwd,args.path,true),sha256=await fileSha256(path),observation=await observe([{path,sha256}],'preview');return {scope:'producer-self-check-only',sourcePath:relative(input.task.cwd,path),sha256,observation,qualityApproved:false,independentReview:false}
 })
 register('studio_preview_frames','Producer only: inspect 8 ordered frames of a project pilot or MP4 across at most 2 seconds. No independent QA receipt.',{path:{type:'string',required:true},start:{type:'number',required:true},end:{type:'number',required:true}},async args=>{
  requireRole(['executor','studio-stage']);if(!options.visionObserve)throw Error('studio-vision-capability-required');const path=await studioPath(input.task.cwd,args.path),sourceSha256=await fileSha256(path),probe=JSON.parse((await command(process.env.FFPROBE_PATH??'ffprobe',['-v','error','-show_streams','-show_format','-of','json',path])).stdout),duration=Number(probe.format?.duration)
