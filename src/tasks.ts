...
export class EventStore {
  ...
  graphSnapshot(taskId: string, batchId: string, after?: number): GraphSnapshot {
    const db = this.kernel.db
    ...
    const eventRows = (after === undefined
      ...
      : db.prepare(`SELECT id,graph_id,task_id,run_id,kind,payload,created_at FROM task_events WHERE graph_id=? AND id>? ORDER BY id LIMIT 201`).all(batchId, after)) as { id: number; graph_id: string; task_id: string; run_id: number | null; kind: string; payload: string | null; created_at: number }[]
    const hasMore = after !== undefined && eventRows.length > 200
    if (hasMore) eventRows.pop()
    ...
    const phases = after === undefined ? events : db.prepare(`SELECT run_id,kind FROM task_events WHERE graph_id=? AND kind IN ('claimed','run_bound','session_created','prompt_dispatched','heartbeat','completed') ORDER BY id`).all(batchId) as {run_id:number|null;kind:string}[]
