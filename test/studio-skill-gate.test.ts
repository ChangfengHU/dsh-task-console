...
import {registerStudioSkillGate,REQUIRED_PRODUCTION_SKILLS} from '../src/studio-skill-gate.ts'
import {createRequire} from 'node:module'
import {dirname} from 'node:path'
import {pathToFileURL} from 'node:url'
import {ToolRuntime,defineTool} from '@deepseek-ai/dsh-tools'

function fixture(role='executor'){
 ...
 const input={task:{design:{evidenceContract:'studio-video-v1'}},card:{role},sessionId:'s'}
...
test('real native ToolRuntime result observer unlocks writes only after successful instruction results',async()=>{
 ...
 const stop=registerStudioSkillGate({tools:runtime,on:ctx.on.bind(ctx)},{input:{task:{design:{evidenceContract:'studio-video-v1'}},card:{role:'executor'},sessionId:agent.session.id},isActive:()=>true,record:r=>records.push(r)})
 let writes=0
 ...
 try{
  ...
  for(const name of REQUIRED_PRODUCTION_SKILLS)assert.equal((await run('skill',{name})).isError,false)
  assert.equal(records.length,3);assert.equal((await run('write')).isError,false);assert.equal(writes,1)
 }finally{stop();d1();d2()}
})
